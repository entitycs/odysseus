  ...
  try {
    ...
    data.users.forEach((u) => {
      ...
      if (!u.is_admin) {
        ...
        privPanel.style.cssText =
          'padding:8px 0 4px;border-top:1px solid var(--border);margin-top:8px;';

        // Boolean toggles
        let html =
          '<div style="font-size:10px;text-transform:uppercase;letter-spacing:0.5px;opacity:0.35;font-weight:600;margin-bottom:4px;">Features</div>';
        ...
        // Rate limit
        html +=
          '<div style="font-size:10px;text-transform:uppercase;letter-spacing:0.5px;opacity:0.35;font-weight:600;margin:10px 0 4px;">Limits</div>';
        ...
        const allowedModels = Array.isArray(
          u.privileges && u.privileges.allowed_models,
        )
        ...
        header.addEventListener('click', (e) => {
          if (
            e.target.closest(
              '.admin-btn-delete, [data-adm-rename-user], [data-adm-toggle-admin]',
            )
          )
            return;
        ...
        privPanel.querySelectorAll('[data-priv]').forEach((input) => {
          const handler = async () => {
            ...
            try {
              ...
              uiModule.showError('Failed to update privilege');
            }
          };
        ...
        renameBtn.addEventListener('click', async (e) => {
          ...
          try {
            const res = await fetch(
              `/api/auth/users/${encodeURIComponent(oldUsername)}/rename`,
              {
                method: 'PUT',
                credentials: 'same-origin',
                headers: { 'Content-Type': 'application/json' },
      ...
      if (delBtn) {
        delBtn.addEventListener('click', async (e) => {
          ...
          if (
            !(await uiModule.styledConfirm(`Remove user "${username}"?`, {
              confirmText: 'Remove',
              danger: true,
            }))
          )
      ...
      if (adminToggleBtn) {
        adminToggleBtn.addEventListener('click', async (e) => {
          ...
          if (
            !(await uiModule.styledConfirm(confirmMsg, {
              confirmText: makeAdmin ? 'Make admin' : 'Revoke admin',
              danger: !makeAdmin,
            }))
          )
            return;
          ...
          try {
            const res = await fetch(
              `/api/auth/users/${encodeURIComponent(username)}/admin`,
              {
                method: 'PUT',
                credentials: 'same-origin',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (!res.ok) {
              uiModule.showError(
                data.detail || 'Failed to change admin status',
              );
...
async function _loadModelsForUser(
  ...
  const listEl = privPanel.querySelector(
    `.priv-models-list[data-user="${username}"]`,
  );
  if (!listEl) return;
  try {
    ...
    if (!allModels.length) {
      listEl.innerHTML =
        '<span style="opacity:0.4;font-size:11px;">No models available</span>';
      return;
    }
    ...
    listEl
      .querySelectorAll('.priv-model-cb')
      .forEach((cb) => cb.addEventListener('change', _saveModels));

    // All / None buttons
    privPanel
      .querySelector(`.priv-models-all[data-user="${username}"]`)
    ...
    privPanel
      .querySelector(`.priv-models-none[data-user="${username}"]`)
      ?.addEventListener('click', (e) => {
        e.preventDefault();
        listEl
          .querySelectorAll('.priv-model-cb')
    ...
    listEl.innerHTML =
      '<span style="opacity:0.4;font-size:11px;">Failed to load models</span>';
  }
}
...
function initSignupToggle() {
  ...
  toggle.addEventListener('change', async () => {
    try {
      const res = await fetch('/api/auth/signup-toggle', {
        method: 'POST',
        credentials: 'same-origin',
      });
      const data = await res.json();
...
function initShareDefaultsToggle() {
  ...
  fetch('/api/auth/settings', { credentials: 'same-origin' })
    .then((r) => r.json())
    .then((d) => {
      toggle.checked = !!d.share_defaults_with_users;
    })
...
function initAddUser() {
  fetch('/api/auth/policy', { credentials: 'same-origin' })
    ...
    .then((policy) => {
      ...
      if (admPw)
        admPw.placeholder = `Password (min ${policy.password_min_length})`;
    })
    .catch(() => {});
  el('adm-addBtn').addEventListener('click', async () => {
    ...
    if (password.length < _authPolicy.password_min_length) {
      msg.textContent = `Password must be at least ${_authPolicy.password_min_length} characters`;
      msg.className = 'admin-error';
      return;
    }
    if (_authPolicy.reserved_usernames.includes(username.toLowerCase())) {
      msg.textContent = 'This username is reserved';
      msg.className = 'admin-error';
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        el('adm-newUsername').value = '';
        el('adm-newPassword').value = '';
        el('adm-newIsAdmin').checked = false;
        loadUsers();
      } else {
...
async function _refreshAfterEndpointChange(deletedEndpointId) {
  try {
    ...
    if (
      deletedEndpointId &&
      pending &&
      String(pending.endpointId || '') === String(deletedEndpointId)
    ) {
...
async function _selectAddedModelInChat(endpoint) {
  const modelId =
    endpoint && Array.isArray(endpoint.models) ? endpoint.models[0] : '';
  if (!modelId) return;
  try {
    if (window.modelsModule && window.modelsModule.refreshModels) {
      await window.modelsModule.refreshModels(true);
  ...
  try {
    document.dispatchEvent(
      new CustomEvent('odysseus:auto-select-model', {
        detail: {
...
async function loadEndpoints() {
  ...
  if (
    settingsModule &&
    typeof settingsModule.refreshAiModelEndpoints === 'function'
  ) {
  ...
  try {
    ...
    const rowHtml = data.map((ep) => {
      ...
      const justAddedClass =
        _recentlyAddedEpId && String(ep.id) === _recentlyAddedEpId
          ? ' adm-ep-just-added'
          : '';
      const category =
        ep.category || (_isLocalEndpoint(ep.base_url) ? 'local' : 'api');
      const kindLabel =
        ep.endpoint_kind && ep.endpoint_kind !== 'auto'
          ? ep.endpoint_kind.toUpperCase()
          : '';
      const keyLabel = ep.has_key
        ? ep.api_key_fingerprint
          ? ` (key ${esc(ep.api_key_fingerprint)})`
          : ' (key set)'
        : '';
      return `
    ...
    const _sortByEnabled = (a, b) =>
      Number(!!data[b].is_enabled) - Number(!!data[a].is_enabled);
    ...
    queryAll('[data-adm-toggle-ep]').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation();
        await fetch(`/api/model-endpoints/${btn.dataset.admToggleEp}`, {
          method: 'PATCH',
        });
        loadEndpoints();
      });
    ...
    queryAll('[data-adm-del-ep]').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        if (!isOffline) {
          var deps = [];
          try {
            var depRes = await fetch(
              '/api/model-endpoints/' + epId + '/dependents',
              { credentials: 'same-origin' },
            );
          ...
          if (
            !(await uiModule.styledConfirm(msg, {
              confirmText: 'Delete',
              danger: true,
            }))
          )
    ...
    queryAll('[data-adm-ep-id]').forEach((row) => {
      ...
      row.addEventListener('click', async (e) => {
        ...
        if (
          e.target.closest(
            '.admin-btn-sm, .admin-btn-delete, .mcp-tools-list, .mcp-tools-header, .mcp-tools-search, input, label',
          )
        )
          return;
        ...
        if (!_modelsLoaded && isOpen) {
          ...
          _ld.style.cssText =
            'opacity:0.55;font-size:11px;display:inline-flex;align-items:center;gap:8px;';
          ...
          try {
            ...
            _modelsSpin.element.style.cssText =
              'width:14px;height:14px;margin:0;display:inline-block;';
            _ld.appendChild(_modelsSpin.element);
          } catch (_) {}
          panel.appendChild(_ld);
          const _stopSpin = () => {
            try {
              _modelsSpin && _modelsSpin.stop();
            } catch (_) {}
          };
          const _loadingHtml = (label) =>
            `<span style="opacity:0.55;font-size:11px;display:inline-flex;align-items:center;gap:8px;">${esc(label)}</span>`;
          const renderModels = (models, warning = '') => {
            const sortedModels = sortModelObjects(models);
            const warningHtml = warning
              ? `<div class="admin-error" style="font-size:11px;margin:6px 0;">${esc(warning)}</div>`
              : '';
            const attachRefresh = () => {
              panel
                .querySelector(`[data-ep-refresh-models="${epId}"]`)
                ?.addEventListener('click', async (e) => {
                  e.preventDefault();
                  panel.innerHTML = _loadingHtml('Refreshing models...');
                  try {
                    const res = await fetch(
                      `/api/model-endpoints/${epId}/models?refresh=true&refresh_timeout=60`,
                      { credentials: 'same-origin' },
                    );
                    const refreshWarning =
                      res.headers.get('X-Model-Refresh-Warning') || '';
                    if (!res.ok) throw new Error(`HTTP ${res.status}`);
                    const refreshedModels = await res.json();
                    renderModels(refreshedModels, refreshWarning);
                    if (refreshWarning && uiModule?.showToast)
                      uiModule.showToast(refreshWarning, 6000);
                  } catch (_) {
                    renderModels(
                      sortedModels,
                      'Model refresh failed; kept cached models.',
                    );
                  }
                });
            };
            ...
            const hiddenSet = new Set(
              sortedModels.filter((m) => m.is_hidden).map((m) => m.id),
            );
            ...
            </div>${warningHtml}${showSearch ? `<input type="search" class="mcp-tools-search" placeholder="Search ${sortedModels.length} models..." data-ep-search="${epId}">` : ''}<div class="mcp-tools-list">` +
              sortedModels
                .map(
                  (m) =>
            ...
            const filterRows = (q) => {
              ...
              panel.querySelectorAll('[data-ep-model-row]').forEach((row) => {
                row.style.display =
                  !needle || row.dataset.search.includes(needle) ? '' : 'none';
              });
            };
            attachRefresh();
            panel
              .querySelector(`[data-ep-search="${epId}"]`)
              ?.addEventListener('input', (e) => filterRows(e.target.value));
            panel
              .querySelector(`[data-ep-select-all="${epId}"]`)
              ?.addEventListener('click', (e) => {
                e.preventDefault();
                panel.querySelectorAll('[data-ep-model-row]').forEach((row) => {
            ...
            panel
              .querySelector(`[data-ep-select-none="${epId}"]`)
              ?.addEventListener('click', (e) => {
                e.preventDefault();
                panel.querySelectorAll('[data-ep-model-row]').forEach((row) => {
                  if (row.style.display !== 'none')
          ...
          try {
            ...
            _stopSpin();
            panel.innerHTML =
              '<span class="admin-error" style="font-size:11px;">Failed to load models</span>';
          }
        }
      });
    });
...
async function _saveEpModelState(epId, panel) {
  ...
  try {
    ...
    if (countLabel)
      countLabel.textContent = `${total - hidden.length}/${total} enabled`;
    ...
    if (row) {
      ...
      if (badge && !badge.classList.contains('admin-badge-off'))
        badge.textContent = `${total - hidden.length}/${total} models enabled`;
    }
    if (
      settingsModule &&
      typeof settingsModule.refreshAiModelEndpoints === 'function'
...
function initEndpointForm() {
  ...
  function _normalizeBaseUrl(raw) {
    ...
    if (
      ...
      !u.includes('opencode.ai') &&
      !u.includes('ollama.com') &&
      !u.endsWith('/v1')
    ) {
      try {
        const parsed = new URL(u);
  ...
  function _renderEndpointTestResult(msg, res, d) {
    ...
    msg.textContent =
      (d && d.detail) ||
      (d && d.ping_error ? `Offline — ${d.ping_error}` : 'Offline');
    msg.className = 'admin-error';
  }

  function _endpointMsg(kind) {
    return (
      el(kind === 'local' ? 'adm-epLocalMsg' : 'adm-epApiMsg') ||
      el('adm-epMsg')
    );
  }
  ...
  if (apiTestBtn) {
    apiTestBtn.addEventListener('click', async () => {
      ...
      if (!rawUrl) {
        msg.textContent = 'Select a provider or enter a base URL';
        msg.className = 'admin-error';
        return;
      }
      ...
      const url =
        provider.value && rawUrl === provider.value
          ? rawUrl
      ...
      try {
        ...
        if (e && e.name === 'AbortError') {
          ...
          msg.textContent =
            'Test failed: ' + (e && e.message ? e.message : 'request failed');
          msg.className = 'admin-error';
        }
      }
  ...
  el('adm-epAddBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      fd.append(
        'model_refresh_mode',
        endpointKind === 'proxy' ? 'manual' : 'auto',
      );
      fd.append('model_refresh_timeout', '30');
      if (apiKey) fd.append('api_key', apiKey);
      if (
      ...
      if (provider.value && /openrouter\.ai|ollama\.com/i.test(provider.value))
      ...
      if (res.ok) {
        ...
        const goLink =
          ' <a href="#" data-go-added-models style="margin-left:6px;text-decoration:underline;color:inherit;font-weight:600;">Added Models →</a>';
        if (!d.online) {
          msg.innerHTML =
            'Added (endpoint offline — will retry on next load)' + goLink;
          msg.className = 'admin-error';
        } else if (d.status === 'empty') {
          msg.innerHTML =
            'Added — endpoint reachable, no models found' + goLink;
          msg.className = 'admin-success';
        } else {
          msg.innerHTML =
            `Added — found ${count} model${count !== 1 ? 's' : ''}` + goLink;
          msg.className = 'admin-success';
        }
      } else {
      ...
      opts.forEach((o) => {
        ...
        if (check)
          check.style.visibility =
            o.dataset.kind === kind.value ? 'visible' : 'hidden';
      });
    };
    sync();
  ...
  if (localTestBtn) {
    localTestBtn.addEventListener('click', async () => {
      ...
      if (!raw) {
        msg.textContent = 'Enter a base URL to test';
        msg.className = 'admin-error';
        return;
      }
      ...
      localTestBtn.innerHTML = testOriginalHtml.replace(
        />Test\s*$/,
        '>Testing...',
      );
      try {
  ...
  if (localAddBtn) {
    localAddBtn.addEventListener('click', async () => {
      ...
      const raw = (el('adm-epLocalUrl').value || '').trim();
      if (!raw) {
        msg.textContent = 'Enter a base URL (e.g. http://localhost:8002/v1)';
        msg.className = 'admin-error';
        return;
      }
      const url = _normalizeBaseUrl(raw);
  ...
  if (ollamaBtn) {
    ollamaBtn.addEventListener('click', async () => {
      ...
      if (msg) {
        msg.innerHTML =
          '<span style="font-size:11px;opacity:0.55;">Ollama ready to test.</span>';
        msg.className = '';
      }
    });
  }
  ...
  if (discoverBtn) {
    discoverBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        txt.textContent =
          'Scanning ports 8000-8020, 8080, 1234, 11434, and 11435 for model servers...';
      ...
      try {
        ...
        if (!items.length) {
            ...
            if (r.ok) {
              try {
                ...
                if (dd && dd.existing) {
                  ...
                  added++;
                  if (dd && dd.id) _recentlyAddedEpId = String(dd.id);
                }
              } catch (_) {
                added++;
              }
          ...
          const totalModels = items.reduce(
            (n, i) => n + (i.models ? i.models.length : 0),
            0,
          );
          const serverNames = items.map(
            (i) =>
              _PROVIDER_DISPLAY[i.provider] ||
              i.url.replace(/^https?:\/\//, '').split('/')[0],
          );
          const parts = [
      ...
      if (discoverBtn._wp) {
        discoverBtn._wp.destroy();
        discoverBtn._wp = null;
      }
...
async function loadBuiltinTools() {
  ...
  try {
    ...
    async function _saveToolState() {
      ...
      const disabled = [];
      allChecks.forEach((c) => {
        if (!c.checked) disabled.push(c.dataset.toolId);
      });
...
async function loadMcpServers() {
  ...
  try {
    ...
    if (!servers.length) {
      list.innerHTML =
        '<div class="admin-empty">No MCP servers configured</div>';
      return;
    }
    list.innerHTML = servers
      .map((s) => {
        const statusColor = s.needs_oauth
          ? '#e5a33a'
          : s.status === 'connected'
            ? 'var(--fg)'
            : s.status === 'error'
        ...
        const toolInfo =
          s.status === 'connected'
            ? `${s.enabled_tool_count}/${s.tool_count} tools enabled`
            : '';
    ...
    list.querySelectorAll('[data-adm-mcp-reconnect]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        ...
        try {
          const res = await fetch(
            `/api/mcp/servers/${btn.dataset.admMcpReconnect}/reconnect`,
            { method: 'POST', credentials: 'same-origin' },
          );
          const data = await res.json();
          msg.textContent = data.connected
            ? `Reconnected (${data.tool_count} tools)`
    ...
    list.querySelectorAll('[data-adm-mcp-toggle]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        const fd = new FormData();
        fd.append('is_enabled', btn.dataset.admMcpEnable);
        await fetch(`/api/mcp/servers/${btn.dataset.admMcpToggle}`, {
          method: 'PATCH',
          body: fd,
          credentials: 'same-origin',
        });
    ...
    list.querySelectorAll('[data-adm-mcp-delete]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        if (
          !(await uiModule.styledConfirm('Delete this MCP server?', {
            confirmText: 'Delete',
            danger: true,
          }))
        )
          return;
        await fetch(`/api/mcp/servers/${btn.dataset.admMcpDelete}`, {
          method: 'DELETE',
          credentials: 'same-origin',
        });
        loadMcpServers();
    ...
    list.querySelectorAll('[data-adm-mcp-id]').forEach((row) => {
      ...
      row.addEventListener('click', async (e) => {
        if (
          e.target.closest(
            '.admin-btn-sm, .admin-btn-delete, a, .mcp-tools-list, .mcp-tools-header',
          )
        )
          return;
        ...
        if (!_toolsLoaded && isOpen) {
          _toolsLoaded = true;
          panel.innerHTML =
            '<span style="opacity:0.5;font-size:11px;">Loading tools...</span>';
          try {
            const res = await fetch(`/api/mcp/servers/${sid}/tools`, {
              credentials: 'same-origin',
            });
            const tools = await res.json();
            if (!tools.length) {
              panel.innerHTML =
                '<span style="opacity:0.5;font-size:11px;">No tools</span>';
              return;
            }
            ...
            panel.innerHTML =
              ...
              '</div>';
            panel
              .querySelector(`[data-mcp-select-all="${sid}"]`)
              ?.addEventListener('click', (e) => {
                e.preventDefault();
                panel
            ...
            panel
              .querySelector(`[data-mcp-select-none="${sid}"]`)
              ?.addEventListener('click', (e) => {
                e.preventDefault();
                panel
                  .querySelectorAll('input[type=checkbox]')
            ...
            panel.innerHTML =
              '<span class="admin-error" style="font-size:11px;">Failed to load tools</span>';
          }
        }
      });
    });
    ...
    if (list)
      list.innerHTML =
        '<div class="admin-error">Failed to load MCP servers</div>';
  }
}
...
async function _saveMcpToolState(serverId, panel) {
  ...
  try {
    ...
    const countLabel = panel.querySelector('.mcp-tools-count');
    if (countLabel)
      countLabel.textContent = `${total - disabled.length}/${total} enabled`;
    // Update badge in the server row
    const row = panel.closest('[data-adm-mcp-id]');
    if (row) {
      const badge = row.querySelector('.admin-badge');
      if (badge)
        badge.textContent = `Connected (${total - disabled.length}/${total} tools enabled)`;
    }
  } catch (e) {
    /* silent */
  }
...
function initMcpForm() {
  ...
  function _buildEnvFields(envObj, help, preset) {
    ...
    if (preset?.providerDropdown) {
      ...
      label.style.cssText =
        'font-size:11px;opacity:0.55;min-width:0;white-space:nowrap;';
      label.textContent = pd.label || 'Provider';
      const select = document.createElement('select');
      select.style.cssText =
        'flex:1;padding:6px 8px;border-radius:6px;border:1px solid var(--border);background:var(--bg-secondary);color:var(--text-primary);font-size:12px;';
      ...
      select.addEventListener('change', () => {
        ...
        for (const [envKey, field] of Object.entries(pd.targets)) {
          const inp = envFieldsWrap.querySelector(
            `.mcp-env-input[data-env-key="${envKey}"]`,
          );
    ...
    if (help) {
      ...
      helpLink.style.cssText =
        'font-size:10.5px;opacity:0.5;margin-top:2px;display:inline-block;';
  ...
  if (presetSel) {
    MCP_PRESETS.forEach((p, i) => {
      ...
      opt.value = i;
      opt.textContent =
        p.name + (Object.keys(p.env).length ? '  (requires keys)' : '');
  ...
  el('adm-mcpAddBtn').addEventListener('click', async () => {
    ...
    if (transport === 'stdio' && !command) {
      msg.textContent = 'Command is required for stdio';
      msg.className = 'admin-error';
      return;
    }
    if (transport === 'sse' && !url) {
      msg.textContent = 'URL is required for SSE';
      msg.className = 'admin-error';
      return;
    }
    try {
      ...
      msg.textContent = 'Env must be valid JSON';
      msg.className = 'admin-error';
      return;
    }
    const fd = new FormData();
    ...
    try {
      ...
      if (data.needs_oauth) {
        ...
        msg.textContent = `Added ${name} (${data.tool_count} tools discovered)`;
        msg.className = 'admin-success';
      } else {
        msg.textContent = `Added but connection failed: ${data.error || 'unknown'}`;
        msg.className = 'admin-error';
      }
      el('adm-mcpName').value = '';
      el('adm-mcpCommand').value = '';
      el('adm-mcpArgs').value = '';
      el('adm-mcpUrl').value = '';
      _clearEnvFields();
      helpBox.style.display = 'none';
      _activeHelp = null;
      _activeOauthFile = null;
      _activeOauth = null;
      loadMcpServers();
    } catch (e) {
      msg.textContent = 'Failed: ' + e.message;
...
async function loadRag() {
  try {
    ...
    if (dirs.length === 0) {
      dirList.innerHTML =
        '<div class="admin-empty">No directories indexed</div>';
    } else {
      dirList.innerHTML = dirs
        .map(
          (d) =>
            `<div class="admin-rag-item"><span class="admin-rag-item-name" title="${esc(d)}">${esc(d)}</span><button class="admin-btn-delete" data-adm-rag-dir="${esc(d)}">Remove</button></div>`,
        )
        .join('');
        ...
        btn.addEventListener('click', async () => {
          if (
            !(await uiModule.styledConfirm(
              `Remove directory "${btn.dataset.admRagDir}" from RAG?`,
          ...
          try {
            const res = await fetch(
              '/api/personal/remove_directory?directory=' +
                encodeURIComponent(btn.dataset.admRagDir),
              { method: 'DELETE' },
            );
            if (res.ok) {
              ragMsg('Directory removed');
              loadRag();
            } else {
              const e = await res.json();
              ragMsg(e.detail || 'Failed', true);
            }
          } catch (e) {
            ragMsg('Error: ' + e.message, true);
          }
        });
      });
    }
    ...
    if (files.length === 0) {
      fileList.innerHTML = '<div class="admin-empty">No files indexed</div>';
    } else {
      ...
      fileList.querySelectorAll('[data-adm-rag-file]').forEach((btn) => {
        btn.addEventListener('click', async () => {
          if (
            !(await uiModule.styledConfirm(
              `Delete "${btn.dataset.admRagFile}" from RAG?`,
          ...
          try {
            const res = await fetch(
              '/api/personal/file?filepath=' +
                encodeURIComponent(btn.dataset.admRagFile),
              { method: 'DELETE' },
            );
            if (res.ok) {
              ragMsg('File removed');
    ...
    el('adm-ragDirList').innerHTML =
      '<div class="admin-error">Failed to load</div>';
...
function ragMsg(text, isError, persist) {
  ...
  s.textContent = text;
  s.style.color = isError ? 'var(--red)' : 'var(--fg)';
  if (_ragMsgTimer) {
    clearTimeout(_ragMsgTimer);
    _ragMsgTimer = null;
  }
  if (text && !persist)
    _ragMsgTimer = setTimeout(() => {
      s.textContent = '';
    }, 5000);
}
...
async function ragUpload(files) {
  ...
  try {
    ...
    if (data.success) {
      ragMsg(
        `Uploaded ${data.uploaded.length} file(s), ${data.indexed_count} chunks indexed`,
      );
      loadRag();
    } else ragMsg(data.detail || 'Upload failed', true);
  } catch (e) {
    ragMsg('Upload error: ' + e.message, true);
  }
}
...
function initRag() {
  ...
  dropZone.addEventListener('drop', (e) => {
    e.preventDefault();
    dropZone.classList.remove('dragover');
    ragUpload(e.dataTransfer.files);
  });
  el('adm-ragAddDirBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        ragMsg(`Indexed ${data.indexed_count} chunks from directory`);
        el('adm-ragDirInput').value = '';
        loadRag();
  ...
  el('adm-ragReloadBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      ragMsg('Reload failed: ' + e.message, true);
    }
    btn.disabled = false;
    btn.textContent = 'Reload Index';
  });
}
...
const _TOKEN_SCOPES = [
  ...
  },
  {
    key: 'cookbook:read',
    label: 'Cookbook read',
    detail: 'List cookbook tasks + tail their tmux output',
  },
  {
    key: 'cookbook:launch',
    label: 'Cookbook launch',
    detail: 'Launch and stop cookbook serve tasks',
  },
];
...
async function loadTokens() {
  ...
  try {
    ...
    if (!tokens.length) {
      list.innerHTML =
        '<div class="admin-empty" style="color:var(--accent, var(--red));opacity:0.7;font-size:10px;">No API tokens</div>';
      return;
    }
    ...
    list.querySelectorAll('[data-adm-del-token]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        if (
          !(await uiModule.styledConfirm(
            'Revoke this API token? External integrations using it will stop working.',
            { confirmText: 'Revoke', danger: true },
          ))
        )
          return;
        await fetch(`/api/tokens/${btn.dataset.admDelToken}`, {
          method: 'DELETE',
          credentials: 'same-origin',
        });
        loadTokens();
        ...
        try {
          window.dispatchEvent(
            new CustomEvent('odysseus-integrations-changed'),
          );
        } catch (_) {}
      });
    });
    ...
    list.querySelectorAll('[data-adm-tok-toggle]').forEach((btn) => {
      btn.addEventListener('click', () => {
        const panel = list.querySelector(
          `[data-adm-tok-perm="${btn.dataset.admTokToggle}"]`,
        );
        if (!panel) return;
    ...
    list.querySelectorAll('.adm-tok-scope').forEach((cb) => {
      cb.addEventListener('change', async () => {
        ...
        const msg = list.querySelector(
          `.adm-tok-scope-msg[data-token-id="${tokenId}"]`,
        );
        const scopes = Array.from(
          panel.querySelectorAll('.adm-tok-scope:checked'),
...
async function loadWebhooks() {
  ...
  try {
    ...
    if (!hooks.length) {
      list.innerHTML = '<div class="admin-empty">No webhooks configured</div>';
      return;
    }
    list.innerHTML = hooks
      .map((w) => {
        const events = (w.events || [])
          .map((e) => `<span class="admin-badge">${esc(e)}</span>`)
          .join(' ');
        ...
        const lastTriggered = w.last_triggered_at
          ...
          : 'Never';
        const errorText = w.last_error
          ? `<div style="font-size:0.75rem;color:var(--red);margin-top:0.2rem;">Error: ${esc(w.last_error.substring(0, 80))}</div>`
          : '';
        return `
    ...
    list.querySelectorAll('[data-adm-wh-test]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        ...
        try {
          const res = await fetch(
            `/api/webhooks/${btn.dataset.admWhTest}/test`,
            { method: 'POST', credentials: 'same-origin' },
          );
          msg.textContent = res.ok ? 'Test sent!' : 'Test failed';
          msg.className = res.ok ? 'admin-success' : 'admin-error';
    ...
    list.querySelectorAll('[data-adm-wh-toggle]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        await fetch(`/api/webhooks/${btn.dataset.admWhToggle}`, {
          method: 'PATCH',
          credentials: 'same-origin',
        });
        loadWebhooks();
    ...
    list.querySelectorAll('[data-adm-wh-delete]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        if (
          !(await uiModule.styledConfirm('Delete this webhook?', {
            confirmText: 'Delete',
            danger: true,
          }))
        )
          return;
        await fetch(`/api/webhooks/${btn.dataset.admWhDelete}`, {
          method: 'DELETE',
          credentials: 'same-origin',
        });
        loadWebhooks();
    ...
    list.innerHTML = '<div class="admin-error">Failed to load webhooks</div>';
  }
}
...
function initWebhookForm() {
  el('adm-whAddBtn').addEventListener('click', async () => {
    ...
    const events = Array.from(modalEl.querySelectorAll('.adm-wh-event:checked'))
      .map((e) => e.value)
      .join(',');
    if (!name) {
      msg.textContent = 'Name is required';
    ...
    try {
      ...
      if (res.ok) {
        msg.textContent = 'Webhook added';
        msg.className = 'admin-success';
        el('adm-whName').value = '';
        el('adm-whUrl').value = '';
        el('adm-whSecret').value = '';
        loadWebhooks();
      } else {
        const d = await res.json();
        msg.textContent = d.detail || 'Failed';
...
async function loadFeatures() {
  ...
  try {
    ...
    container.innerHTML = Object.entries(featureLabels)
      .map(
        ([key, label]) => `
    ...
    container.querySelectorAll('input[data-adm-feature]').forEach((toggle) => {
      toggle.addEventListener('change', async () => {
        const body = {};
        body[toggle.dataset.admFeature] = toggle.checked;
        await fetch('/api/auth/features', {
          method: 'POST',
          credentials: 'same-origin',
          headers: { 'Content-Type': 'application/json' },
    ...
    container.innerHTML =
      '<div class="admin-error">Failed to load features</div>';
  }
}
...
function initCalDAV() {
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/calendar/config`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          caldav_url: urlIn.value,
          caldav_username: userIn.value,
          caldav_password: passIn.value,
        }),
      });
      const d = await res.json();
  ...
  testBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/calendar/test`, {
        method: 'POST',
        credentials: 'same-origin',
      });
      const d = await res.json();
      status.textContent = d.ok
        ? `Connected (${d.calendars} calendars)`
        : `Failed: ${d.error}`;
      status.style.color = d.ok ? 'var(--green)' : 'var(--red)';
    } catch (e) {
      status.textContent = 'Error';
...
function initBackup() {
  ...
  el('adm-importDataBtn').addEventListener('click', () => {
    fileInput.value = '';
    fileInput.click();
  });
  fileInput.addEventListener('change', async () => {
    ...
    try {
      ...
      if (res.ok && result.ok) {
        msg.textContent = result.message || 'Import successful.';
        msg.className = 'admin-success';
      } else {
        msg.textContent = result.message || result.detail || 'Import failed';
        msg.className = 'admin-error';
      }
    } catch (e) {
...
function initDangerZone() {
  ...
  modalEl.querySelectorAll('[data-wipe-kind]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      if (
        !(await uiModule.styledConfirm(
          `Really delete every one of your ${label}?`,
          {
            confirmText: isAll
      ...
      try {
        if (isAll) {
          ...
          for (const k of kinds) {
            try {
              const r = await fetch(`/api/admin/wipe/${k}`, {
                method: 'DELETE',
                credentials: 'same-origin',
              });
              const d = await r.json().catch(() => ({}));
              results.push({
                ...
                ok: r.ok,
                count: d.count ?? 0,
                error: r.ok ? null : d.detail || 'failed',
              });
            } catch (e) {
          ...
          const res = await fetch(`/api/admin/wipe/${kind}`, {
            method: 'DELETE',
            credentials: 'same-origin',
          });
          const data = await res.json().catch(() => ({}));
          if (res.ok) {
            if (_wipeMsg) {
              _wipeMsg.textContent = `Deleted ${data.count ?? 0} ${label}.`;
              _wipeMsg.className = 'admin-success';
            }
          } else {
            if (_wipeMsg) {
              _wipeMsg.textContent = data.detail || 'Failed';
              _wipeMsg.className = 'admin-error';
            }
          }
        }
        ...
        if (_wipeMsg) {
          _wipeMsg.textContent = 'Request failed: ' + e.message;
          _wipeMsg.className = 'admin-error';
        }
      }
      btn.disabled = false;
...
function renderLogs(isAutoPoll = false) {
  ...
  if (logs.length === 0) {
    consoleContainer.innerHTML =
      '<div class="settings-system-logs-placeholder">No logs found matching current filters.</div>';
    return;
  }

  // Preserve scroll position if user is reading previous logs
  const atBottom =
    consoleContainer.scrollHeight -
      consoleContainer.scrollTop -
      consoleContainer.clientHeight <
    40;
...
function startLogsPolling() {
  ...
  logsPollInterval = setInterval(() => {
    ...
    const systemPanel = el('settings-modal')?.querySelector(
    ...
    if (
      ...
      !systemPanel ||
      systemPanel.classList.contains('hidden')
    ) {
      stopLogsPolling();
      return;
    }
...
function initLogsView() {
  ...
  if (levelSelect)
    levelSelect.addEventListener('change', () => renderLogs(false));
  if (limitSelect)
    limitSelect.addEventListener('change', () => loadLogs(false));
  if (searchInput)
    searchInput.addEventListener('input', () => renderLogs(false));
...
function initAll() {
  ...
  for (const fn of inits) {
    try {
      ...
      console.error('Admin init error in', fn.name || 'anonymous', e);
    }
  }
  initialized = true;
  refreshAll();
