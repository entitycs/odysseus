  ...
  try {
    ...
    if (_docModule) {
      ...
      if (existingDocId) {
        ...
        if (
          aiSuggestedBody &&
          typeof _docModule.replaceEmailReplyBody === 'function'
        ) {
          await _docModule.replaceEmailReplyBody(
            existingDocId,
            aiSuggestedBody,
            { force: true },
          );
...
async function _createEmailChat(emailData) {
  const subject =
    String(emailData?.subject || 'New Email').trim() || 'New Email';
  const title =
    subject === 'New Email' ? 'New Email' : `Email: ${subject.slice(0, 60)}`;
  try {
    const currentSid = sessionModule.getCurrentSessionId?.() || '';
    const current = sessionModule
      .getSessions?.()
      .find((s) => s.id === currentSid);
