  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      if (
        ...
        const name =
          (chip.dataset && chip.dataset.name) ||
...
async function _prewarmDefaultEmailView() {
  ...
  const res = await fetch(
    `${API_BASE}/api/email/list?folder=${encodeURIComponent(folder)}${accountQS}&limit=500&offset=0&filter=${filter}`,
    {
      credentials: 'same-origin',
    },
  );
...
async function _loadFolders({ resetMissing = false } = {}) {
  ...
  try {
    const res = await fetch(
      `${API_BASE}/api/email/folders?_=${Date.now()}${_acct()}`,
    );
...
const _LIB_FILTER_OPTIONS = [
  ...
  {
    value: 'filter:tag:newsletter',
    label: 'Newsletter',
    keywords: ['newsletter', 'newsletters', 'subscriptions'],
  },
  {
    value: 'filter:tag:marketing',
    label: 'Marketing',
    keywords: ['marketing', 'promo', 'promotional'],
  },
];
...
function _applyPillFilter() {
  ...
  const serverHandledDraft = !!(
    libSearchLower &&
    draft &&
    libSearchLower === draft.toLowerCase()
  );
  const draftPill =
  ...
  const effectiveBasePills = libSearchLower
    ? pills.filter(
        (p) =>
          !(
            p.type === 'text' && (p.text || '').toLowerCase() === libSearchLower
          ),
      )
    : pills;
...
async function _initEmailSearchChipBar() {
  ...
  input.addEventListener('input', async () => {
    ...
    try {
      console.log(
        '[email-search] input event, value=',
    ...
    if (v.length >= 2) {
      _libSearchTypingTimer = setTimeout(() => {
        ...
        if (cur === v && cur.length >= 2) {
          ...
          try {
            console.log('[email-search] firing _doSearch for', cur);
          } catch {}
          _doSearch();
        } else {
          try {
            console.log(
              '[email-search] debounce expired but input changed (was',
              v,
              'now',
              cur,
              ')',
...
const _EMAIL_FILTER_ICONS = {
  ...
  'tag:marketing':
    '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 11l18-5v12L3 14v-3z"/><path d="M11.6 16.8a3 3 0 1 1-5.8-1.6"/></svg>',
};
...
async function _refreshUnreadBadge() {
  ...
  try {
    ...
    const res = await fetch(
      `${API_BASE}/api/email/list?folder=${encodeURIComponent(folder)}${_acct()}&limit=1&filter=unread`,
    );
    const data = await res.json();
...
function _renderGrid() {
  ...
  if (state._libSort === 'unread') {
    ...
    filtered = [...filtered].sort(
      (a, b) => Number(b.is_flagged) - Number(a.is_flagged),
    );
  }
  ...
  if (filtered.length === 0) {
    ...
    if (_libSearchInFlight) {
      ...
      wrap.style.cssText =
        'display:flex;align-items:center;justify-content:center;gap:8px;flex-wrap:wrap;padding:24px;opacity:0.75;';
      grid.appendChild(wrap);
    ...
    tagWrap.innerHTML = tags
      .map((t) => {
        const tag = String(t || '')
          ...
          .replace(/_/g, '-');
        return tag
          ? `<span class="email-tag email-tag-${_esc(tag)}">${_esc(tag)}</span>`
          : '';
      })
      .join('');
...
async function _toggleCardPreview(card, em) {
  ...
  loadingWrap.style.cssText =
    'padding:20px;display:flex;justify-content:center;align-items:center;flex:1;';
...
async function _openEmailWindow(em, folder) {
  ...
  try {
    ...
    try {
      _wireAttachmentHandlers(bodyEl, useFolder);
    } catch {}
