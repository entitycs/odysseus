...
async function _doSearch() {
  ...
  try {
    ...
    const res = await fetch(
      `${API_BASE}/api/email/search?folder=${encodeURIComponent(folderAtStart)}${accountQS}&q=${encodeURIComponent(q)}&limit=100`,
    );
    const data = await res.json();
    if (
      seq !== _libSearchSeq ||
    ...
    if (stats)
      stats.textContent = `${count} match${count === 1 ? '' : 'es'} on server`;
    try {
      console.log(
        '[email-search]',
        JSON.stringify({
