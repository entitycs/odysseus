...
function _showNotesFirstOpenHint(pane) {
  ...
  const place = () => {
    ...
    hint.style.left =
      Math.min(window.innerWidth - hw - 12, Math.max(12, r.left + 18)) + 'px';
  };
  const close = () => {
...
function _wireNotesWindow(pane) {
  ...
  makeWindowDraggable(pane, {
    ...
    skipSelector:
      'button, input, select, textarea, label, .notes-mobile-grabber',
    enableDock: true,
    enableLeftDock: true,
    onEnterFullscreen: () => {
...
function _bringNotesToFront(pane = document.getElementById('notes-pane')) {
  if (!pane) return;
  const backdrop =
    document.getElementById('notes-pane-backdrop') || pane.parentElement;
...
function _loadPendingHighlights() {
  try {
    return new Set(
      JSON.parse(localStorage.getItem(REMINDER_PENDING_HIGHLIGHT_KEY) || '[]'),
    );
  } catch {
    return new Set();
  }
...
function _loadGlowedReminders() {
  try {
    return new Set(
      JSON.parse(localStorage.getItem(REMINDER_GLOWED_KEY) || '[]'),
    );
  } catch {
    return new Set();
  }
...
function _saveGlowedReminders(set) {
  try {
    localStorage.setItem(REMINDER_GLOWED_KEY, JSON.stringify([...set]));
  } catch {}
}
function _loadActiveHighlights() {
  try {
    return new Set(
      JSON.parse(localStorage.getItem(REMINDER_ACTIVE_HIGHLIGHT_KEY) || '[]'),
    );
  } catch {
    return new Set();
  }
...
function _clearViewedReminderGlows() {
  ...
  document
    .querySelectorAll('.note-card-reminder-fired-sticky')
    .forEach((card) => {
...
function _setReminderCardGlow(noteId, on = true) {
  ...
  document
    .querySelectorAll(`.note-card[data-note-id="${noteId}"]`)
    .forEach((card) => {
...
function _savePendingHighlights(set) {
  try {
    localStorage.setItem(
      REMINDER_PENDING_HIGHLIGHT_KEY,
      JSON.stringify([...set]),
    );
  } catch {}
}
...
const _CUSTOM_GRADIENT =
  'conic-gradient(from 0deg, #e06c75, #d19a66, #e5c07b, #98c379, #61afef, #c678dd, #e06c75)';
...
function _isBgImage(c) {
  return typeof c === 'string' && c.startsWith('bg:');
}
function _bgImageUrl(c) {
  return _isBgImage(c) ? c.slice(3) : '';
}
...
function _linkify(s) {
  ...
  return escaped.replace(urlRe, (m) => {
    ...
    return (
      `<a href="${_attrEsc(href)}" class="note-link" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()">${url}</a>` +
      (url !== m ? m.slice(url.length) : '')
    );
  });
}
function _uid() {
  ...
  el.addEventListener(
    ...
    (e) => {
      ...
      pane.style.transform =
        dy > 0 ? `translateY(${dy}px)` : `translateY(${dy * RUBBER}px)`;
    },
    { passive: false },
  );
...
function _snapToRepeat(currentDate, normRepeat) {
  ...
  if (kind === 'monthly') {
    ...
    for (let tries = 0; tries < 14; tries++) {
      ...
      if (target.getTime() > now && target.getTime() >= anchor.getTime())
        return target;
      m++;
      if (m > 11) {
        m = 0;
...
function _loadFiredReminders() {
  try {
    return new Set(
      JSON.parse(localStorage.getItem(REMINDER_FIRED_KEY) || '[]'),
    );
  } catch {
    return new Set();
  }
...
function _saveFiredReminders(set) {
  try {
    localStorage.setItem(REMINDER_FIRED_KEY, JSON.stringify([...set]));
  } catch {}
}
...
function _countDueReminders() {
  return _notes.filter(
    (n) =>
      !n.archived && _isDueTodayOrOverdue(n.due_date) && !_isNoteFullyDone(n),
  ).length;
}
...
let _firedDotDismissedAt = (() => {
  try {
    const v = parseInt(
      localStorage.getItem(REMINDER_DISMISSED_AT_KEY) || '0',
      10,
    );
...
export function dismissFiredReminderDot() {
  ...
  try {
    localStorage.setItem(
      REMINDER_DISMISSED_AT_KEY,
      String(_firedDotDismissedAt),
    );
  } catch {}
  _updateRailBadge();
}
...
export function openPanel() {
  ...
  if (window.innerWidth <= 768) {
    ...
    pane.style.animation =
      'sheet-enter 0.25s cubic-bezier(0.2, 0.8, 0.2, 1) both';
  ...
  if (archiveBtn) {
    const ARCHIVE_ICON =
      '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="20" height="5" rx="1"/><path d="M4 8v11a2 2 0 002 2h12a2 2 0 002-2V8"/><path d="M10 12h4"/></svg><span class="notes-header-btn-label">Archive</span>';
    const CLOSE_ICON =
      '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><line x1="6" y1="6" x2="18" y2="18"/><line x1="18" y1="6" x2="6" y2="18"/></svg><span class="notes-header-btn-label">Archive</span>';
  ...
  if (viewBtn) {
    ...
    _setViewLabel();
    requestAnimationFrame(() =>
      _applyMasonry(document.querySelector('#notes-pane .notes-pane-body')),
    );
    viewBtn.addEventListener('click', () => {
      _viewMode = _viewMode === 'grid' ? 'list' : 'grid';
      try {
        localStorage.setItem('odysseus-notes-view', _viewMode);
      } catch {}
      pane.classList.toggle('notes-view-grid', _viewMode === 'grid');
      _setViewLabel();
      requestAnimationFrame(() =>
  ...
  document
    .getElementById('notes-select-all')
  ...
  document
    .getElementById('notes-bulk-archive')
    .addEventListener('click', async () => {
      const ids = [..._selectedIds];
      if (!ids.length) return;
      await Promise.all(
        ids.map((id) => _patchNote(id, { archived: true }).catch(() => {})),
      );
      _exitSelectMode();
      await _fetchNotes();
      _renderNotes();
  ...
  document
    .getElementById('notes-bulk-delete')
    .addEventListener('click', async () => {
      const ids = [..._selectedIds];
      if (!ids.length) return;
...
function _updateBulkBar() {
  ...
  if (allEl)
    allEl.checked =
      _notes.length > 0 && _notes.every((n) => _selectedIds.has(n.id));
...
function _noteTags(n) {
  ...
  return [
    ...new Set(tags.map((t) => t.replace(/^#+/, '').trim()).filter(Boolean)),
  ];
}
...
async function _clearPastReminders() {
  ...
  const ok = uiModule?.styledConfirm
    ? await uiModule.styledConfirm(
        `Delete ${targets.length} past reminder${targets.length === 1 ? '' : 's'}?`,
        { confirmText: 'Delete', danger: true },
      )
    : confirm(
        `Delete ${targets.length} past reminder${targets.length === 1 ? '' : 's'}?`,
  ...
  _renderNotes();
  uiModule.showToast?.(
    `Cleared ${targets.length} past reminder${targets.length === 1 ? '' : 's'}`,
  );
}

function _renderLabels(root = document) {
  const bar =
    root.querySelector?.('.notes-labels-bar') ||
    document.querySelector('.notes-labels-bar');
  if (!bar) return;
  ...
  const reminderCount = _notes.filter(
    (n) => !n.archived && n.due_date && _hasTimeComponent(n.due_date),
  ).length;
  const pastReminderCount = _notes.filter(
    (n) => !n.archived && _isPastReminder(n),
  ).length;
  const defaultCount = _notes.filter(
    (n) => !n.archived && _visibleNoteTags(n).length === 0,
  ).length;
  // Active goals = non-archived goal notes. Today view lists pending steps
  // from each, so we surface the count next to the chip.
  const goalCount = _notes.filter(
    (n) => n.note_type === 'goal' && !n.archived,
  ).length;
  const todayCount = _notes.filter(
    (n) => n.note_type === 'goal' && !n.archived && _nextGoalStep(n),
  ).length;
  bar.style.display = '';
  ...
  if (todayCount > 0) {
    ...
    const icon =
      '<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:2px"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>';
  ...
  if (goalCount > 0) {
    ...
    const icon =
      '<svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor" style="vertical-align:-1px;margin-right:2px"><path d="M12 0L14.59 8.41L23 12L14.59 15.59L12 24L9.41 15.59L1 12L9.41 8.41Z"/></svg>';
    ...
    chip.addEventListener('click', () => {
      if (chip.dataset.action === 'all') {
        ...
        if (_activeFilter === null) {
          ...
          _reminderChipNext =
            _reminderChipNext === 'reminders' ? 'no-reminders' : 'reminders';
        } else {
          _activeFilter = null;
        }
...
function _renderNotes() {
  ...
  let filtered = _activeLabel
    ? _notes.filter((n) => _noteTags(n).includes(_activeLabel))
    : _notes;
  if (_activeFilter === 'reminders') {
    filtered = filtered.filter(
      (n) => n.due_date && _hasTimeComponent(n.due_date),
    );
  ...
  if (_activeFilter === 'today') {
    ...
    if (sorted.length === 0) {
      body.insertAdjacentHTML(
        'beforeend',
        `<div class="notes-empty">All caught up — no pending goal steps right now.</div>`,
      );
    } else {
  ...
  for (const note of sorted) {
    ...
    if (_hasItems(note) && Array.isArray(note.items)) {
      ...
      for (let i = 0; i < note.items.length; i++) {
        ...
        const agentDoneClass =
          agentStatus === 'stream_complete' ? ' is-agent-stream-complete' : '';
        const agentTitle =
          agentStatus === 'stream_complete'
            ? 'Agent stream finished for this todo'
            : agentStatus === 'running'
              ? 'Agent is working on this todo'
              : 'Solve this todo with the agent';
        const agentSessionAttr = item.agent_session_id
          ? ` data-session-id="${_attrEsc(item.agent_session_id)}"`
          : '';
        const agentMenuTitle =
          item.agent_session_title ||
          `Agent: ${(item.text || '').slice(0, 40)}`;
      ...
      const fullText = note.content || '';
      const preview =
        fullText.length > 600 ? fullText.slice(0, 600) + '…' : fullText;
      ...
      contentHtml = preview
        ? `<div class="note-content-preview">${_linkify(preview)}</div>`
        : '';
    }
    ...
    const reminderTagHtml =
      note.due_date && _hasTimeComponent(note.due_date)
    ...
    const dueBadge =
      dueFmt && !_hasTimeComponent(note.due_date)
        ? `<span class="note-due-inline${overdue ? ' note-due-overdue' : ''}">${dueFmt}</span>`
        : '';
    const colorDots = COLORS.map(
      (c) =>
        `<span class="note-card-color-dot${_dotIsActive(c.value, note.color) ? ' active' : ''}" data-color="${c.value}" style="background:${_dotBg(c.value, note.color)}" title="${c.name || 'default'}"></span>`,
    ).join('');
    const goalClass = note.note_type === 'goal' ? ' note-card-goal' : '';
    const reminderGlowClass =
      activeReminderHighlights.has(note.id) && _hasActiveReminder(note)
        ? ' note-card-reminder-fired-sticky'
        : '';
    const goalPill =
      note.note_type === 'goal'
    ...
    html += `<div class="note-card${note.pinned ? ' note-card-pinned' : ''}${cc}${sel}${goalClass}${reminderGlowClass}${_selectMode ? ' note-card-selectmode' : ''}" draggable="${_selectMode || _isNotesMobileMode() ? 'false' : 'true'}" data-note-id="${note.id}"${cardStyle}>
      ...
      ${
        _showingArchived
          ...
          ${
            _hasItems(note)
              ? `<button class="note-card-copy note-card-copy-corner" data-note-id="${note.id}" title="Copy all items" aria-label="Copy all items">
  ...
  if (existingForm && _editingId === '__new__') {
    ...
    if (sorted.length === 0) {
      body.insertAdjacentHTML(
        'beforeend',
        '<div class="notes-empty-msg">No notes <span style="vertical-align:-3px;margin-left:4px;">' +
          uiModule.emptyStateIcon('smiley') +
          '</span></div>',
      );
    } else {
    ...
    if (sorted.length === 0) {
      body.insertAdjacentHTML(
        'beforeend',
        '<div class="notes-empty-msg">No notes yet <span style="vertical-align:-3px;margin-left:4px;">' +
          uiModule.emptyStateIcon('smiley') +
          '</span></div>',
      );
    } else {
...
function _applyMasonry(body) {
  ...
  const isMobileGrid =
    isGrid && window.matchMedia('(max-width: 768px)').matches;
  // Tear down any prior observer (defensive — _renderNotes wipes body.innerHTML).
  if (_masonryObserver) {
    try {
      _masonryObserver.disconnect();
    } catch {}
  ...
  if (!isGrid) {
    ...
    body
      .querySelectorAll(
        '.note-card, .notes-labels-bar, .notes-quick-add, .note-form',
      )
      .forEach((c) => {
        c.style.gridRowEnd = '';
      });
  ...
  const recomputeFullRows = () => {
    ...
    body.querySelectorAll('.note-form').forEach((form) => {
      ...
      const measuredHeight =
        Math.max(form.scrollHeight, renderedHeight) + drawReserve;
  ...
  if ('ResizeObserver' in window) {
    ...
    body
      ...
      .forEach((c) => _masonryObserver.observe(c));
    body
      .querySelectorAll('.notes-labels-bar, .notes-quick-add, .note-form')
      .forEach((c) => _masonryObserver.observe(c));
  }
}
...
function _renderQuickAdd(body) {
  ...
  const expandToForm = (initialType = 'note', initialText = '') => {
    ...
    const mobileGrid =
      body.closest('.notes-pane')?.classList.contains('notes-view-grid') &&
  ...
  wrap.querySelector('[data-action="photo"]').addEventListener('click', (e) => {
    ...
    setTimeout(
      () => document.querySelector('.note-form-photo-btn')?.click(),
      50,
    );
  });
}
...
function _bindCardEvents(body) {
  ...
  if (_isNotesMobileMode()) {
    body
      .querySelectorAll('.note-card')
      .forEach((card) => _bindLongPressDrag(card));
  }
  body
    .querySelectorAll('.note-card.note-card-reminder-fired-sticky')
    .forEach((card) => {
      card.addEventListener(
        'click',
        () => _setReminderCardGlow(card.dataset.noteId, false),
        true,
      );
    });
  // Click title — edit, or toggle select in select mode
  body
    .querySelectorAll('.note-card-title[data-action="edit"]')
    .forEach((el) => {
  ...
  body.querySelectorAll('.note-checklist-preview').forEach((el) => {
    el.addEventListener('click', (e) => {
      if (
        e.target.closest(
          '.note-checkbox, .note-checkbox-rm, .note-checkbox-agent, .note-cl-quickadd, input',
        )
      )
        return;
      e.stopPropagation();
  ...
  if (_isNotesMobileMode() && !_selectMode) {
    const _INTERACTIVE =
      'button, a, input, label, .note-card-color-dot, .note-checkbox, .note-checkbox-rm, .note-checkbox-agent, .note-cl-quickadd, .note-agent-tag, .note-card-pin, .note-card-corner-trash, .note-card-corner-menu, .note-card-corner-unarchive, .note-card-edit-corner, .note-card-reminder, .note-card-cb';
    body.querySelectorAll('.note-card').forEach((card) => {
      card.addEventListener('click', (e) => {
  ...
  body.querySelectorAll('.note-card-cb').forEach((cb) => {
    ...
    cb.addEventListener('change', () => {
      ...
      cb.closest('.note-card').classList.toggle(
        'note-card-selected',
        cb.checked,
      );
      _updateBulkBar();
  ...
  const _applyCardColor = async (card, id, newColor) => {
    ...
    COLORS.forEach((c) => {
      if (c.value && c.value !== 'custom')
        card.classList.remove('note-color-' + c.value);
    });
    ...
    try {
      await _patchNote(id, { color: newColor || null });
      const note = _notes.find((n) => n.id === id);
      if (note) note.color = newColor;
    } catch {
      uiModule.showError('Failed to update color');
    }
  };
  body.querySelectorAll('.note-card-color-dot').forEach((dot) => {
    dot.addEventListener('click', (e) => {
      ...
      if (dot.dataset.color === 'custom') {
        _pickCustomBgImage().then((url) => {
          if (url) _applyCardColor(card, id, 'bg:' + url);
        });
        return;
      }
  ...
  // handler off it.
  body
    .querySelectorAll('.note-card-edit-corner:not(.note-card-unarchive-corner)')
    .forEach((btn) => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const id = btn.dataset.noteId;
  ...
  body.querySelectorAll('.note-card-done').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      ...
      const _undoIcon =
        '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;"><polyline points="9 14 4 9 9 4"/><path d="M4 9h11a5 5 0 0 1 5 5v0a5 5 0 0 1-5 5H9"/></svg>';
      const finish = () => {
        _renderNotes();
        _patchNote(id, { archived: true })
          .then(() => {
            uiModule.showToast('Archived', {
              duration: 6000,
              action: 'Undo',
  ...
  body.querySelectorAll('.note-card-corner-unarchive').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      ...
      const removed = _notes.splice(idx, 1)[0];
      _renderNotes();
      _patchNote(id, { archived: false })
        .then(() => uiModule.showToast('Unarchived'))
        .catch(() => {
          _notes.splice(idx, 0, removed);
          _renderNotes();
  ...
  body.querySelectorAll('.note-card-corner-trash').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      ...
      _renderNotes();
      _deleteNoteApi(id)
        .then(() => uiModule.showToast('Deleted'))
        .catch(() => {
          _notes.splice(idx, 0, removed);
          _renderNotes();
  ...
  body.querySelectorAll('.note-card-copy').forEach((btn) => {
    btn.addEventListener('click', async (e) => {
      ...
      try {
        ...
        uiModule.showToast?.(
          `Copied ${(note.items || []).filter((i) => (i?.text || '').trim()).length} items`,
        );
      } catch {
    ...
    input.addEventListener('keydown', async (e) => {
      ...
      setTimeout(() => {
        const next = document.querySelector(
          `.note-cl-quickadd-input[data-note-id="${noteId}"]`,
        );
        if (next) next.focus();
      }, 0);
  ...
  body.querySelectorAll('.note-checkbox').forEach((el) => {
    el.addEventListener('click', (e) => {
      ...
      const wasAllDone =
        note.items.length > 0 && note.items.every((it) => it.done);
      note.items[idx].done = !note.items[idx].done;
  ...
  if (!_isNotesMobileMode()) {
    body.querySelectorAll('.note-card').forEach((card) => {
      card.addEventListener('dragstart', (e) => {
        if (
          e.target.closest(
            '.note-checkbox, .note-card-x, .note-card-select, .note-card-pin, .note-card-action, .note-card-color-dot, .note-card-title, .note-card-edit, .note-card-edit-corner, .note-card-done, .note-card-corner-menu, .note-agent-tag, .note-card-label-chip',
          )
        ) {
          e.preventDefault();
          return;
      ...
      card.addEventListener('dragend', async () => {
        ...
        const ids = [...body.querySelectorAll('.note-card')].map(
          (c) => c.dataset.noteId,
        );
        try {
  ...
  function _maybeSwap(dragging, clientX, clientY) {
    ...
    const cards = [...body.querySelectorAll('.note-card')].filter(
      (c) => c !== dragging,
    );
    const prevRects = new Map(cards.map((c) => [c, c.getBoundingClientRect()]));
    const draggingNext =
      dragging.nextSibling === target ? dragging : dragging.nextSibling;
  ...
  if (
    ...
    const _selectorSkip =
      '.note-checkbox, .note-card-x, .note-card-select, .note-card-pin, .note-card-action, .note-card-color-dot, .note-card-title, .note-card-edit, .note-card-edit-corner, .note-card-done, .note-card-corner-menu, .note-agent-tag, .note-card-label-chip, input, textarea, button, a';
    ...
    const _endDrag = (committed) => {
      ...
      if (isDragging) {
        ...
        if (committed) {
          ...
          fetch(`${API_BASE}/api/notes/reorder`, {
            method: 'POST',
            credentials: 'same-origin',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ids }),
    ...
    body.addEventListener(
      ...
      (e) => {
        ...
        if (!isDragging) {
          ...
          if (
            Math.abs(t.clientX - startX) > MOVE_THRESHOLD_PX ||
            Math.abs(t.clientY - startY) > MOVE_THRESHOLD_PX
          ) {
            clearTimeout(longPressTimer);
            longPressTimer = null;
            dragCard = null;
        ...
        const before =
          dragCard.parentNode &&
          [...dragCard.parentNode.children].indexOf(dragCard);
...
function _draftKey(id) {
  return _DRAFT_PREFIX + (id || '__new__');
}
function _loadDraft(id) {
  try {
    return JSON.parse(localStorage.getItem(_draftKey(id)) || 'null');
  } catch {
    return null;
  }
}
...
function _collectFormDraft(form) {
  if (!form) return null;
  const type =
    form.querySelector('.note-form-type-pill.active')?.dataset.type || 'note';
  const d = {
    _ts: Date.now(),
    note_type: type,
  ...
  if (type === 'note')
    d.content = form.querySelector('.note-form-content')?.value || '';
  else if (type === 'goal') {
    d.content = form.querySelector('.note-form-goal-desc')?.value || '';
    d.items = _collectItems(form);
...
function _isDraftEmpty(d) {
  ...
  if (Array.isArray(d.items) && d.items.some((it) => (it.text || '').trim()))
    return false;
  return true;
}
function _wireDraftAutosave(form, id) {
  ...
  const save = () => {
    ...
    try {
      localStorage.setItem(_draftKey(id), JSON.stringify(d));
    } catch {}
  };
  form._flushDraft = () => {
    clearTimeout(t);
...
function _buildForm(note = null) {
  ...
  let _stashedTodoItems =
    type === 'todo' && Array.isArray(note?.items) ? note.items.slice() : null;
  ...
  let _stashedGoalItems =
    type === 'goal' && Array.isArray(note?.items) ? note.items.slice() : null;

  // Drawing also stashes the saved image URL so it survives Note↔Draw flips.
  let _stashedDrawUrl =
    type === 'draw' ? safeDisplayImageSrc(note?.image_url) || null : null;
    ...
    pill.addEventListener('click', () => {
      ...
      if (newType === 'todo') {
        ...
        if (_stashedTodoItems && _stashedTodoItems.length) {
          ...
          const lines = _stashedNoteText
            .split('\n')
            .map((s) => s.trim())
            .filter(Boolean);
          nextItems = lines.length
            ? lines.map((t) => ({ id: _uid(), text: t, done: false }))
        ...
        _wireCanvas(
          ...
          _stashedDrawUrl ||
            currentImageUrl ||
            safeDisplayImageSrc(note?.image_url) ||
            null,
        );
      } else {
        const text =
          _stashedNoteText !== null &&
          _stashedNoteText !== undefined &&
          _stashedNoteText !== ''
            ? _stashedNoteText
            : (_stashedGoalDesc && _stashedGoalDesc) ||
              (_stashedTodoItems || _stashedGoalItems || [])
                .map((i) => i.text)
                .join('\n');
      ...
      form
        .querySelectorAll('.note-form-type-pill')
        .forEach((p) =>
      ...
      if (
        form.closest('.notes-pane.notes-view-grid') &&
        window.matchMedia('(max-width: 768px)').matches
      ) {
        form.style.gridColumn = '1 / -1';
  ...
  if (_typeSeg) {
    ...
    const _activateAt = (x, y) => {
      const pill = document
        ...
        ?.closest?.('.note-form-type-pill');
      if (pill && !pill.classList.contains('active')) pill.click();
    };
    _typeSeg.addEventListener(
      'touchstart',
      () => {
        _sliding = true;
      },
    ...
    _typeSeg.addEventListener(
      'touchmove',
      (e) => {
        if (_sliding && e.touches[0])
          _activateAt(e.touches[0].clientX, e.touches[0].clientY);
      },
      { passive: true },
    );
    _typeSeg.addEventListener('touchend', () => {
      _sliding = false;
    });
    _typeSeg.addEventListener('touchcancel', () => {
      _sliding = false;
    });
  }
  ...
  const _applyFormColor = (newColor) => {
    ...
    COLORS.forEach((c) => {
      if (c.value && c.value !== 'custom')
        form.classList.remove('note-color-' + c.value);
    });
  ...
  if (currentType === 'todo')
    _wireChecklist(form.querySelector('.note-form-body'));
  if (currentType === 'goal')
    _wireGoalForm(form, form.querySelector('.note-form-body'));
  if (currentType === 'draw') {
    _wireCanvas(
      form.querySelector('.note-form-body'),
      safeDisplayImageSrc(note?.image_url) || null,
    );
    // Same hides we apply on type-switch — keep them consistent on initial open.
    const _ip = form.querySelector('.note-form-image-wrap');
    if (_ip) _ip.style.display = 'none';
    const _cp = form.querySelector('.note-color-picker');
  ...
  function _renderReminderTag() {
    ...
    const repLabel =
      rep !== 'none' ? ` · ${_formatRepeatLabel(rep, new Date(v))}` : '';
    ...
    tagsEl
      .querySelector('.note-reminder-tag')
  ...
  function _openReminderMenu(anchor, isEdit = false) {
    ...
    const presetItems = [
      {
        label: 'Later today',
        sub: _laterTodayDate().toLocaleTimeString([], {
          hour: 'numeric',
          minute: '2-digit',
        }),
        action: () => _setReminder(_toLocalDatetimeStr(_laterTodayDate())),
      },
      {
        label: 'Tomorrow',
        sub: _tomorrowDate().toLocaleTimeString([], {
          hour: 'numeric',
          minute: '2-digit',
        }),
        action: () => _setReminder(_toLocalDatetimeStr(_tomorrowDate())),
      },
      {
        label: 'Next week',
        sub:
          _nextWeekDate().toLocaleDateString([], { weekday: 'short' }) +
          ' ' +
          _nextWeekDate().toLocaleTimeString([], {
            hour: 'numeric',
            minute: '2-digit',
          }),
        action: () => _setReminder(_toLocalDatetimeStr(_nextWeekDate())),
      },
      {
      ...
      if (subMode === null) {
        ...
        if (isEdit && dueInput.value) {
          ...
          // Weekly →
          {
            ...
            const sub =
              isW && !isNaN(wd)
                ? `<span class="note-reminder-menu-sub">${_DAYS[wd]}</span>`
                : '';
          ...
          // Monthly →
          {
            ...
            const sub = isM
              ? `<span class="note-reminder-menu-sub">${_monthlyShortDescriptor(norm)}</span>`
              : '';
        ...
        const norm = getNorm();
        const curWd = norm.startsWith('weekly:')
          ? parseInt(norm.split(':')[1], 10)
          : -1;
        ...
        html +=
          '<div class="note-reminder-menu-title">Nth weekday of month</div>';
        ...
        const lbl = ready
          ? `Save: ${_ORDINALS[nthDraft.n - 1]} ${_DAYS[nthDraft.w]}`
          : 'Pick week and weekday';
        ...
        el.addEventListener('click', (e) => {
          ...
          if (a === 'preset') {
            ...
            if (
              ...
              if (m)
                nthDraft = { n: parseInt(m[1], 10), w: parseInt(m[2], 10) };
            }
            render();
          } else if (a === 'back') {
            subMode = null;
  ...
  function _pickCustomDate() {
    ...
    menu
      .querySelector('.note-reminder-menu-confirm')
  ...
  if (remindBtn)
    remindBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      _openReminderMenu(remindBtn, !!dueInput.value);
    });
  _renderReminderTag();
  ...
  if (photoBtn && photoInput) {
    ...
    photoInput.addEventListener('change', async () => {
      ...
      try {
        ...
        wrap
          .querySelector('.note-form-image-rm')
          .addEventListener('click', () => {
            wrap.remove();
            currentImageUrl = '';
          });
  ...
  _saveBtnEl0.addEventListener('click', async () => {
    ...
    try {
      ...
      if (
        form.querySelector('.note-form-due').value &&
        !_tags.includes('reminder')
      )
        _tags.push('reminder');
      ...
      if (currentType === 'note') {
        ...
        if (!url) {
          uiModule.showError('Failed to save drawing');
          return;
        }
  ...
  // Cancel
  form.querySelector('.note-form-cancel').addEventListener('click', () => {
  ...
  form
    .querySelector('.note-form-archive-btn')
    ?.addEventListener('click', () => {
      if (!isEdit) return;
      const id = note.id;
      const idx = _notes.findIndex((n) => n.id === id);
  ...
  form
    .querySelector('.note-form-delete-btn')
    ?.addEventListener('click', async () => {
      if (!isEdit) return;
      const id = note.id;
...
function _wireRow(row, container) {
  ...
  row.querySelector('.note-cl-dot')?.addEventListener('click', () => {
    ...
    const hasText = rows.some(
      (r) => (r.querySelector('.note-cl-text')?.value || '').trim().length > 0,
    );
    if (
      hasText &&
      rows.every(
        (r) =>
          ...
          !(r.querySelector('.note-cl-text')?.value || '').trim(),
      )
    ) {
      spawnConfetti(
        dRect.left + dRect.width / 2,
  ...
  txt?.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      container.querySelector('.note-cl-add')?.click();
    } else if (e.key === 'Tab') {
      e.preventDefault();
      ...
      row.remove();
      if (prev && prev.classList.contains('note-cl-row'))
        prev.querySelector('.note-cl-text')?.focus();
    }
  });
  // Drag handlers
...
function _wireChecklist(container) {
  ...
  container
    .querySelectorAll('.note-cl-row')
    .forEach((row) => _wireRow(row, container));
...
function _wireCanvas(container, initialImageUrl) {
  ...
  const _begin = (e) => {
    ...
    if (mode.startsWith('line-') || mode.startsWith('circle-')) {
      ...
      try {
        _shapeSnapshot = ctx.getImageData(0, 0, canvas.width, canvas.height);
      } catch {}
      return;
    }
    ctx.beginPath();
  ...
  canvas.addEventListener(
    'touchstart',
    (e) => {
      if (mode.startsWith('text-')) e.preventDefault();
      _begin(e);
    },
    { passive: false },
  );
  ...
  const _setMode = (next) => {
    ...
    if (cBadge)
      cBadge.textContent = isCircle ? next.slice(-1).toUpperCase() : '';
    ...
    [textBtn, lineBtn, circleBtn].forEach((b) =>
      b?.classList.remove('size-s', 'size-m', 'size-l'),
    );
...
function _createNote(type = 'todo') {
  ...
  const { note: _n, restored } = _applyDraftToNote(
    { note_type: type },
    '__new__',
  );
...
function _openNoteCornerMenu(btn) {
  document
    .querySelectorAll('.note-corner-menu-dropdown')
  ...
  menu.querySelector('[data-act="agent"]').addEventListener('click', () => {
    close();
    _agentSolveNote(id);
  });
}
...
function _openTodoAgentMenu(btn) {
  ...
  menu.innerHTML = `
    ${
      sid
        ? `<button type="button" class="ncm-item" data-act="open">
  ...
  if (openBtn) {
    openBtn.addEventListener('click', () => {
      ...
      if (sid && _sm && _sm.selectSession) {
        closePanel();
        _sm.selectSession(sid);
      }
    });
  }
...
async function _agentSolveNote(id) {
  ...
  if (!prompt) {
    uiModule.showToast('Nothing to solve — note is empty');
    return;
  }
  try {
    const dc = await (
      await fetch(`${API_BASE}/api/default-chat`, {
        credentials: 'same-origin',
      })
    ).json();
    if (!dc.endpoint_url || !dc.model) {
      uiModule.showError('No default chat model configured');
      return;
    }
    ...
    const label = (
      note.title ||
      (Array.isArray(note.items) && note.items[0]?.text) ||
      'todo'
    ).slice(0, 40);
    ...
    if (!csRes.ok) {
      uiModule.showError('Could not create agent session');
      return;
    }
    ...
    fetch(`${API_BASE}/api/chat_stream`, {
      method: 'POST',
      credentials: 'same-origin',
      body: fd,
    })
      .then(async (res) => {
        ...
        if (window.sessionModule && window.sessionModule.markStreamComplete) {
          try {
            window.sessionModule.markStreamComplete(sid);
          } catch {}
        }
      })
      .catch(() => {});
...
async function _agentSolveTodoItem(noteId, idx) {
  ...
  try {
    ...
    _renderNotes();
    _patchNote(noteId, {
      items: n && Array.isArray(n.items) ? n.items : note.items,
    ...
    fetch(`${API_BASE}/api/chat_stream`, {
      ...
      .then(async (res) => {
        ...
        if (doneNote && Array.isArray(doneNote.items) && doneNote.items[idx]) {
          ...
          doneNote.items[idx].agent_stream_completed_at =
            new Date().toISOString();
          _renderNotes();
          _patchNote(noteId, {
            items: doneNote.items,
            agent_session_id: sid,
          }).catch(() => {});
        }
      })
...
async function _deleteNote(id) {
  const ok = uiModule?.styledConfirm
    ? await uiModule.styledConfirm('Delete this note?', {
        confirmText: 'Delete',
        danger: true,
      })
...
function _openMobileFullscreenEdit(id, fromCard) {
  ...
  if (fromCard) {
    ...
    overlay.style.transformOrigin = `${((r.left + r.width / 2) / vw) * 100}% ${((r.top + r.height / 2) / vh) * 100}%`;
  }
  ...
  if (addBtn && photoBtn) {
    ...
    addBtn.addEventListener(
      ...
      (e) => {
        ...
        newRow.innerHTML =
          '<span class="note-cl-grip" title="Drag">⋮⋮</span><span class="note-cl-dot"></span><input type="text" class="note-cl-text" placeholder="Item..." /><button type="button" class="note-cl-rm">&times;</button>';
...
function _closeMobileFullscreenEdit(opts = {}) {
  ...
  if (opts.save) {
    const saveBtn = overlay.querySelector(
      '.note-form-save, [data-action="save"]',
    );
    if (saveBtn)
      try {
        saveBtn.click();
...
function _bindLongPressDrag(card) {
  ...
  card.addEventListener(
    ...
    (e) => {
      ...
      const t = e.touches[0];
      if (
        Math.abs(t.clientX - startX) > CANCEL_PX ||
        Math.abs(t.clientY - startY) > CANCEL_PX
      ) {
        armed = false;
        if (pressTimer) {
...
function _onDocTouchMove(e) {
  ...
  const minTop = quickAdd
    ? quickAdd.getBoundingClientRect().bottom + 4
    : grid.getBoundingClientRect().top;
  const maxTop = Math.max(
    minTop,
    window.innerHeight - card.getBoundingClientRect().height - 8,
  );
  const nextTop = Math.max(
    minTop,
    Math.min(maxTop, touch.clientY - _dragState.grabOffsetY),
  );
  card.style.top = nextTop + 'px';

  const hitY = Math.max(
    minTop + 1,
    Math.min(window.innerHeight - 1, touch.clientY),
  );
...
async function _commitNoteReorder() {
  ...
  if (!grid) return;
  const ids = Array.from(grid.querySelectorAll('.note-card'))
    .map((c) => c.dataset.noteId)
    .filter(Boolean);
  if (!ids.length) return;
  try {
    await fetch(`${API_BASE}/api/notes/reorder`, {
  ...
  const findAndFlash = () => {
    const card =
      ...
      document.querySelector(
        `.note-card[data-note-id^="${noteId.slice(0, 8)}"]`,
      );
    if (card) {
      try {
