...
export async function loadEmails(append = false) {
  ...
  try {
    const fromQS = _senderFilter
      ? `&from=${encodeURIComponent(_senderFilter)}`
      : '';
    const res = await fetch(
      `${API_BASE}/api/email/list?folder=${encodeURIComponent(_currentFolder)}&limit=50&offset=${_offset}${fromQS}${_acct()}&_=${Date.now()}`,
    );
...
async function _openEmail(
  ...
  const aiReplyMode =
    mode === 'ai-reply-fast' ? 'fast' : mode === 'ai-reply-full' ? 'full' : '';
  ...
  try {
    ...
    if (!data) {
      const res = await fetch(
        `${API_BASE}/api/email/read/${em.uid}?folder=${encodeURIComponent(_currentFolder)}${_acct()}`,
      );
      data = await res.json();
    }
    if (data.error) {
    ...
    if (subjectPrefix === 'Re: ' && /^re\s*:/i.test(_baseSubject))
      subjectPrefix = '';
    else if (subjectPrefix === 'Fwd: ' && /^fwd?\s*:/i.test(_baseSubject))
      subjectPrefix = '';
    ...
    if (_docModule) {
      ...
      const existingDocId =
        reuseExisting && _docModule.findEmailDocId
      ...
      if (existingDocId) {
        ...
        let activeSid = '';
        try {
          activeSid = sessionModule?.getCurrentSessionId?.() || '';
        } catch {}
        if (!activeSid) {
        ...
        if (!activeSid) {
          try {
            ...
            const _sres = await fetch(`${API_BASE}/api/session`, {
              method: 'POST',
              body: _fd,
              credentials: 'same-origin',
            });
            if (_sres.ok) {
              ...
              if (_sdata && _sdata.id) {
                activeSid = _sdata.id;
                if (sessionModule?.loadSessions)
                  await sessionModule.loadSessions();
                if (sessionModule?.selectSession)
                  await sessionModule.selectSession(activeSid);
              }
            }
          } catch (e) {
            console.error('reply: bare session create failed', e);
          }
        }
        ...
        if (!docRes.ok) {
          ...
          console.error(
            '[reply-debug] POST /api/document failed',
            docRes.status,
            errText,
          );
...
async function _createEmailChat(emailData) {
  try {
    ...
    if (url && model) {
      ...
      if (meta)
        meta.textContent = `Email: ${(emailData.subject || '').slice(0, 60)}`;
    }
  } catch (e) {
  ...
  try {
    ...
    if (!sid) {
      try {
        ...
        console.error('compose: bare session create failed', e);
      }
    }
    if (!sid) {
