...
function _taskBadge(task) {
  if (task._unreachable && task.status === 'running')
    return { text: 'unreachable', cls: 'cookbook-task-error' };
  if (task.type === 'download' && task.status === 'running') {
    return {
      text: _statusLabel(task.status, task.type),
      cls: 'cookbook-task-downloading',
    };
  }
...
function _ggufDisplayPartFromPath(path) {
  ...
  const text = `${dir} ${file}`;
  const quant = text.match(
    /\b(?:UD-)?(?:IQ[1-8]_[A-Z0-9]+|Q[2-8]_K_[MLS]|Q[2-8]_[0-9A-Z]+|Q[2-8])\b/i,
  );
...
function _canLaunchDownloadedTask(task) {
  return (
    ...
    !!(task.payload?.repo_id || task.name)
  );
}
...
function _downloadOutputLooksActive(task) {
  ...
  if (!out) return false;
  if (out.includes('DOWNLOAD_OK') || out.includes('DOWNLOAD_FAILED'))
    return false;
  ...
  return (
    /model-\d+-of-\d+\.[a-z]+:\s+(?!100%)\d+%/i.test(out) ||
...
function _canClearTask(task) {
  ...
  if (
    ...
    (task.status === 'ready' ||
      (task._serveReady &&
        !['stopped', 'error', 'crashed', 'failed', 'completed'].includes(
          task.status,
        )))
  )
    return false;
...
function _serveTaskLooksAwqOnLocalBackend(task, outputText = '') {
  const repo =
    `${task?.payload?.repo_id || ''} ${task?.name || ''}`.toLowerCase();
  const cmd = `${task?.payload?._cmd || ''} ${outputText || ''}`.toLowerCase();
  return (
    /\b(awq|gptq|fp8)\b/.test(repo) &&
    /(llama-server|llama_cpp\.server|ollama|ggml_cuda_enable_unified_memory)/.test(
      cmd,
    )
  );
}
...
function _serveTaskLooksAwqWithoutUsableAccelerator(task, outputText = '') {
  ...
  return (
    /\b(awq|gptq|fp8)\b/.test(repo) &&
    /(no accelerator|no cuda runtime|failed to infer device type|triton is not supported|0 active driver)/i.test(
      out,
    )
  );
}
...
function _terminalServeDiagnosis(task, outputText) {
  ...
  if (
    !task ||
    task.type !== 'serve' ||
    !['stopped', 'error', 'crashed', 'failed'].includes(task.status) ||
    !out.trim()
  )
    return null;
  ...
  const _isPipTask =
    (task.payload?.repo_id || '').startsWith('pip-') ||
  ...
  if (_serveTaskLooksAwqOnLocalBackend(task, out)) {
    return {
      ...
      fixes: [
        {
          label: 'Find GGUF download',
          action: () => _openDownloadForGgufTask(task),
        },
  ...
  if (_serveTaskLooksAwqWithoutUsableAccelerator(task, out)) {
    return {
      message:
        'AWQ/GPTQ/FP8 needs a working vLLM/SGLang accelerator path; this server did not expose one.',
      suggestion:
        'Suggested action: choose a CUDA/ROCm server where vLLM/SGLang can see the GPU, or download a GGUF version and serve it with llama.cpp/Ollama.',
      fixes: [
        {
          label: 'Find GGUF download',
          action: () => _openDownloadForGgufTask(task),
  ...
  return (
    _diagnose(out) || {
      message:
        /Native llama-server not found|building llama-server|llama\.cpp/i.test(
          out,
        )
...
function _redactCrashReportText(text) {
  ...
  return String(text)
    ...
    .replace(
      /\b((?:HF_TOKEN|HUGGING_FACE_HUB_TOKEN|OPENAI_API_KEY|ANTHROPIC_API_KEY|BRAVE_API_KEY|TAVILY_API_KEY|SERPER_API_KEY|GOOGLE_API_KEY|API_KEY|TOKEN|PASSWORD)\s*=\s*)(['"]?)[^\s'"\\]+/gi,
      '$1$2[redacted]',
    )
    .replace(
      /\b(--(?:api-key|token|hf-token|password)\s+)([^\s]+)/gi,
      '$1[redacted]',
    );
}
...
function _buildCrashReport(task, outputText) {
  ...
  report.push(
    ...
    '```text',
    _codeFence(_lastLines(capturedOutput)),
    '```',
  );
  return report.join('\n');
}
...
function _taskServerSelection(task) {
  ...
  const savedKey =
    task?.remoteServerKey || task?.payload?.remote_server_key || '';
  const server =
...
function _selectTaskServer(task) {
  ...
  document
    ...
    .forEach((sel) => {
      ...
      else if (host && [...sel.options].some((o) => o.value === host))
        sel.value = host;
...
export function _parseServePhase(snapshot) {
  ...
  const dlMatches = downloadingMatches.length
    ? downloadingMatches
    : fetchingMatches;
  // "Avg generation throughput: X tokens/s, Running: N reqs"
  const tpsMatches = [
    ...flat.matchAll(
      /(?:Avg )?generation throughput:\s*([\d.]+)\s*tokens\/s.*?Running:\s*(\d+)\s*reqs/g,
    ),
  ];
  ...
  const llamaBuildMatches = [
    ...flat.matchAll(/\[\s*(\d{1,3})%\]\s*(?:Building|Linking)/gi),
  ];
  if (llamaBuildMatches.length) {
    const pct = Math.min(
      100,
      parseInt(llamaBuildMatches[llamaBuildMatches.length - 1][1], 10),
    );
    return { phase: `building llama.cpp ${pct}%`, status: 'running', pct };
  }
  if (/Native llama-server not found|building from source/i.test(flat)) {
    if (
      /Cloning into ['"]?llama\.cpp/i.test(flat) &&
      !/Receiving objects:\s*100%/i.test(flat)
    ) {
    ...
    if (
      /CMAKE_BUILD_TYPE|Detecting CXX|Found Threads|Including CPU backend|CUDA nvcc found|building llama-server/i.test(
        flat,
      )
    ) {
...
async function _removeEndpointByUrl(baseUrl) {
  try {
    ...
    if (ep) {
      await fetch(`/api/model-endpoints/${ep.id}`, {
        method: 'DELETE',
        credentials: 'same-origin',
      });
...
function _refreshModelsAfterEndpointChange() {
  ...
  setTimeout(() => {
    ...
    const currentModel = window.sessionModule.getCurrentModel
    ...
    if (currentModel) {
      ...
      if (!stillExists && allModels.length > 0) {
        ...
        if (window.sessionModule.createDirectChat) {
          window.sessionModule.createDirectChat(
            fallback.url,
            fallback.mid,
            fallback.endpointId,
          );
...
function _endpointFromAdvertisedUrl(
  rawUrl,
  currentHost,
  fallbackPort = '11434',
) {
  try {
    ...
    const host = _isAnyBindHost(u.hostname)
      ? currentHost
    ...
    return {
      host,
      port,
      baseUrl: `${u.protocol}//${bracketedHost}${port ? `:${port}` : ''}/v1`,
    };
  } catch {
    return null;
  }
...
function _processQueue() {
  ...
  const running = tasks.filter(
    (t) => t.type === 'download' && t.status === 'running',
  );
  const queued = tasks.filter(
    (t) => t.type === 'download' && t.status === 'queued',
  );
  ...
  try {
    ...
    if (!res.ok) {
      ...
      _updateTask(task.sessionId, {
        status: 'error',
        output: `HTTP ${res.status}: ${errText.slice(0, 200)}`,
      });
      _renderRunningTab();
      return;
    }
    ...
    _updateTask(task.sessionId, {
      status: 'error',
      output: e.message || 'Network error',
    });
    _renderRunningTab();
  }
}
...
function _normalizeTaskForDisplay(task) {
  ...
  if (_isPipTask) {
    ...
    const out = String(task.output || '');
    const ranOk =
      /Successfully installed|Requirement already (?:satisfied|up-to-date)/i.test(
        out,
...
export function _loadTasks() {
  try {
    return (JSON.parse(localStorage.getItem(TASKS_KEY)) || []).map(
      _normalizeTaskForDisplay,
    );
  } catch {
    return [];
  }
...
function _downloadRepoKey(task) {
  return String(
    task?.payload?.repo_id || task?.repo_id || task?.repo || task?.name || '',
  ).trim();
}

function _downloadHostKey(task) {
  return (
    String(task?.remoteHost || task?.payload?.remote_host || 'local').trim() ||
    'local'
  );
}
...
function _tombstoneTask(id) {
  ...
  tomb[id] = now;
  for (const k in tomb) {
    if (now - tomb[k] > _TOMBSTONE_TTL_MS) delete tomb[k];
  }
...
function _redactStoredText(value) {
  return String(value || '')
    ...
    .replace(
      /((?:api[_-]?key|token|authorization|password|passwd|secret)\s*[=:]\s*)(["']?)[^\s"']+/gi,
      '$1$2[redacted]',
    );
}
...
function _redactTaskForStorage(task) {
  ...
  if (typeof safe.output === 'string')
    safe.output = _redactStoredText(safe.output);
  if (safe.payload && typeof safe.payload === 'object') {
    ...
    if (typeof safe.payload._cmd === 'string')
      safe.payload._cmd = _redactStoredText(safe.payload._cmd);
    if (typeof safe.payload.cmd === 'string')
      safe.payload.cmd = _redactStoredText(safe.payload.cmd);
  }
  return safe;
}
...
function _stripStateSecrets(state) {
  ...
  if (Array.isArray(safe.tasks))
    safe.tasks = safe.tasks.map(_redactTaskForStorage);
  return safe;
}
...
export function _saveTasks(tasks) {
  localStorage.setItem(
    TASKS_KEY,
    JSON.stringify((tasks || []).map(_redactTaskForStorage)),
  );
  _syncToServer();
}
...
export function _addTask(sessionId, name, type, payload) {
  ...
  const remoteHost =
    (payload && payload.remote_host) || _envState.remoteHost || '';
  const remoteServerKey = (payload && payload.remote_server_key) || '';
  const remoteServerName = (payload && payload.remote_server_name) || '';
  const sshPort =
    (payload && payload.ssh_port) ||
    _getPort(remoteServerKey || remoteHost) ||
    '';
  const platform =
    (payload && payload.platform) ||
    _getPlatform(remoteServerKey || remoteHost) ||
    '';
  ...
  const task = _redactTaskForStorage({
    id: sessionId,
    sessionId,
    name,
    type,
...
function _updateTask(sessionId, updates) {
  ...
  if (updates.status && updates.status !== 'running') {
    const el = document.querySelector(
      `.cookbook-task[data-task-id="${sessionId}"]`,
    );
    if (el) {
...
function _winSessionCmd(task, tmuxArgs) {
  ...
  return host
    ? `ssh ${pf}${host} 'tmux ${tmuxArgs}' 2>/dev/null`
...
function _taskLooksOllama(task, outputText = '') {
  ...
  return (
    /\bollama\b/i.test(haystack) ||
    /Ollama API ready on port\s+\d+/i.test(haystack)
  );
}
...
function _ollamaBaseUrlForTask(task, outputText = '') {
  ...
  const ready = out.match(
    /Ollama API ready on port\s+\d+:\s*(http:\/\/[^\s]+)/i,
  );
...
function _ollamaModelForTask(task) {
  return String(
    task?.payload?.model || task?.payload?.repo_id || task?.name || '',
  ).trim();
}
...
function _showCookbookNotif(isError = false) {
  ...
  if (btn) {
    btn.style.opacity = '1';
    btn.classList.add('cookbook-notif-active');
  }
  ...
  if (railBtn) {
    ...
    railBtn.classList.add(
      'rail-notify',
      isError ? 'rail-notify-error' : 'rail-notify-success',
      'cookbook-notif-active',
    );
  }
...
export function _clearCookbookNotif() {
  ...
  if (btn) {
    btn.style.opacity = '';
    btn.classList.remove('cookbook-notif-active');
  }
  const railBtn = document.getElementById('rail-cookbook');
  if (railBtn) {
...
function _presetEnvFields(task) {
  ...
  if (p._envPath && (p._env === 'venv' || p._env === 'conda'))
    fields.venv = fields.venv || p._envPath;
...
function _redactPresetForStorage(preset) {
  ...
  if (typeof safe.command === 'string')
    safe.command = _redactStoredText(safe.command);
...
function _saveTaskAsPreset(task, label) {
  ...
  presets.push(
    _redactPresetForStorage({
      name: task.name,
      model: task.payload.repo_id,
      backend: 'vllm',
      host,
      port,
...
function _autoConfigLabel(task) {
  ...
  if (ml) {
    const n = parseInt(ml[1]);
    bits.push((n >= 1024 ? Math.round(n / 1024) + 'k' : n) + ' ctx');
  }
...
function _autoSaveWorkingConfig(task) {
  ...
  if (cmd.includes('diffusion_server')) {
...
function _syncToServer() {
  ...
  _syncTimer = setTimeout(async () => {
    try {
      ...
      if (
        !_envState ||
        !Array.isArray(_envState.servers) ||
        _envState.servers.length === 0
      )
        return;
      const state = {
        tasks: _loadTasks(),
...
function _normalizeState(state) {
  ...
  if (state.env && Array.isArray(state.env.servers)) {
    for (const s of state.env.servers) {
      ...
      dirs = dirs
        .map((d) =>
          (d || '').replaceAll('\u2715', '').replaceAll('\u2716', '').trim(),
        )
        .filter(Boolean);
      if (!dirs.includes('~/.cache/huggingface/hub'))
        dirs.unshift('~/.cache/huggingface/hub');
...
export async function _syncFromServer() {
  try {
    ...
    const serverTombstones =
      state.removedTasks && typeof state.removedTasks === 'object'
        ? state.removedTasks
        : {};
    ...
    for (const [id, ts] of Object.entries(serverTombstones)) {
      if (
        localTombstones[id] == null ||
        Number(ts) > Number(localTombstones[id])
      )
    ...
    localStorage.setItem(
      TASKS_KEY,
      JSON.stringify(merged.map(_redactTaskForStorage)),
    );
    ...
    if (state.env) {
      ...
      const selected =
        (_envState.remoteServerKey &&
          _serverByVal?.(_envState.remoteServerKey)) ||
        (_envState.remoteHost
          ? (_envState.servers || []).find(
              (s) => s.host === _envState.remoteHost,
            )
          : null);
      if (selected) {
        ...
        const local = (_envState.servers || []).find(
          (s) => !s.host || s.host === 'local',
        );
...
async function _retryTask(el, task) {
  ...
  if (badge) {
    ...
    badge.className = 'cookbook-task-status';
  }
  try {
  ...
  if (task.payload) {
    if (task.type === 'serve' && task.payload._cmd) {
      ...
      _launchServeTask(
        ...
        task.payload._cmd,
        task.payload._fields,
        task.remoteHost || '',
      );
    } else {
      uiModule.showToast(
        'Retrying download — progress may look reset while HuggingFace checks cached files, then it should resume.',
        7000,
      );
      _updateTask(task.sessionId, {
        status: 'running',
...
async function _retryDownload(name, payload, replaceSessionId = '') {
  try {
    ...
    if (!res.ok) {
      ...
      if (replaceSessionId)
        _updateTask(replaceSessionId, { status: 'crashed', _retrying: false });
      return;
    }
    const data = await res.json();
    if (!data.ok) {
...
async function _openServeEditForTask(task, cmdOverride, fieldOverrides = null) {
  ...
  if (!repo) {
    uiModule.showToast('No model info on this task');
    return;
  }
  ...
  try {
    const { openServePanelForRepo } = await import(
      '$lib/legacy/cookbookServe.js'
    );
...
export async function _serveAutoRetryReplace(panel, flag, value) {
  ...
  try {
    await fetch('/api/shell/exec', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        command: _tmuxCmd(task, `kill-session -t ${taskId}`),
      }),
    });
  } catch {}
  ...
  return new Promise((resolve) => {
    ...
    const onKey = (e) => {
      if (e.key === 'Escape') close(null);
        ...
        close(ta.value.trim() || null);
    };
    overlay
      .querySelector('.cookbook-edit-cancel')
      .addEventListener('click', () => close(null));
    overlay
      .querySelector('.cookbook-edit-save')
      .addEventListener('click', () => close(ta.value.trim() || null));
...
function _parseServeCmdToFields(cmd) {
  ...
  const fields = {
    ...
    llama_split_mode:
      ex(/(?:--split-mode|-sm)\s+(none|layer|row|tensor)/) || '',
  ...
  const spec = cmd.match(
    /--speculative-config\s+'?\{[^}]*"method"\s*:\s*"([^"]+)"[^}]*"num_speculative_tokens"\s*:\s*(\d+)/,
  );
  if (spec) {
    fields.spec_method = spec[1];
    fields.spec_tokens = spec[2];
  }
  return fields;
}
...
export async function _launchServeTask(
  ...
  const _hsrv =
    (_targetKey && _targetKey !== 'local' ? _serverByVal(_targetKey) : null) ||
    (hostOverride === undefined
      ? _serverByVal(_envState.remoteServerKey || _host)
      : null) ||
    _envState.servers.find((s) => s.host === _host) ||
    {};
  const _serverMetaKey =
    _targetKey ||
    (_hsrv && _serverKey ? _serverKey(_hsrv) : '') ||
    _host ||
    'local';
  const _serverMetaName =
    targetMeta?.serverName || _hsrv.name || (_host ? _host : 'Local');
  ...
  if (_isWindows()) {
    ...
    if (_envState.env === 'venv' && _envState.envPath) {
      ...
      envPrefix =
        'source ' + (p.endsWith('/bin/activate') ? p : p + '/bin/activate');
    } else if (_envState.env === 'conda' && _envState.envPath) {
      envPrefix =
  ...
  try {
    ...
    if (!data.ok) {
      ...
      console.error('[cookbook] /api/model/serve failed', {
        status: res.status,
        body: data,
      });
    ...
    const payload = {
      repo_id: repo,
      remote_host: _host || undefined,
      remote_server_key: _serverMetaKey || undefined,
      remote_server_name: _serverMetaName || undefined,
      ssh_port: _sp || undefined,
      _cmd: cmd,
      _fields: fields || undefined,
      _env: _usedEnv,
      _envPath: _usedEnvPath,
      _gpus: _usedGpus,
    };
...
export function _renderRunningTab() {
  ...
  try {
    const _activeTasks = _loadPrunedTasks().filter(
      (t) =>
        t.status === 'running' || t.status === 'queued' || t.status === 'error',
    );
  ...
  body.querySelectorAll('.cookbook-task').forEach((tEl) => {
    ...
    if (!wrap) return;
    if (wrap.classList.contains('cookbook-task-collapsed'))
  ...
  if (_soloExpandTaskId) {
    ...
    _allIds.forEach((id) => {
      if (id !== _soloExpandTaskId) _collapsedTaskIds.add(id);
    });
  ...
  const activeCountHtml = activeCount
    ? ` <span class="cookbook-tab-count">${activeCount}</span>`
    : '';
  ...
  if (hasContent && !runTab) {
    ...
    const _errCount = tasks.filter(
      (t) => t.status === 'error' || t.status === 'crashed',
    ).length;
    ...
    const _errCount2 = tasks.filter(
      (t) => t.status === 'error' || t.status === 'crashed',
    ).length;
    runTab.innerHTML = tasks.length
      ? `Active${activeCountHtml}${_errCount2 ? '<span class="cookbook-tab-error-dot"></span>' : ''}`
      : 'Active';
    if (!hasContent) {
      if (runTab.classList.contains('active')) {
        const wfTab = tabBar.querySelector(
          '.cookbook-tab[data-backend="Search"]',
        );
  ...
  let group = body.querySelector(
    '.cookbook-group[data-backend-group="Running"]',
  );
  if (hasContent && !group) {
    ...
    group.innerHTML =
      ...
      '<h2 style="margin:0;padding:0;line-height:1;">Active <span id="running-count" class="memory-count" style="font-size:0.6em;opacity:0.6;font-weight:normal">' +
      activeCount +
      '</span></h2>' +
      '</div>' +
  ...
  const _taskServerKey = (task) =>
    task?.remoteServerKey || task?.remoteHost || '';
  const _serverName = (keyOrTask) => {
    if (keyOrTask && typeof keyOrTask === 'object') {
      ...
      const srv = task.remoteServerKey
        ? _serverByVal(task.remoteServerKey)
        : null;
      if (srv?.name) return srv.name;
      if (!task.remoteHost) return 'Local';
      return (
        _envState.servers.find((s) => s.host === task.remoteHost)?.name ||
        task.remoteHost
      );
    }
    const key = keyOrTask || '';
  ...
  for (const t of tasks) {
    ...
    if (!serverGroups[key])
      serverGroups[key] = { name: _serverName(t), serve: [], download: [] };
    serverGroups[key][t.type === 'serve' ? 'serve' : 'download'].push(t);
  }

  // ── Server-grouped sections ──
  group
    .querySelectorAll('.cookbook-serve-section, .cookbook-dl-section')
  ...
  const _liveSafeKeys = new Set(
    serverKeys.map((k) => (k || 'local').replace(/[^a-zA-Z0-9-]/g, '_')),
  );
  (_adminCard || group)
    .querySelectorAll('[class*="cookbook-server-section-"]')
    .forEach((el) => {
      const cls = [...el.classList].find((c) =>
        c.startsWith('cookbook-server-section-'),
      );
      if (
        cls &&
        !_liveSafeKeys.has(cls.replace('cookbook-server-section-', ''))
      )
        el.remove();
    });
  ...
  group.querySelectorAll('[data-clear-server]').forEach((btn) => {
    ...
    btn.addEventListener('click', async (e) => {
      ...
      const toRemove = allTasks.filter(
        (t) => _taskServerKey(t) === host && _canClearTask(t),
      );
      ...
      if (!toRemove.length) {
        const stillRunning = allTasks.filter(
          (t) => _taskServerKey(t) === host && t.status === 'running',
        ).length;
      ...
      if (
        !(await window.styledConfirm(
          `Clear ${toRemove.length} finished task${toRemove.length === 1 ? '' : 's'} on ${_serverName(host)}?`,
          { confirmText: 'Clear' },
        ))
      )
        return;
      ...
      toRemove.forEach((t) => {
        const el = document.querySelector(
          `.cookbook-task[data-task-id="${t.sessionId}"]`,
        );
        if (el) {
          if (el._abort) el._abort.abort();
      ...
      setTimeout(() => {
        toRemove.forEach((t) =>
          document
            .querySelector(`.cookbook-task[data-task-id="${t.sessionId}"]`)
            ?.remove(),
        );
  ...
  group.querySelectorAll('[data-stop-server]').forEach((btn) => {
    ...
    btn.addEventListener('click', async (e) => {
      ...
      const running = _loadTasks().filter(
        (t) => _taskServerKey(t) === host && t.status === 'running',
      );
      if (!running.length) {
        uiModule.showToast(`Nothing running on ${_serverName(host)}`);
        return;
      }
      if (
        !(await window.styledConfirm(
          `Stop ${running.length} running task${running.length > 1 ? 's' : ''} on ${_serverName(host)}?`,
      ...
      uiModule.showToast(
        `Stopped ${running.length} task${running.length > 1 ? 's' : ''} on ${_serverName(host)}`,
      );
    });
  });
  ...
  group
    .querySelectorAll('.cookbook-section-header[data-collapse]')
    .forEach((hdr) => {
  ...
  group.querySelectorAll('.cookbook-task').forEach((el) => {
    ...
    if (task) {
      ...
      if (badge) {
        ...
        badge.className =
          'cookbook-task-status' + (_bdg.cls ? ' ' + _bdg.cls : '');
        badge.style.display = '';
      }
      ...
      if (startNow)
        startNow.style.display =
          task.type === 'download' && task.status === 'queued' ? '' : 'none';
      const terminalDiag = _terminalServeDiagnosis(
        task,
  ...
  for (const task of tasks) {
    ...
    el.className =
      'cookbook-task' +
      (task._unreachable && task.status === 'running'
    ...
    const _bdgTitle =
      task._unreachable && task.status === 'running'
        ? ' title="Server not responding — it may have crashed"'
        : '';
    ...
    if (
      !terminalDiag &&
      (task.status === 'error' || task.status === 'crashed') &&
      task._backendDiagnosis
    ) {
    ...
    if (
      _uptimeEl &&
      (task.type === 'serve' || task.type === 'download') &&
      task.status === 'running'
    ) {
          ...
          if (_pct >= 3 && _pct < 100 && secs > 5) {
            ...
            _eta =
              _eh > 0
                ...
                : _em > 0
                  ? ` · ETA ${_em}m ${String(_es).padStart(2, '0')}s`
                  : ` · ETA ${_es}s`;
          }
        }
      ...
      if (_serveBtn) {
        _serveBtn.addEventListener('click', async (e) => {
          ...
          try {
            ...
            uiModule.showToast('Could not open Serve: ' + err.message);
          }
        });
      }
    }
      ...
      _clearChk.addEventListener('click', (e) => {
        ...
        try {
          fetch('/api/shell/exec', {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              command: _tmuxCmd(task, `kill-session -t ${task.sessionId}`),
            }),
          }).catch(() => {});
        } catch {}
        _animateOutThenRemove(el, task.sessionId);
    ...
    if (menuBtn) {
      ...
      el.addEventListener(
        ...
        (e) => {
          ...
          if (
            e.target.closest(
              'button, a, input, textarea, .cookbook-task-dropdown',
            )
          )
            return;
          _lpStart(e);
      ...
      menuBtn.addEventListener('click', (e) => {
        ...
        if (task.type === 'serve' && task.payload?.repo_id) {
          items.push({
            ...
            action: 'edit-panel',
            tooltip:
              'Open the Serve config panel pre-filled with this task — pick a different backend, change GPUs, edit env vars or the raw cmd, then Launch.',
            custom: () => _openEdit(),
          });
        }
        if (task.type === 'serve' && task.payload?._cmd) {
            ...
            custom: async () => {
              ...
              try {
                ...
                if (existing) {
                  ...
                  if (existing.id && !(existing.models || []).length)
                    _probeEndpointUntilOnline(existing.id, host, port);
                  return;
                }
                ...
                if (task.payload?._cmd?.includes('diffusion_server'))
                  fd.append('model_type', 'image');
                const res = await fetch('/api/model-endpoints', {
                  method: 'POST',
                  credentials: 'same-origin',
                  body: fd,
                ...
                if (res.ok) {
                  ...
                  if (_ep && _ep.id)
                    _probeEndpointUntilOnline(_ep.id, host, port);
                } else {
                  const body = await res.text().catch(() => '');
                  uiModule.showError(
                    `Register failed: ${res.status} ${body.slice(0, 140)}`,
                  );
                }
              } catch (e) {
        ...
        if (_shouldOfferCrashReport(task)) {
          items.push({
            group: 'copy',
            label: 'Copy crash report',
            action: 'copy-crash-report',
            custom: () => {
              const out =
                el.querySelector('.cookbook-output-pre')?.textContent ||
                task.output ||
        ...
        const _isLive =
          task.type === 'serve' &&
          ['running', 'ready', 'loading', 'warming', 'starting'].includes(
            task.status || '',
          );
        items.push({
          group: 'danger',
        ...
        const _MENU_ICONS = {
          ...
          reconnect:
            '<path d="M1 4v6h6"/><path d="M3.5 15a9 9 0 1 0 2.1-9.4L1 10"/>',
          retry:
            '<path d="M1 4v6h6"/><path d="M3.5 15a9 9 0 1 0 2.1-9.4L1 10"/>',
          ...
          'copy-crash-report':
            '<path d="M10.3 2.3 1.8 17a2 2 0 0 0 1.7 3h17a2 2 0 0 0 1.7-3L13.7 2.3a2 2 0 0 0-3.4 0z"/><path d="M12 8v5M12 17h.01"/>',
          'copy-log':
            '<rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/>',
        ...
        for (const item of items) {
          ...
          if (item.group && _lastGroup && item.group !== _lastGroup) {
            ...
            sep.style.cssText =
              'height:1px;margin:4px 6px;background:color-mix(in srgb, var(--fg) 12%, transparent);pointer-events:none;';
        ...
        // else clamp to the bottom edge.
        {
          ...
          if (top + dh > viewBottom - m) {
            ...
            top =
              above >= viewTop + m
                ? above
                : Math.max(viewTop + m, viewBottom - dh - m);
          }
    ...
    // Wire reconnect
    el.querySelector('.cookbook-task-action-reconnect').addEventListener(
      'click',
      () => {
    ...
    // Wire stop
    el.querySelector('.cookbook-task-action-stop').addEventListener(
      'click',
      async () => {
    ...
    el.querySelector('.cookbook-task-action-kill').addEventListener(
      'click',
      async () => {
        const outputText =
          el.querySelector('.cookbook-output-pre')?.textContent ||
            ...
            if (task.sessionId && isLive) {
              try {
                const probe = await fetch('/api/shell/exec', {
                  ...
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({
                    command: _tmuxCmd(task, `has-session -t ${task.sessionId}`),
                  }),
                });
                if (probe.ok) {
        ...
        if (!killOk) {
          try {
            uiModule.showToast(
              'Kill failed — session may still be running. Check `tmux ls` on the server.',
              'error',
            );
          } catch (_) {}
        ...
        if (task.type === 'serve' && task.payload) {
          ...
          if (modelName) {
            fetch('/api/model-endpoints', { credentials: 'same-origin' })
              ...
              .then((eps) => {
                const ep = eps.find(
                  (e) => e.name === modelName || e.base_url === endpointUrl,
                );
                if (ep)
                  fetch(`/api/model-endpoints/${ep.id}`, {
                    method: 'DELETE',
                    credentials: 'same-origin',
                  }).then(() => _refreshModelsAfterEndpointChange());
              })
              .catch(() => {});
          }
        }
    ...
    // Wire retry
    el.querySelector('.cookbook-task-action-retry').addEventListener(
      'click',
      () => _retryTask(el, task),
    );
  ...
  _collapsedTaskIds.forEach((id) => {
    const wrap = body.querySelector(
      `.cookbook-task[data-task-id="${id}"] .cookbook-output-wrap`,
    );
  ...
  _collapsedSectionIds.forEach((sid) => {
    ...
    const hdr = body.querySelector(
      `.cookbook-section-header[data-collapse="${sid}"]`,
    );
    const chevron = hdr?.querySelector('.cookbook-section-chevron');
    if (chevron) {
    ...
    try {
      const res = await fetch('/api/shell/exec', {
        ...
        body: JSON.stringify({
          command: _tmuxCmd(
            task,
            `capture-pane -t ${task.sessionId} -p -S -200`,
          ),
          timeout: 15,
        }),
      });
      ...
      if (data.exit_code !== 0) {
        ...
        if (diag) {
          ...
          const downloadLooksSuccessful =
            !lastOutput.includes('DOWNLOAD_FAILED') &&
          ...
          const _isPipTask =
            (task.payload?.repo_id || '').startsWith('pip-') ||
            /python3? -m pip\b/.test(task.payload?._cmd || '');
          const pipLooksSuccessful =
            _isPipTask &&
            /Successfully installed|Requirement already (?:satisfied|up-to-date)/i.test(
              lastOutput,
            ) &&
            !/error:|ERROR:/.test(lastOutput.slice(-1024));
          const serveLooksReady =
            task.type === 'serve' &&
          ...
          const depInstallSucceeded =
            !!task.payload?._dep && _depInstallSucceeded(lastOutput);
          ...
          if (!lastOutput.trim() || !looksSuccessful) {
            ...
            const badge = el.querySelector('.cookbook-task-status');
            if (badge) {
              badge.textContent = _statusLabel('crashed', task.type);
              badge.className = 'cookbook-task-status cookbook-task-crashed';
            }
            if (_isPipTask) {
              ...
              if (!_ranOk) {
                _showDiagnosis(
                  ...
                  {
                    message:
                      'Pip install did not finish with a success marker. Check the output for the underlying error.',
                    suggestion:
                      'Suggested action: copy the troubleshooting bundle. Common causes: missing build deps, network blip, mismatched torch ABI.',
                    fixes: [],
                  },
                  lastOutput,
                );
              ...
              const diag = _diagnose(lastOutput) || {
                ...
                fixes: [
                  {
                    label: 'Edit serve',
                    action: (panel) => _openServeEditForTask(task),
                  },
                ],
              };
              _showDiagnosis(el, diag, lastOutput);
            } else if (task.type === 'download') {
              const isDisk = /no space left|disk quota|enospc/i.test(
                lastOutput,
              );
              const isNetwork =
                /connection|timeout|timed out|incompleteread|chunkedencoding|reset by peer|protocolerror|all connection attempts failed/i.test(
                  lastOutput,
                );
              ...
              const _reconnectFix = {
                ...
                action: () => {
                  ...
                  if (badge2) {
                    badge2.textContent = _statusLabel('running', task.type);
                    badge2.className = 'cookbook-task-status';
                  }
                  ...
                  ? [
                      {
                        ...
                        action: () => _retryTask(el, task),
                      },
                      {
                        label: 'Copy last 50 lines',
                        action: () => {
                          const last = String(lastOutput || '')
                            .split('\n')
                            .slice(-50)
                            .join('\n');
            ...
            if (_strongDone) {
              ...
              const _chk = el.querySelector('.cookbook-task-check');
              if (_chk) _chk.style.display = '';
              const _sb = el.querySelector('.cookbook-task-serve-btn');
            ...
            if (!task._doneConfirmAt) {
              _updateTask(task.sessionId, {
                _doneConfirmAt: Date.now() + 30000,
              });
              setTimeout(async () => {
                try {
                  const fresh = _loadTasks().find(
                  ...
                  try {
                    ...
                    /* network blip — treat as inconclusive, prefer running */ stillAlive = true;
                  }
                  if (stillAlive) {
                    _updateTask(task.sessionId, {
                      status: 'running',
                    ...
                    const _el = document.querySelector(
                      `.cookbook-task[data-task-id="${task.sessionId}"]`,
                    );
                    if (_el) {
                      _el.dataset.status = 'running';
                      const _badge = _el.querySelector('.cookbook-task-status');
                      if (_badge) {
                        _badge.textContent = _statusLabel('running', task.type);
                        _badge.className = 'cookbook-task-status';
                      }
                  ...
                  if (_el) {
                    ...
                    const _badge = _el.querySelector('.cookbook-task-status');
                    if (_badge) {
                      _badge.textContent = _statusLabel('done', task.type);
                      _badge.className =
                        'cookbook-task-status cookbook-task-done';
                    }
                    const _chk = _el.querySelector('.cookbook-task-check');
                    if (_chk) _chk.style.display = '';
                    const _sb = _el.querySelector('.cookbook-task-serve-btn');
                    if (_sb) _sb.style.display = '';
                  }
                  _showCookbookNotif();
                  _refreshDepsAfterInstall(task);
                  ...
                  /* swallow — next polling cycle will retry */
                }
              }, 30000);
            }
          }
      ...
      if (snapshot) {
        ...
        if (task.type === 'download') {
          const badge = el.querySelector('.cookbook-task-status');
          if (badge) {
            const completed = (snapshot.match(/Download complete/g) || [])
              .length;
            const downloading = snapshot.match(/Downloading '([^']+)'/g) || [];
            const totalFiles = downloading.length;
            const pctMatches = [...snapshot.matchAll(/(\d+)%\|/g)];
            const lastPct = pctMatches.length
              ? pctMatches[pctMatches.length - 1][1]
              : null;
            const speedMatch = [...snapshot.matchAll(/([\d.]+)(?:MB|GB)\/s/g)];
            const lastSpeed = speedMatch.length
              ? speedMatch[speedMatch.length - 1][0]
              : null;
            ...
            const _dlAggMatches = [
              ...snapshot.matchAll(
                /Downloading\s*\(incomplete[^)]*\):\s*(\d+)%/g,
              ),
            ];
            const _dlAgg = _dlAggMatches.length
              ? parseInt(_dlAggMatches[_dlAggMatches.length - 1][1])
              : null;
            ...
            const _byteMatches = [
              ...snapshot.matchAll(
                /([\d.]+\s?[KMGT])B?\s*\/\s*[\d.]+\s?[KMGT]B?/gi,
              ),
            ];
            const _bytes = _byteMatches.length
              ? _byteMatches[_byteMatches.length - 1][1].replace(/\s/g, '')
              : null;
            ...
            const curProgress = computeProgressSignal(
              _bytes,
              _dlAgg,
              lastPct,
              snapshot,
            );
            const _fetchPctMatches = [
              ...snapshot.matchAll(/Fetching\s+\d+\s+files:\s*(\d+)%/g),
            ];
            const _fetchPct = _fetchPctMatches.length
              ? parseInt(_fetchPctMatches[_fetchPctMatches.length - 1][1])
              : null;
            const isPipDep = !!(task.payload && task.payload._dep);
            const _startupStalled =
              !_bytes &&
              (_dlAgg === 0 || _fetchPct === 0) &&
              curProgress === '0';
            const _STALE_TIMEOUT = _startupStalled
              ? STARTUP_STALE_PROGRESS_MS
              : STALE_PROGRESS_MS;
            if (!el._lastProgress) {
            ...
            if (curProgress !== el._lastProgress) {
              ...
              !isPipDep &&
              Date.now() - (el._lastProgressTime || 0) > _STALE_TIMEOUT &&
              task._autoRestarted
            ) {
              const mins = Math.floor(
                (Date.now() - (el._lastProgressTime || 0)) / 60000,
              );
              ...
              badge.textContent = _startupStalled
                ? '0% stall — retrying'
                : 'stale — restarting';
              badge.className = 'cookbook-task-status cookbook-task-error';
              _showCookbookNotif(true);
              ...
              try {
                ...
                if (data.ok && data.session_id) {
                  ...
                  badge.className =
                    'cookbook-task-status cookbook-task-running';
                  continue;
                }
              } catch {}
            ...
            const _shardPat = [
              ...snapshot.matchAll(
                /model-(\d+)-of-(\d+)\.(?:safetensors|bin)/g,
              ),
            ];
            const _lastShard = _shardPat.length
              ? _shardPat[_shardPat.length - 1]
              : null;
            const _curShardNum = _lastShard
              ? parseInt(_lastShard[1], 10)
              : null;
            const _totalShards = _lastShard
              ? parseInt(_lastShard[2], 10)
              : null;
            const _useShardAgg =
              _curShardNum && _totalShards && _totalShards > 1;
            ...
            if (_useShardAgg) {
              ...
              const curShardFrac =
                _dlAgg != null
                  ? _dlAgg / 100
                  : lastPct
                    ? parseInt(lastPct, 10) / 100
                    : 0;
              let overallPct = Math.round(
                ((_curShardNum - 1 + curShardFrac) / _totalShards) * 100,
              );
              if (_fetchPct != null)
                overallPct = Math.max(overallPct, _fetchPct);
              let text = `${overallPct}%`;
              ...
              let overallPct = Math.round(
                ((completed + curFilePct) / totalFiles) * 100,
              );
              if (_fetchPct != null)
                overallPct = Math.max(overallPct, _fetchPct);
              let text = `${overallPct}%`;
            ...
            if (snapshot.includes('DOWNLOAD_FAILED')) {
              ...
              const _accessDenied =
                /Access to model.*is restricted|gated repo|GatedRepoError|401 Unauthorized|403 Forbidden|not in the authorized list|awaiting a review|must (?:be authenticated|have access)/i.test(
                  snapshot,
                );
              ...
              if (
                ...
                badge.className = 'cookbook-task-status cookbook-task-running';
                uiModule.showToast(
                  `Download interrupted — retrying (${_dlN + 1}/${_DL_MAX_AUTO_RETRY}), resumes where it stopped…`,
                  6000,
                );
                const _p = task.payload,
                  _nm = task.name;
                try {
                  await fetch('/api/shell/exec', {
                    method: 'POST',
            ...
            if (
              snapshot.includes('DOWNLOAD_OK') ||
              (snapshot.includes('/snapshots/') &&
              ...
              if (_typeChip) {
                _typeChip.textContent = 'finished';
                _typeChip.classList.add('cookbook-task-type-done');
              }
              _updateTask(task.sessionId, { status: 'done' });
              const _sb2 = el.querySelector('.cookbook-task-serve-btn');
              if (_sb2) _sb2.style.display = '';
              _showCookbookNotif();
              _refreshDepsAfterInstall(task);
              fetch('/api/shell/exec', {
                method: 'POST',
          ...
          if (badge) {
            ...
            if (info.phase) {
              ...
              if (info.status !== 'ready') {
                if (task.progress !== info.phase)
                  _updateTask(task.sessionId, { progress: info.phase });
        ...
        if (
          ...
          const ollamaUrlMatch = snapshot.match(
            /Ollama API ready on port\s+\d+:\s*(http:\/\/[^\s]+)/i,
          );
          if (ollamaUrlMatch) {
            const endpoint = _endpointFromAdvertisedUrl(
              ollamaUrlMatch[1],
              host,
              '11434',
            );
          ...
          fetch('/api/model-endpoints', { credentials: 'same-origin' })
            ...
            .then(async (eps) => {
              ...
              const _isDiffusion =
                task.payload?._cmd?.includes('diffusion_server');
              const fd = new FormData();
              fd.append('base_url', baseUrl);
              fd.append('name', task.name);
              fd.append('skip_probe', 'true');
            ...
            .then(async (res) => {
              if (res && res.ok) {
                ...
                const _trySelectModel = async (attempt) => {
                  ...
                  if (attempt < 3)
                    setTimeout(() => _trySelectModel(attempt + 1), 2000);
                  else if (window.sessionModule?.updateModelPicker)
                    window.sessionModule.updateModelPicker();
                };
                ...
                uiModule.showError(
                  `Auto-register endpoint failed (${res.status}). Use ⋮ → Register endpoint to retry.`,
                );
              }
            })
            .catch((e) => {
              ...
              uiModule.showError(
                `Auto-register endpoint error: ${e.message || e}. Use ⋮ → Register endpoint to retry.`,
              );
            })
            .finally(() => {
              task._endpointAddInFlight = false;
            });
          _updateTask(task.sessionId, { status: 'running' });
          const badge = el.querySelector('.cookbook-task-status');
          if (badge) {
        ...
        if (snapshot.includes('=== Process exited with code')) {
          const codeMatch = snapshot.match(
          ...
          const status =
            task.type === 'serve' && !task._serveReady
              ? 'error'
              : code === 0
                ? 'done'
                : 'error';
          ...
          if (badge) {
            ...
            badge.className = `cookbook-task-status cookbook-task-${status}`;
          }
          _renderRunningTab();
        }
...
async function _checkServeReachability() {
  let serveTasks;
  try {
    serveTasks = _loadTasks().filter(
      (t) => t.type === 'serve' && t.status === 'running',
    );
  } catch {
    return;
  }
  ...
  try {
    [eps, probe] = await Promise.all([
      fetch('/api/model-endpoints', { credentials: 'same-origin' })
        .then((r) => r.json())
        .catch(() => []),
      fetch('/api/model-endpoints/probe-local', { credentials: 'same-origin' })
        .then((r) => r.json())
        .catch(() => ({})),
    ]);
  } catch {
    ...
    if (el) {
      ...
      if (badge) {
        if (unreachable) {
          ...
          badge.className = 'cookbook-task-status cookbook-task-error';
          badge.title =
            pr.error || 'Server not responding — it may have crashed';
...
function _serveTaskFailed(task) {
  ...
  return (
    !!task._unreachable || ['error', 'crashed', 'failed'].includes(task.status)
  );
}
...
function _syncSettingsServerDots(byKey) {
  document.querySelectorAll('.cookbook-server-entry').forEach((entry) => {
    ...
    _setServerDot(
      ...
      failed,
      failed
        ? 'Server not responding - running serve may have crashed'
        : 'Reachable',
    );
    if (!msg) return;
    ...
    if (failed) {
      ...
      /failed|crashed|not responding|unreachable/i.test(msg.textContent || '')
    ) {
...
function _refreshServerDots() {
  ...
  const byKey = {};
  const _taskServerKeyForDot = (task) =>
    task?.remoteServerKey || task?.remoteHost || '';
  for (const t of tasks) {
    (byKey[_taskServerKeyForDot(t)] =
      byKey[_taskServerKeyForDot(t)] || []).push(t);
  }
  document.querySelectorAll('.cookbook-section-header').forEach((header) => {
    const dot = header.querySelector('.cookbook-srv-status');
    if (!dot) return;
    const key =
      header.querySelector('[data-stop-server]')?.dataset.stopServer || '';
...
export async function _selfHealStaleTasks(opts = {}) {
  ...
  const candidates = tasks.filter((t) => {
    ...
    if (!['done', 'error', 'crashed', 'stopped'].includes(t.status))
      return false;
    ...
    if (t.status === 'done' && /DOWNLOAD_OK|\/snapshots\//.test(t.output || ''))
      return false;
    ...
    if (t._lastStatusFlipAt && Date.now() - t._lastStatusFlipAt < 45000)
      return false;
    return true;
  });
  ...
  for (const t of candidates) {
    try {
      const res = await fetch('/api/shell/exec', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          command: _tmuxCmd(t, `has-session -t ${t.sessionId}`),
          timeout: 5,
        }),
      });
      const data = await res.json();
      ...
      /* network blip — skip this one */
    }
  }
  if (flipped) {
    console.log(
      `[cookbook] auto-reconnect: revived ${flipped} task(s) whose tmux session was still alive`,
    );
    _renderRunningTab();
  }
}
...
async function _probeEndpointUntilOnline(epId, host, port) {
  ...
  for (let i = 0; i < MAX_TRIES; i++) {
    ...
    try {
      ...
      const probeRes = await fetch(`/api/model-endpoints/${epId}/probe`, {
        credentials: 'same-origin',
      }).catch(() => null);
      if (probeRes && probeRes.status === 404) return;
      if (probeRes) await probeRes.text().catch(() => {});
      const eps = await fetch('/api/model-endpoints', {
        credentials: 'same-origin',
      })
        .then((r) => r.json())
        .catch(() => []);
...
async function _pollBackgroundStatus() {
  try {
    ...
    try {
      const stateRes = await fetch('/api/cookbook/state', {
        credentials: 'same-origin',
      });
      if (stateRes.ok) {
        const serverState = await stateRes.json();
        const serverTasks =
          serverState && Array.isArray(serverState.tasks)
            ? serverState.tasks
            : [];
        if (serverTasks.length) {
          ...
          for (const t of serverTasks) {
            if (
              t &&
              t.sessionId &&
              !localIds.has(t.sessionId) &&
              !_isTombstoned(t.sessionId)
            ) {
              merged.push(t);
              added++;
            }
    ...
    try {
      ...
      for (const task of localTasks) {
        ...
        const depDone =
          !!task.payload?._dep && _depInstallSucceeded(task.output);
      ...
      if (changed) {
        ...
        for (const task of localTasks) {
          ...
          const el = document.querySelector(
            `[data-session-id="${CSS.escape(task.sessionId)}"]`,
          );
      ...
      /* non-fatal: background status should never break polling */
    }
    ...
    const readyServes = tasks.filter(
      (t) => t.type === 'serve' && t.status === 'ready',
    );
    for (const t of readyServes) {
      ...
      const portMatch =
        localTask?.payload?._cmd?.match(/--port\s+(\d+)/) ||
      ...
      const _isDiffusion =
        localTask?.payload?._cmd?.includes('diffusion_server');
      ...
      const _supportsTools =
        _cmd.includes('--enable-auto-tool-choice') ||
        (_isDiffusion === false &&
          /(?:^|\s)(?:deepseek|gpt-[45o]|claude|gemini|qwen3|qwen2\.5|mixtral|llama-[34]|minimax|kimi|hermes|glm-4)/i.test(
            t.model,
          ));
...
export { _nextAvailablePort, _processQueue, _retryDownload };
