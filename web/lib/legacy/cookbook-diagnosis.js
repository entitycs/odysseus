      ...
      if (opts.expandRecipe) {
        ...
        if (opts.model) {
          const sel = document.querySelector(
            `[data-dep-recipe-pick="${CSS.escape(opts.expandRecipe)}"]`,
          );
          if (sel) {
            ...
            if (!matched) {
              for (let i = 0; i < sel.options.length; i++) {
                if (
                  sel.options[i].value &&
                  wanted.includes(sel.options[i].value)
                ) {
...
export const ERROR_PATTERNS = [
  ...
  {
    pattern:
      /sgl_kernel[\s\S]*(Python\.h|libnuma\.so\.1|common_ops)|(Python\.h|libnuma\.so\.1|common_ops)[\s\S]*sgl_kernel|Please ensure sgl_kernel is properly installed/i,
    message: 'SGLang native dependencies are missing on this server.',
    fixes: [
      {
        label: 'Copy OS package command',
        action: () =>
          _copyText(
            'sudo apt-get install -y libnuma-dev python3.12-dev build-essential',
          ),
      },
      {
        label: 'Copy kernel upgrade',
        action: () =>
          _copyText('python3 -m pip install --upgrade sglang-kernel'),
      },
      {
        label: 'Open Dependencies',
      ...
      },
      {
        ...
        action: () => {
          ...
          _launchServeTask(
            'update-vllm',
            'pip-update',
            `${_vp} -m pip install -U vllm transformers`,
          );
        },
      },
    ],
    ...
    fixes: [
      {
        ...
        action: () => {
          ...
          _launchServeTask(
            'repair-kernels',
            'pip-update',
            `${_vp} -m pip install --user --break-system-packages "kernels<0.15"`,
          );
        },
      },
      {
  ...
  // missing OS-package toolchain that pip can't ship.
  {
    ...
    message:
      'cmake is required to compile llama.cpp from source, but it is not installed on this server.',
    suggestion:
      'Suggested action: install cmake via the OS package manager — apt: cmake build-essential / pacman: cmake base-devel / dnf: cmake gcc-c++ make / brew: cmake. Cookbook can do this automatically on the next launch if your user has passwordless sudo for apt/pacman/dnf.',
    fixes: [
      {
        label: 'Open Dependencies',
      ...
      {
        label: 'Copy apt install',
        action: () =>
          _copyText('sudo apt install -y cmake build-essential git'),
      },
      {
        label: 'Copy pacman install',
        action: () =>
          _copyText('sudo pacman -Sy --needed cmake base-devel git'),
      },
      {
        label: 'Copy dnf install',
        action: () =>
          _copyText('sudo dnf install -y cmake gcc gcc-c++ make git'),
      },
    ],
  },
  {
    pattern:
      /^(make|g\+\+|gcc): command not found|Could not find C\+\+ compiler/i,
    message:
      'A C/C++ compiler (build-essential / base-devel) is required to compile llama.cpp.',
    fixes: [
      {
        label: 'Open Dependencies',
        action: () => _openCookbookDependencies('llama_cpp'),
      ...
      {
        label: 'Copy apt install',
        action: () => _copyText('sudo apt install -y build-essential'),
      },
    ],
  },
  {
    ...
    fixes: [
      ...
      {
        label: 'Copy apt install',
        action: () => _copyText('sudo apt install -y git'),
      },
    ],
  },
  {
    ...
    fixes: [
      {
        ...
        action: () => {
          ...
          _launchServeTask(
            'update-triton',
            'pip-update',
            `${_vp} -m pip install -U triton triton-kernels`,
          );
        },
      },
    ],
      ...
      },
      {
        ...
        action: () => {
          ...
          _launchServeTask(
            ...
            'pip-update',
            `${_vp} -m pip uninstall flashinfer-python -y`,
          );
        },
      },
      {
    ...
    fixes: [
      {
        ...
        action: () => {
          ...
          _launchServeTask(
            'reinstall-vllm',
            'pip-reinstall',
            `${_vp} -m pip install --force-reinstall vllm`,
          );
        },
      },
      {
  ...
  },
  {
    ...
    match: (text) => {
      ...
      if (
        /Application startup complete|"(?:GET|POST)\s+\/v1\/[^"]+ HTTP\/[\d.]+"\s*2\d\d|Uvicorn running on|server is listening on https?:\/\//i.test(
          TAIL,
        )
      )
        return false;
      return /Failed to build\b|subprocess-exited-with-error|Could not build wheels|metadata-generation-failed/i.test(
        TAIL,
      );
    },
    message:
      'A dependency failed to build during install — usually an older package whose build breaks on this Python version, not a server problem. The install did not finish.',
    suggestion:
      'Suggested action: check the captured output for the package that failed to build; it may need a newer release or a patch to install on this Python version.',
    fixes: [],
  },
  {
  ...
  },
  {
    ...
    message:
      'Python traceback detected — check the captured output below for the underlying error.',
    suggestion:
      'Suggested action: read the captured output for the failing step; copy the troubleshooting bundle if you need help.',
    fixes: [],
  },
];
...
export function _showDiagnosis(panel, diagnosis, sourceText) {
  ...
  copyBtn.addEventListener('click', async (e) => {
    e.stopPropagation();
    const bundle = _diagnosisCopyBundle(
      task,
      diagnosis,
      sourceText,
      suggestionText,
    ...
    if (ok) {
      ...
      setTimeout(() => {
        if (copyBtn.isConnected) copyBtn.classList.remove('copied');
      }, 1200);
    }
  });
