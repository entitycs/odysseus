...
const _isRenderLang = (l) =>
  ['html', 'svg', 'xml'].includes((l || '').toLowerCase());
...
async function _getEmailAccountsCached() {
  const now = Date.now();
  if (_emailAccountsCache && now - _emailAccountsCacheAt < 30000)
...
async function _resolveComposeSendAccountId() {
  ...
  const activeAccount = accounts.find(
    (a) => String(a.id) === String(activeAccountId),
  );
  if (!activeAccount || _accountCanSend(activeAccount)) return activeAccountId;
  if (uiModule)
    uiModule.showToast(
      'Selected email account is receive-only; using your SMTP account.',
    );
  return null;
}
...
function _wireSwipeDismiss(el) {
  ...
  const endSwipe = () => {
    ...
    const shouldDismiss =
      dy > DISMISS_THRESHOLD || (dy > 20 && velocity > VELOCITY_THRESHOLD);
    if (shouldDismiss) {
      closePanel('down');
    } else {
...
function renderTabs() {
  ...
  let html = '';
  html +=
    '<button class="doc-tab-arrow doc-tab-arrow-left" id="doc-tab-left" title="Scroll left">&#x2039;</button>';
    ...
    const lic =
      doc.language && doc.language !== 'text'
        ? langIcon(doc.language, 12, {
            style:
              'opacity:0.65;flex-shrink:0;color:currentColor;margin-right:4px;',
          })
        : '';
  ...
  html += '</div>';
  html +=
    '<button class="doc-tab-arrow doc-tab-arrow-right" id="doc-tab-right" title="Scroll right">&#x203A;</button>';
  ...
  if (!tabBar._swipeWired) {
    tabBar._swipeWired = true;
    _wireSwipeDismiss(tabBar);
  }
  ...
  const _scrollTabIntoView = (tab, behavior = 'smooth') => {
    ...
    if (tabRight > visRight) {
      ...
      sa.scrollTo({
        left: Math.max(0, sa.scrollLeft + tabLeft - visLeft),
        behavior,
      });
    }
  };
  ...
  tabBar.querySelectorAll('.doc-tab').forEach((tab) => {
    tab.addEventListener('click', (e) => {
      ...
      if (
        ...
        e.target.closest('.doc-tab-play') ||
        e.target.closest('.doc-tab-menu-btn') ||
        e.target.closest('.doc-tab-version')
      )
        return;
  ...
  if (tabBar._closeHandler)
    tabBar.removeEventListener('click', tabBar._closeHandler);
  ...
  if (newBtn) {
    newBtn.addEventListener('click', async () => {
      let sessionId =
        docs.get(activeDocId)?.sessionId ||
        _lastSessionId ||
  ...
  requestAnimationFrame(() => {
    const at = document
      .getElementById('doc-tab-scroll')
      ?.querySelector('.doc-tab.active');
...
function showEmptyState() {
  ...
  if (textarea)
    textarea.placeholder = 'Start typing or paste text to create a document...';
...
function _dismissDocKb() {
  ...
  const el =
    ae && (ae.tagName === 'INPUT' || ae.tagName === 'TEXTAREA') ? ae : ta;
  if (!el) return;
  try {
...
async function _downloadFilledPdf() {
  ...
  try {
    ...
    a.download =
      ...
      _slug(docs.get(activeDocId)?.title) + '_annotated.pdf';
...
async function _openExportPdfModal() {
  ...
  overlay.style.cssText =
    'pointer-events:auto;background:rgba(0,0,0,0.5);backdrop-filter:blur(4px);';
  overlay.innerHTML = `
  ...
  try {
    const res = await fetch(
      `${API_BASE}/api/document/${activeDocId}/export-pdf/preview`,
      { method: 'POST' },
    );
    if (!res.ok) {
    ...
    jumpBar.style.cssText =
      'position:sticky;top:0;background:var(--panel);padding:6px 0;margin-bottom:8px;border-bottom:1px solid var(--border);display:flex;gap:6px;flex-wrap:wrap;align-items:center;font-size:0.72rem;z-index:1;';
    jumpBar.innerHTML =
      '<span style="opacity:0.6;margin-right:4px;">Jump to:</span>';
    const pageAnchors = {};
    ...
    for (const p of pages) {
      ...
      a.addEventListener('click', () =>
        pageAnchors[p]?.scrollIntoView({ behavior: 'smooth', block: 'start' }),
      );
    ...
    for (const p of pages) {
      ...
      for (const f of byPage.get(p)) {
        ...
        const isSignature =
          f.type === 'signature' ||
          /sign(?:ed|ature)/i.test((f.name || '') + ' ' + (f.label || ''));
        const isDate =
          f.type === 'text' &&
          /\b(date|dated)\b/i.test(`${f.name || ''} ${f.label || ''}`);
        let input;
        if (isSignature) {
          ...
          thumb.style.cssText =
            'max-height:32px;max-width:140px;object-fit:contain;border:1px solid var(--border);border-radius:3px;background:#fff;display:none;';
          const clearBtn = document.createElement('button');
          clearBtn.textContent = '×';
          clearBtn.title = 'Remove signature from this field';
          clearBtn.className = 'confirm-btn confirm-btn-secondary';
          clearBtn.style.cssText =
            'padding:0 8px;font-size:0.85rem;line-height:1;display:none;';
          const apply = (sig) => {
          ...
          const last =
            signatureModule.getLastUsed && signatureModule.getLastUsed();
          if (last) apply(last);
          input = wrap;
        } else if (isDate) {
          const wrap = document.createElement('div');
    ...
    downloadBtn.addEventListener('click', async () => {
      ...
      try {
        const r = await fetch(
          `${API_BASE}/api/document/${activeDocId}/export-pdf`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function _annotationLine(a) {
  ...
  const escaped =
    a.value === '' || a.value == null
      ? '_(empty)_'
      : _escapeAnnotationValue(a.value);
...
function _setPdfDropMode(mode) {
  ...
  for (const id of [
    'doc-pdf-add-text-btn',
    'doc-pdf-add-check-btn',
    'doc-pdf-add-sign-btn',
  ]) {
    const b = document.getElementById(id);
    if (!b) continue;
    const want =
      (mode === 'text' && id === 'doc-pdf-add-text-btn') ||
...
function _wirePdfPaneProximity(pane) {
  ...
  pane.addEventListener('mouseleave', () => {
    const refs = _pdfPaneAnnotationsByDoc.get(activeDocId) || [];
    for (const ref of refs)
      ref._setHandlesVisible && ref._setHandlesVisible(false);
  });
}
...
async function _renderPdfPane() {
  ...
  pane.innerHTML =
    '<div style="color:#bbb;font-size:13px;text-align:center;padding:40px;">Loading PDF…</div>';
  ...
  for (const page of data.pages) {
    ...
    for (const f of page.fields) {
      ...
      if (isSig) {
        ...
        const initialSigId =
          typeof f.value === 'string' && f.value.startsWith('signature:')
        ...
        const renderSigUI = async (sigId) => {
          ...
          if (sigId) {
            ...
            try {
              ...
              el.appendChild(img);
              el.style.border =
                '1px solid color-mix(in srgb, var(--accent, var(--red)) 45%, transparent)';
            ...
            el.style.border =
              '1px dashed color-mix(in srgb, var(--accent, var(--red)) 65%, transparent)';
            el.style.background =
              'color-mix(in srgb, var(--accent, var(--red)) 10%, transparent)';
            const span = document.createElement('span');
            span.style.cssText =
              'color:var(--accent, var(--red));font-size:11px;';
        ...
        el.style.cssText =
          baseStyle +
          'border:1px solid color-mix(in srgb, var(--accent, var(--red)) 45%, transparent);background:rgba(255,255,255,0.85);font-size:11px;padding:0 2px;';
      } else {
        el = document.createElement('input');
        el.type = 'text';
        ...
        el.style.cssText =
          baseStyle +
          `border:1px solid color-mix(in srgb, var(--accent, var(--red)) 45%, transparent);background:rgba(255,255,255,0.85);font-size:${fontPx}px;padding:0 2px;`;
      }
      if (!isSig) {
        el.dataset.fieldName = f.name;
      ...
      const isDate =
        f.type === 'text' && /\b(date|dated)\b/i.test(`${f.name} ${f.label}`);
      if (isDate) {
        const today = document.createElement('button');
        today.type = 'button';
        today.textContent = 'Today';
    ...
    pageWrap.addEventListener('click', (ev) => {
      ...
      const x = Math.max(
        0,
        Math.min(100 - size.w, centered ? xPct - size.w / 2 : xPct),
      );
      const y = Math.max(
        0,
        Math.min(100 - size.h, centered ? yPct - size.h / 2 : yPct),
      );
      const ann = {
        id: _newAnnotationId(),
        page: page.page,
        ...
        lineHeight:
          _pdfDropMode === 'text'
            ? _pdfLastLineHeight.get(docId) || 1.3
            : undefined,
      };
...
function _buildAnnotation(pageWrap, ann) {
  ...
  if (kind === 'check') {
    ...
    input.textContent =
      ann.value && ann.value.startsWith('signature:') ? '' : 'Sign here';
    input.dataset.signatureId =
      ann.value && ann.value.startsWith('signature:')
        ? ann.value.slice(10)
        : '';
  } else {
  ...
  resize.innerHTML =
    '<svg width="14" height="14" viewBox="0 0 10 10" style="display:block;margin:auto;height:100%;"><path d="M2 8 L8 2 M5 8 L8 5" stroke="currentColor" stroke-width="1.4" fill="none" stroke-linecap="round"/></svg>';
    ...
    const _autoGrow = () => {
      ...
      if (!_mirror) {
        ...
        _mirror.style.cssText =
          'position:absolute;visibility:hidden;white-space:pre;font-family:inherit;padding:1px 4px;left:-9999px;top:-9999px;';
  ...
  if (kind === 'text') {
    ...
    const _applyLh = (v, fromSlider) => {
      ...
      if (fromSlider) valInput.value = v.toFixed(2);
        ...
        slider.value = String(
          Math.max(parseFloat(slider.min), Math.min(parseFloat(slider.max), v)),
        );
      _pdfLastLineHeight.set(activeDocId, v);
      _schedulePdfPaneSave();
    };
    slider.addEventListener('input', () =>
      _applyLh(parseFloat(slider.value), true),
    );
    valInput.addEventListener('input', () =>
      _applyLh(parseFloat(valInput.value), false),
    );
    // Reject invalid typed values on blur — snap back to the live ref value.
    valInput.addEventListener('blur', () => {
      const v = parseFloat(valInput.value);
      if (!Number.isFinite(v))
        valInput.value = (ref.lineHeight || 1.3).toFixed(2);
    ...
    menuBtn?.addEventListener('click', (e) => {
      ...
      popover.style.display =
        popover.style.display === 'flex' ? 'none' : 'flex';
    });
...
async function _aiFillAnnotations() {
  ...
  try {
    const res = await fetch(
      `${API_BASE}/api/document/${docId}/ai-fill-annotations`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (!proposed.length) {
      ...
      if (uiModule && uiModule.showToast)
        uiModule.showToast('AI found nothing to fill');
      return;
    }
...
function _syncHeaderBarVisibility() {
  ...
  // resurrect it here.
  if (docs.get(activeDocId)?.language === 'email') {
...
function _syncHeaderActions() {
  ...
  if (_copyBtn) {
    ...
    if (_replyable && _copyBtn.dataset.mode !== 'reply') {
      ...
      _copyBtn.innerHTML =
        '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>Attach';
  ...
  const lang = (
    document.getElementById('doc-language-select')?.value || ''
  ).toLowerCase();
  ...
  if (mdToggle)
    mdToggle.style.display = lang === 'markdown' ? 'inline-flex' : 'none';
  ...
  if (renderToggle) {
    ...
    if (runBtn) {
      ...
      if (lang === 'csv') {
        icon =
          '<svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" stroke="none"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>';
        title = 'Table view';
  ...
  if (lang === 'markdown') {
    ...
    if (mdToggle) {
      mdToggle
        .querySelector('[data-mdview="edit"]')
        ?.classList.toggle('active', !_mdActive);
      mdToggle
        .querySelector('[data-mdview="preview"]')
        ?.classList.toggle('active', _mdActive);
    }
  } else if (lang === 'csv') {
    show = true;
    actionBtn.innerHTML = _csvActive
      ? _penIco
    ...
    if (renderToggle) {
      renderToggle
        .querySelector('[data-renderview="code"]')
        ?.classList.toggle('active', !_htmlActive);
      renderToggle
        .querySelector('[data-renderview="run"]')
        ?.classList.toggle('active', _htmlActive);
    }
  } else if (canRun) {
    show = true;
...
function _parseEmailHeader(content) {
  ...
  for (const line of header.split('\n')) {
    const m = line.match(
      /^(To|Cc|Bcc|Subject|In-Reply-To|References|X-Source-UID|X-Source-Folder|X-Attachments):\s*(.*)$/i,
    );
    if (m) {
...
function _emailBodyToHtml(text) {
  ...
  if (
    /<\/?(b|i|u|s|strong|em|del|strike|a|p|div|br|ul|ol|li|h[1-3]|blockquote|span|code|pre)\b[^>]*>/i.test(
      t,
    )
  )
    return t;
...
function _wireEmailRichbody(rich) {
  ...
  const syncActive = () => {
    ...
    if (hBtn)
      hBtn.classList.toggle(
        'is-active',
        cur === 'h1' || cur === 'h2' || cur === 'h3',
      );
    try {
      const inList =
...
function _captureEmailBodyFocusState() {
  ...
  if (rich && (active === rich || rich.contains(active))) {
    ...
    return {
      type: 'rich',
      range:
        range && rich.contains(range.commonAncestorContainer)
          ? range.cloneRange()
          : null,
    };
  }
...
function _syncEmailHeaderSummary() {
  const to =
    document.getElementById('doc-email-to')?.value?.trim() || 'No recipient';
  const subject =
    document.getElementById('doc-email-subject')?.value?.trim() || 'No subject';
...
function _maybeAutoCollapseEmailHeader() {
  ...
  if (_shouldAutoCollapseEmailHeader())
    _setEmailHeaderCollapsed(true, { manual: false });
}
...
function _showEmailFields(doc) {
  ...
  document.querySelectorAll('.md-toolbar-email-hide').forEach((el) => {
    el.style.display = 'none';
  });
  // Show email-only toolbar items (AI reply button).
  document.querySelectorAll('.md-toolbar-email-only').forEach((el) => {
  ...
  _setEmailHeaderCollapsed(!!(doc && doc._emailHeaderCollapsed), {
    manual: false,
  });
      ...
      for (const att of fields.attachments) {
        ...
        if (isPdf) {
          ...
          chip.addEventListener('click', () =>
            _withSpinner(chip, async () => {
              try {
                const folderQs = encodeURIComponent(
                  fields.sourceFolder || 'INBOX',
                );
                const res = await fetch(
                  `${API_BASE}/api/email/attachment-as-doc/${encodeURIComponent(fields.sourceUid)}/${att.index}?folder=${folderQs}`,
                  { method: 'POST' },
                );
                const data = await res.json();
                if (data.doc_id) {
                  ...
                  window.open(
                    `${API_BASE}/api/email/attachment/${encodeURIComponent(fields.sourceUid)}/${att.index}?folder=${folderQs}`,
                    '_blank',
                  );
                }
              } catch (e) {
          ...
          chip.addEventListener('click', () =>
            _withSpinner(chip, async () => {
              try {
                ...
                if (uiModule)
                  uiModule.showError('Download failed: ' + e.message);
              }
            }),
          );
          attDiv.appendChild(chip);
...
async function _uploadMarkdownImages(files) {
  ...
  if (_activeDocLanguage() !== 'markdown') {
    if (uiModule)
      uiModule.showError(
        'Switch the document to markdown before inserting images',
      );
    return;
  }
  ...
  try {
    ...
    if (uiModule)
      uiModule.showToast(
        images.length === 1 ? 'Image inserted' : 'Images inserted',
      );
  } catch (err) {
...
function _renderComposeAttachments() {
  ...
  for (const att of atts) {
    ...
    chip.innerHTML = `
      ...
      `;
    chip
      .querySelector('.compose-chip-remove')
      .addEventListener('click', async (e) => {
        e.stopPropagation();
        try {
          await fetch(
            `${API_BASE}/api/email/compose-upload/${encodeURIComponent(att.token)}`,
            { method: 'DELETE' },
          );
        } catch (_) {}
        const d = docs.get(activeDocId);
        if (d)
          d._composeAtts = d._composeAtts.filter((a) => a.token !== att.token);
        _renderComposeAttachments();
      });
...
async function _searchContacts(input, sugg) {
  ...
  try {
    const res = await fetch(
      `${API_BASE}/api/contacts/search?q=${encodeURIComponent(fragment)}`,
    );
    const data = await res.json();
    ...
    for (const c of data.results) {
      for (const em of c.emails || []) {
        ...
        item.addEventListener('mousedown', (e) => {
          e.preventDefault();
          _commitRecipient(input, sugg, em);
        });
...
function _wireRecipientAutocomplete(inputId, suggId) {
  ...
  input.addEventListener('keydown', (e) => {
    ...
    const active = open
      ? sugg.querySelector('.contact-suggestion.active')
      : null;
    ...
    if (open && e.key === 'ArrowDown') {
      ...
      e.key === ',' ||
      (e.key === ' ' && input.value.trim().endsWith(','))
    ) {
...
function _hideEmailFields() {
  ...
  document
    .getElementById('doc-editor-textarea')
    ?.classList.remove('email-mode');
  document.getElementById('doc-editor-code')?.classList.remove('email-mode');
  document
    .getElementById('doc-editor-highlight')
    ?.classList.remove('email-mode');
}

const _ATTACH_RE =
  /\b(attach(ed|ment|ments|ing)?|enclosed|enclosing|PFA|find attached|see attached|ci-joint|en pi[eè]ce jointe|ajout[eé]|joint|jointe|anbei|im Anhang|beigef[uü]gt|添付|fichier joint)\b/i;
...
function _confirmMissingAttachment() {
  return new Promise((resolve) => {
    ...
    overlay
      .querySelector('#att-warn-cancel')
      .addEventListener('click', () => cleanup(false));
    overlay
      .querySelector('#att-warn-send')
...
async function _sendEmail() {
  ...
  const inReplyTo = document
    .getElementById('doc-email-in-reply-to')
    ?.value?.trim();
  const references = document
    .getElementById('doc-email-references')
    ?.value?.trim();
  const sourceUid = document
    .getElementById('doc-email-source-uid')
    ?.value?.trim();
  const sourceFolder =
    document.getElementById('doc-email-source-folder')?.value?.trim() ||
    'INBOX';
  ...
  const textarea = document.getElementById('doc-editor-textarea');
  const body = (
    _rich ? _rich.innerText || _rich.textContent || '' : textarea?.value || ''
  ).trim();
  ...
  if (btn) {
    ...
    sendSpinner.element.style.cssText =
      'display:inline-block;vertical-align:-2px;margin-right:6px;width:14px;height:14px;';
    btn.innerHTML = '';
  ...
  try {
    ...
    if (data.success) {
      ...
      const _contactPieces = [to, cc, bcc]
        .join(',')
        .split(/[,;]/)
        .map((s) => s.trim())
        .filter(Boolean);
      ...
      if (sourceUid) {
        fetch(
          `${API_BASE}/api/email/mark-answered/${sourceUid}?folder=${encodeURIComponent(sourceFolder)}`,
          { method: 'POST' },
        ).catch(() => {});
        // Tell the inbox to refresh so the answered state shows
        window.dispatchEvent(
          new CustomEvent('email-answered', { detail: { uid: sourceUid } }),
        );
      }
      ...
      if (sendDocId) {
        fetch(`${API_BASE}/api/document/${sendDocId}`, {
          method: 'DELETE',
        }).catch(() => {});
        ...
        if (wasActiveSentDoc) {
          activeDocId = null;
          const nextId = _visibleDocIdsForCurrentSession().find((id) =>
            docs.has(id),
          );
...
async function _saveDraft() {
  ...
  try {
    ...
    if (uiModule)
      uiModule.showError(
        timedOut ? 'Saving draft timed out' : 'Failed to save draft',
      );
  } finally {
...
function _detachActiveEmailForBackground(docId) {
  ...
  if (wasActive) {
    const remaining = visibleBefore.filter(
      (id) => id !== docId && docs.has(id),
    );
...
function _showDocAiReplyChoice(btn) {
  ...
  const top =
    spaceBelow >= estHeight || spaceBelow >= spaceAbove
      ? Math.max(
          8,
...
async function _aiReply(opts = {}) {
  ...
  const to = document.getElementById('doc-email-to')?.value?.trim() || '';
  const subject =
    document.getElementById('doc-email-subject')?.value?.trim() || '';
  const textarea = document.getElementById('doc-editor-textarea');
  if (!textarea) return;
  const currentBody = textarea.value || '';
  const inReplyTo =
    document.getElementById('doc-email-in-reply-to')?.value?.trim() || '';
  const sourceUid =
    document.getElementById('doc-email-source-uid')?.value?.trim() || '';
  const sourceFolder =
    document.getElementById('doc-email-source-folder')?.value?.trim() ||
    'INBOX';
  ...
  const shouldUseFastAiReply = () => {
    ...
    if (
      /\b(attach(?:ed|ment)?|pdf|document|contract|invoice|receipt|quote|estimate|proposal|question|questions|details|schedule|booking|reservation|meeting|calendar|availability|confirm|confirmation|review|sign|signature)\b/.test(
        text,
      )
    ) {
      return false;
  ...
  if (btn) {
    btn.disabled = true;
    btn.innerHTML =
      '<svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor" style="vertical-align:-1px;margin-right:3px"><path d="M12 0L14.59 8.41L23 12L14.59 15.59L12 24L9.41 15.59L1 12L9.41 8.41Z"/></svg>Drafting...';
  }
  ...
  try {
    ...
    const bodyForApi =
      currentBody ||
      (noteHint
        ? '(no prior email — compose a new message based on the To, Subject, and user instructions)'
        : currentBody);
    const fastFlag =
      mode === 'ai-reply-fast'
        ? true
    ...
    if (data.success && data.reply) {
      ...
      if (uiModule)
        uiModule.showToast(`AI draft inserted (${data.model_used || 'AI'})`);
    } else {
      if (uiModule)
        uiModule.showError(data.error || 'Failed to generate reply');
    }
  } catch (e) {
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML =
        '<svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" style="color:var(--accent, var(--red));flex-shrink:0;position:relative;top:-1px;"><path d="M12 0L14.59 8.41L23 12L14.59 15.59L12 24L9.41 15.59L1 12L9.41 8.41Z"/></svg><span style="font-size:11px;margin-left:4px;">Reply</span>';
    }
  }
}
...
async function _scheduleSend(anchorEl = null) {
  ...
  const anchor =
    anchorEl ||
    document.getElementById('doc-email-send-caret') ||
    document.getElementById('doc-email-send-btn');
  ...
  overlay
    .querySelector('#sched-confirm')
    .addEventListener('click', async () => {
      const localDt = dtInput.value;
      if (!localDt) {
        if (uiModule) uiModule.showError('Please pick a time');
        return;
      }
      ...
      try {
        ...
        if (data.success) {
          if (uiModule)
            uiModule.showToast(
              `Scheduled for ${new Date(localDt).toLocaleString()}`,
            );
          cleanup();
          // Close the document
          _closeWithoutDeleting(true);
...
async function _markUnreadAndClose() {
  ...
  if (sourceUid) {
    try {
      await fetch(
        `${API_BASE}/api/email/mark-unread/${sourceUid}?folder=${encodeURIComponent(sourceFolder)}`,
        { method: 'POST' },
      );
    } catch (e) {
...
function switchToDoc(docId) {
  ...
  if (badge) {
    const _v = doc.version || 1;
    badge.textContent = `v${_v}`;
    badge.style.display = _v > 1 ? '' : 'none';
  }
  {
    const _v = doc.version || 1;
    const _dbtn = document.getElementById('doc-diff-toggle-btn');
    if (_dbtn) _dbtn.style.display = _v > 1 ? '' : 'none';
  }
  syncHighlighting();
  ...
  if (isPdf && !doc._ocrTriggered) {
    ...
    (async () => {
      try {
        const r = await fetch(
          `${API_BASE}/api/document/${docId}/extract-pdf-text`,
        ...
        if (j && j.extracted) {
          ...
          const dr = await fetch(`${API_BASE}/api/document/${docId}`, {
            credentials: 'same-origin',
          });
          if (dr.ok) {
  ...
  if (isEmail) {
    ...
    const wantsMarkdownPreview =
      (doc.language || 'markdown') === 'markdown' &&
      doc._markdownPreviewActive === true;
...
function _detachDocFromSession(docId, { toast = false } = {}) {
  ...
  if (hasContent) {
    fetch(`${API_BASE}/api/document/${docId}`, {
      ...
      .then(() => {
        if (toast && uiModule)
          uiModule.showToast('Document unlinked from session');
      })
      .catch(() => {});
  } else {
...
function saveCurrentToMap() {
  ...
  if (doc.language === 'email' && textarea) {
    ...
    const references =
      document.getElementById('doc-email-references')?.value || '';
    const sourceUid =
      document.getElementById('doc-email-source-uid')?.value || '';
    const sourceFolder =
      document.getElementById('doc-email-source-folder')?.value || '';
    ...
    const _rich = document.getElementById('doc-email-richbody');
    const _emailBody =
      _rich && _rich.style.display !== 'none'
        ? _rich.innerHTML
        : textarea.value;
...
export function openPanel() {
  ...
  // "exit fullscreen").
  divider.innerHTML =
    '<button type="button" class="doc-divider-collapse" title="Collapse panel" data-mode="collapse"><span>›</span></button>' +
  ...
  if (_divHide) {
    ...
    _divHide.addEventListener('click', (e) => {
      e.stopPropagation();
      closePanel('down');
    });
  }
  ...
  // edit↔preview) — both view toggles live in the same place.
  {
    ...
    if (_footer)
      _footer.addEventListener('pointerdown', (e) => {
  ...
  requestAnimationFrame(() => {
    pane.style.transition =
      'transform 0.15s cubic-bezier(0.22,1,0.36,1), opacity 0.12s ease-out';
  ...
  if (_divCollapse) {
    ...
    _divCollapse.addEventListener('pointermove', (ev) => {
      ...
      const pct = Math.max(
        6,
        Math.min(94, ((ev.clientY - rect.top) / rect.height) * 100),
      );
    ...
    const _endDrag = () => {
      if (_pid !== null) {
        try {
          _divCollapse.releasePointerCapture?.(_pid);
        } catch {}
        _pid = null;
      }
    };
  ...
  document
    .getElementById('doc-mobile-grabber')
    ?.addEventListener('click', () => closePanel('down'));

  // Wire up events
  document
    .getElementById('doc-close-btn')
    ?.addEventListener('click', () => closePanel('down'));
  document
    .getElementById('doc-footer-close-btn')
    ?.addEventListener('click', () => {
      if (activeDocId) closeTab(activeDocId);
    });
  document
    .getElementById('doc-import-btn')
    ?.addEventListener('click', () => openLibrary());
  document
    .getElementById('doc-footer-copy-btn')
    ?.addEventListener('click', (e) => {
      if (e.currentTarget.dataset.mode === 'reply') {
        if (activeDocId) _sendSignedReply(activeDocId);
      } else copyDocument();
    });
  document
    .getElementById('doc-footer-export-btn')
    ?.addEventListener('click', (e) =>
      showExportMenu(null, e.currentTarget.getBoundingClientRect()),
    );
  ...
  document.getElementById('doc-mobile-close')?.addEventListener('click', () => {
    if (activeDocId) closeTab(activeDocId);
  });
  document
    .getElementById('doc-mobile-copy')
    ?.addEventListener('click', () => copyDocument());
  // Save, copy, run, export, delete, preview toggles are now in per-tab context menu
  document
    .getElementById('doc-version-badge')
    .addEventListener('click', toggleVersionHistory);
  document
    .getElementById('doc-version-close')
    .addEventListener('click', _closeVersionPanel);
  ...
  const _syncLangIcon = () => {
    ...
    if (iconEl)
      iconEl.innerHTML = v ? langIcon(v, 14, { style: 'opacity:0.75;' }) : '';
  };
  ...
  (function _initLangPicker() {
    ...
    _syncLangPicker = () => {
      const v = ls.value || '';
      const sel =
        Array.from(ls.options).find((o) => o.value === v) || ls.options[0];
      const ic = v
  ...
  document
    .getElementById('doc-language-select')
    .addEventListener('change', () => {
      ...
      if (mdToolbar) {
        ...
        mdToolbar.style.display = '';
        if (mdToolbar._syncOverflow)
          requestAnimationFrame(mdToolbar._syncOverflow);
      }
  ...
  if (emojiSlot && !emojiSlot.querySelector('.emoji-picker-btn')) {
    ...
    emojiSlot.appendChild(
      emojiPicker.createEmojiButton(
        () =>
          _emailRichbodyActive() ||
          document.getElementById('doc-editor-textarea'),
      ),
    );
  }

  document
    .getElementById('doc-email-send-btn')
    ?.addEventListener('click', () => {
      ...
      document
        .getElementById('doc-email-send-caret')
  ...
  document
    .getElementById('doc-email-draft-btn')
    ?.addEventListener('click', () => {
      document.getElementById('doc-email-more-menu').style.display = 'none';
      _saveDraft();
    });
  document
    .getElementById('doc-email-discard-btn')
    ?.addEventListener('click', _discardEmail);
  document
    .getElementById('doc-email-unread-btn')
    ?.addEventListener('click', () => {
      document.getElementById('doc-email-more-menu').style.display = 'none';
      _markUnreadAndClose();
    });
  document
    .getElementById('doc-email-schedule-btn')
    ?.addEventListener('click', (e) => {
      const anchor =
        document.getElementById('doc-email-send-caret') || e.currentTarget;
      document.getElementById('doc-email-more-menu').style.display = 'none';
      _scheduleSend(anchor);
    });
  document
    .getElementById('doc-email-ai-reply-btn')
  ...
  if (collapseBtn && !collapseBtn._emailCollapseWired) {
    ...
    collapseBtn.addEventListener('pointerdown', (e) => {
      ...
      const header = document.getElementById('doc-email-header');
      const nextCollapsed = !header?.classList.contains(
        'doc-email-header-collapsed',
      );
  ...
  [
    ...
    document
      .getElementById(id)
      ?.addEventListener('input', _syncEmailHeaderSummary);
    document
      .getElementById(id)
  ...
  document
    .getElementById('doc-email-richbody')
    ?.addEventListener('focus', _maybeAutoCollapseEmailHeader);
  if (window.visualViewport && !window._docEmailViewportCollapseBound) {
    window._docEmailViewportCollapseBound = true;
    window.visualViewport.addEventListener(
      'resize',
      _maybeAutoCollapseEmailHeader,
    );
  }
  ...
  document.addEventListener('click', (e) => {
    ...
    if (
      menu &&
      !e.target.closest('#doc-email-send-caret, #doc-email-more-menu')
    ) {
      menu.style.display = 'none';
      document
        .getElementById('doc-email-send-caret')
  ...
  // Attachments
  document
    .getElementById('doc-email-attach-btn')
    ?.addEventListener('click', () => {
      document.getElementById('doc-email-file-input')?.click();
    });
  document
    .getElementById('md-toolbar-attach-btn')
    ?.addEventListener('click', () => {
  ...
  document
    .getElementById('doc-email-file-input')
    ?.addEventListener('change', _handleAttachUpload);
  document
    .getElementById('doc-md-image-input')
    ?.addEventListener('change', _handleMarkdownImageUpload);

  // Cc/Bcc toggle
  document
    .getElementById('doc-email-show-cc')
    ?.addEventListener('click', () => {
      _setEmailHeaderCollapsed(false, { manual: false });
      const ccRow = document.getElementById('doc-email-cc-row');
      const bccRow = document.getElementById('doc-email-bcc-row');
  ...
  document
    .getElementById('doc-header-preview-btn')
    .addEventListener('click', () => {
      const lang = (
        document.getElementById('doc-language-select')?.value || ''
      ).toLowerCase();
  ...
  document
    .getElementById('doc-md-view-toggle')
    ?.addEventListener('click', (e) => {
  ...
  document
    .getElementById('doc-render-view-toggle')
    ?.addEventListener('click', (e) => {
      const opt = e.target.closest('.md-view-opt');
      if (!opt) return;
  ...
  let _fontIdx = parseInt(
    localStorage.getItem('odysseus-doc-fontsize') || '0',
    10,
  );
  ...
  function _applyDocFont() {
    ...
    [editorWrap, richEmailBody].filter(Boolean).forEach((el) => {
      ...
      if (_fontSizes[_fontIdx] !== 's')
        el.classList.add('doc-font-' + _fontSizes[_fontIdx]);
    });
    if (fontBtn) {
      ...
      if (svg) {
        const sz = _iconSizes[_fontIdx];
        svg.setAttribute('width', sz);
        svg.setAttribute('height', sz);
      }
      // Show only the active size letter (just S, or just M, or just L).
      fontBtn
        .querySelectorAll('.doc-fontsize-levels [data-sz]')
        .forEach((el) => {
  ...
  if (docUndoBtn)
    docUndoBtn.addEventListener('click', async () => {
      const pdfPane = document.getElementById('doc-pdf-view');
  ...
  if (diffToggleBtn)
    diffToggleBtn.addEventListener('click', async () => {
      if (_diffModeActive) {
        exitDiffMode(true);
        return;
      }
      ...
      try {
        const res = await fetch(
          `${API_BASE}/api/document/${activeDocId}/versions`,
        );
  ...
  document
    .getElementById('doc-export-pdf-btn')
    ?.addEventListener('click', _downloadFilledPdf);
  ...
  // it off explicitly.
  document
    .getElementById('doc-pdf-add-text-btn')
    ?.addEventListener('click', () =>
      _setPdfDropMode(_pdfDropMode === 'text' ? null : 'text'),
    );
  document
    .getElementById('doc-pdf-add-check-btn')
    ?.addEventListener('click', () =>
      _setPdfDropMode(_pdfDropMode === 'check' ? null : 'check'),
    );
  document
    .getElementById('doc-pdf-add-sign-btn')
    ?.addEventListener('click', () =>
      _setPdfDropMode(_pdfDropMode === 'signature' ? null : 'signature'),
    );
  document
    .getElementById('doc-pdf-refresh-btn')
    ?.addEventListener('click', () => _renderPdfPane());
  ...
  if (ta && pre) {
    ta.addEventListener('input', () => {
      ...
      clearTimeout(_autoTitleDebounce);
      _autoTitleDebounce = setTimeout(
        () => autoTitleFromContent(ta.value),
        600,
      );
      clearTimeout(_autoSaveDebounce);
      _autoSaveDebounce = setTimeout(() => {
    ...
    ta.addEventListener('scroll', () => {
      ...
      if (_findMatches && _findMatches.length) {
        ...
        if (_q)
          renderFindRects(
            _findMatches.map((s) => [s, s + _q.length]),
            _findIdx,
          );
      }
    });
    ...
    function _doFind(dir, focusTextarea) {
      ...
      renderFindRects(
        _findMatches.map((s) => [s, s + q.length]),
        _findIdx,
      );
    ...
    document
      .getElementById('doc-find-close')
      ?.addEventListener('click', _closeFindBar);
    document
      .getElementById('doc-find-next')
      ?.addEventListener('click', () => _doFind('next', true));
    document
      .getElementById('doc-find-prev')
      ?.addEventListener('click', () => _doFind('prev', true));
    document
      .getElementById('doc-find-input')
      ?.addEventListener('input', () => _doFind('first', false));
    document
      .getElementById('doc-find-input')
...
function _promptLink(defaultText = '') {
  return new Promise((resolve) => {
    ...
    requestAnimationFrame(() => {
      (defaultText ? urlEl : textEl).focus();
    });
  });
}
  ...
  switch (action) {
    ...
    case 'codeblock': {
      ...
      for (let i = 0; i < linesAfter.length; i++) {
        if (/^```\s*$/.test(linesAfter[i].trimEnd())) {
          closeIdx = i;
          break;
        }
      }
      if (openIdx >= 0 && closeIdx >= 0) {
        ...
        const closeLineStart =
          ...
          linesAfter.slice(0, closeIdx).join('\n').length +
          (closeIdx > 0 ? 1 : 0);
        const closeLineEnd =
          closeLineStart +
          linesAfter[closeIdx].length +
          (closeIdx < linesAfter.length - 1 ? 1 : 0);
...
function _currentBlockTag(root) {
  ...
  while (node && node !== root) {
    ...
    if (tag && /^(h1|h2|h3|h4|h5|h6|p|div|pre|blockquote|li)$/.test(tag))
      return tag;
...
function _showMdDropdown(toggleBtn) {
  ...
  if (existing && existing.dataset.dd === kind && now - _mdDdOpenedAt < 400)
    return;
...
function initMdToolbar() {
  ...
  const overflowWrapper = document.getElementById(
  ...
  toolbar.addEventListener('mousedown', (e) => {
    if (
      e.target.closest(
        '[data-md], .md-dd-toggle, .emoji-picker-btn, .md-toolbar-attach-btn',
      )
    )
      e.preventDefault();
  });
  ...
  function updateScrollArrows() {
    ...
    scrollLeftBtn.style.display =
      overflowing && itemsWrap.scrollLeft > 1 ? 'flex' : 'none';
    scrollRightBtn.style.display =
      overflowing && itemsWrap.scrollLeft < maxScroll - 1 ? 'flex' : 'none';
  }
  scrollLeftBtn?.addEventListener('click', () =>
    itemsWrap.scrollTo({ left: 0, behavior: 'smooth' }),
  );
  scrollRightBtn?.addEventListener('click', () =>
    itemsWrap.scrollTo({ left: itemsWrap.scrollWidth, behavior: 'smooth' }),
  );
...
function _getDocRecent() {
  try {
    return JSON.parse(localStorage.getItem(_DOC_RECENTS_KEY) || '[]');
  } catch {
    return [];
  }
}
...
function initActionOverflow() {
  ...
  const allBtns = Array.from(
    actionsEl.querySelectorAll('.doc-collapsible-btn'),
  );
  let _menuOpen = false;

  function syncOverflow() {
    allBtns.forEach((b) => {
      b.classList.remove('doc-collapsed');
    });
    menu.innerHTML = '';
    ...
    if (overflow.length > 0) {
      ...
      overflow.forEach((btn) => {
        ...
        item.innerHTML =
          btn.innerHTML + '<span>' + (btn.title || '') + '</span>';
        item.addEventListener('click', (e) => {
...
function initDividerDrag(divider, pane, isRight) {
  ...
  document.addEventListener('mousemove', (e) => {
    if (!dragging) return;
    const width = isRight ? e.clientX : window.innerWidth - e.clientX;
    pane.style.width =
      Math.max(250, Math.min(width, window.innerWidth * 0.7)) + 'px';
    ...
    restoreFn: () => {
      ...
      if (id && docs.has(id)) {
        try {
          ...
          console.error('Restore doc failed:', e);
        }
      }
    },
  });
...
export function closePanel(direction) {
  ...
  if (pane) {
    ...
    if (direction === 'down') {
      ...
      transform =
        window.innerWidth <= 768 ? 'translateY(100%)' : 'translateY(30px)';
    } else {
      const fromLeft = pane.classList.contains('doc-left');
    ...
    pane.style.opacity = '0';
    if (divider) {
      divider.style.transition = 'opacity 0.1s ease-in';
...
export async function createDocument(sessionId) {
  ...
  try {
    ...
    const typed =
      wasEmpty && textarea && textarea.value.trim() ? textarea.value : '';
    switchToDoc(doc.id);
    if (typed) {
...
export async function replaceEmailReplyBody(docId, replyText) {
  ...
  if (
    ownText &&
    !/^(\[AI reply draft will appear here\]|Drafting AI reply)/i.test(ownText)
  ) {
...
export async function loadSessionDocs(sessionId, opts = {}) {
  ...
  const shouldRestoreOpen =
    localStorage.getItem(_docOpenKey(sessionId)) === '1';
  const shouldRestoreMinimized =
    localStorage.getItem(_docMinimizedKey(sessionId)) === '1';
...
function populateEditor(doc) {
  ...
  if (langSelect) langSelect.value = doc.language || 'markdown';
  if (badge) {
    const _v = doc.version_count || doc.version || 1;
    badge.textContent = `v${_v}`;
    badge.style.display = _v > 1 ? '' : 'none';
  }
  {
...
function _postProcessMarkdown(codeEl) {
  ...
  for (const node of textNodes) {
    ...
    if (
      node.parentElement !== codeEl &&
      node.parentElement.className &&
      /hljs-(string|link|code|section)/.test(node.parentElement.className)
    )
      continue;
...
function applyFindMarks(codeEl) {
  ...
  for (const node of nodes) {
    ...
    while (i < val.length) {
      ...
      if (hit < 0) {
        frag.appendChild(document.createTextNode(val.slice(i)));
        break;
      }
      if (hit > i) frag.appendChild(document.createTextNode(val.slice(i, hit)));
      const mark = document.createElement('mark');
      mark.className =
        'doc-find-mark' + (occurrence === currentIdx ? ' current' : '');
...
function _lineNumberMeasureEl(textarea) {
  const wrap =
    document.getElementById('doc-editor-wrap') ||
...
function syncGutterScroll() {
  ...
  const gutter = document.getElementById('doc-line-numbers');
  if (textarea && gutter) {
    _lineNumberContentEl(gutter).style.transform =
...
function attemptAutoDetect() {
  ...
  if (mdToolbar2)
    mdToolbar2.style.display = mapped === 'markdown' ? '' : 'none';
}
...
function updateSelectionState() {
  ...
  const overlapIdx = _selections.findIndex(
    (s) =>
...
function _measurePos(mirror, text, pos) {
  mirror.innerHTML = '';
  if (pos > 0)
    mirror.appendChild(document.createTextNode(text.substring(0, pos)));
  const marker = document.createElement('span');
  marker.textContent = '​';
...
function clearSelection() {
  ...
  if (wrap)
    wrap
      .querySelectorAll('.doc-selection-overlay')
...
function _saveSuggestionsToStorage() {
  if (!activeDocId) return;
  const data = _activeSuggestions.map((s) => ({
    id: s.id,
    find: s.find,
    replace: s.replace,
    reason: s.reason,
  ...
  if (data.length) {
    localStorage.setItem(
      'odysseus-suggestions-' + activeDocId,
      JSON.stringify(data),
    );
  } else {
...
function _showCurrentSuggestion() {
  ...
  function _positionCard(card) {
    ...
    const rawTop =
      textareaRect.top + paddingTop + linesBefore * lineH - textarea.scrollTop;
  ...
  // Wire buttons
  card
    .querySelector('.doc-suggestion-close')
    .addEventListener('click', clearAllSuggestions);
  card.querySelector('.doc-suggestion-prev').addEventListener('click', () => {
    ...
    _suggestionIndex =
      (_suggestionIndex - 1 + _suggestionTotal) % _suggestionTotal;
    _showCurrentSuggestion();
  });
  ...
  card
    .querySelector('.doc-suggestion-dismiss')
    .addEventListener('click', () => {
      _activeSuggestions.shift();
      _animateNext();
    });
  ...
  const _reposition = () => {
    if (card.isConnected) _positionCard(card);
  };
...
function _showInlineDiff(findText, replaceText) {
  ...
  let cPre = 0;
  while (
    cPre < findText.length &&
    cPre < replaceText.length &&
    findText[cPre] === replaceText[cPre]
  )
    cPre++;
  let cSuf = 0;
  while (
    cSuf < findText.length - cPre &&
    cSuf < replaceText.length - cPre &&
    findText[findText.length - 1 - cSuf] ===
      replaceText[replaceText.length - 1 - cSuf]
  )
    cSuf++;
...
function _resolveChunk(chunkId, accept) {
  ...
  const gutterActions = document.querySelector(
    `.diff-chunk-actions[data-chunk-id="${chunkId}"]`,
  );
...
function _applyResolvedChunksToTextarea() {
  const textarea = document.getElementById('doc-editor-textarea');
  if (!textarea) return;
  const entries = _computeLineDiff(
    _diffOldContent || '',
    _diffNewContent || '',
  );
  const result = [];
...
function runDocument() {
  ...
  if (!codeRunnerModule) {
    outputPanel.innerHTML =
      '<pre class="doc-run-error">Code runner not loaded</pre>';
    setTimeout(() => {
      if (outputPanel) outputPanel.style.display = 'none';
    }, 5000);
    return;
  ...
  outputPanel.innerHTML =
    '<pre class="doc-run-error">Unsupported language. Supported: bash, python, javascript, html</pre>';
  setTimeout(() => {
    if (outputPanel) outputPanel.style.display = 'none';
  }, 5000);
}
...
function _closeDocTabMenu() {
  if (_docTabMenu) {
    _docTabMenu.style.display = 'none';
  }
}
...
function showDocTabMenu(btnEl, docId) {
  ...
  if (!_docTabMenu) {
    ...
    _docTabMenu.style.cssText =
      'position:fixed;z-index:1000;min-width:0;width:max-content;padding:4px;background:var(--panel);border:1px solid var(--border);border-radius:8px;box-shadow:0 8px 24px rgba(0,0,0,0.3);backdrop-filter:blur(12px);font-size:12px;display:none;';
    document.body.appendChild(_docTabMenu);
    // Close on outside click
    document.addEventListener('click', (e) => {
      if (
        _docTabMenu &&
        !_docTabMenu.contains(e.target) &&
        !e.target.closest('.doc-tab-menu-btn')
      ) {
        _closeDocTabMenu();
      }
  ...
  const canRun =
    _isRenderLang(lang) ||
    ['javascript', 'js', 'python', 'py', 'bash', 'sh', 'shell', 'zsh'].includes(
      lang,
    );
  ...
  if (lang === 'markdown') {
    previewIcon = 'MD';
    previewLabel = _mdActive ? 'Edit' : 'Preview';
  } else if (lang === 'csv') {
    previewIcon = '⊞';
    previewLabel = _csvActive ? 'Edit' : 'Table View';
  } else if (_isRenderLang(lang)) {
    previewIcon = '▶';
    previewLabel = _htmlActive ? 'Edit' : 'Run / Preview';
  }
  ...
  _docTabMenu.querySelectorAll('.doc-tab-action').forEach((item) => {
    item.addEventListener('click', (e) => {
      ...
      switch (action) {
        ...
        case 'download': {
          const btn =
            document.getElementById('doc-fontsize-btn') ||
            document.getElementById('doc-language-select');
          showExportMenu(null, btn?.getBoundingClientRect());
          break;
        }
        case 'signed-reply':
          _sendSignedReply(docId);
          break;
        case 'close':
          closeTab(docId);
          break;
...
async function _sendSignedReply(docId) {
  ...
  try {
    const res = await fetch(
      `${API_BASE}/api/document/${encodeURIComponent(docId)}/prepare-signed-reply`,
      {
        method: 'POST',
        credentials: 'same-origin',
      },
  ...
  for (const [, d] of docs) {
    if (d.language === 'email' && d._draftForMessageId === mid && mid) {
      ...
      _renderComposeAttachments();
      if (uiModule)
        uiModule.showToast(`Added "${att.filename}" to the reply draft`);
      return;
    }
  }
  ...
  const content =
    headerLines.join('\n') +
    '\n---\n\nHi' +
    (reply.to_name ? ' ' + reply.to_name.split(/\s+/)[0] : '') +
  ...
  _renderComposeAttachments();
  if (uiModule)
    uiModule.showToast(`Reply draft ready — "${att.filename}" attached`);
}
...
export async function saveDocument({ silent = false } = {}) {
  ...
  try {
    ...
    if (uiModule && (!silent || now - _lastAutoSaveErrorAt > 10000)) {
      uiModule.showError(
        silent ? 'Autosave failed' : 'Failed to save document',
      );
  ...
  const fi = document.createElement('input');
    ...
    try {
      ...
      if (uiModule && uiModule.showError)
        uiModule.showError('Import failed: ' + (err.message || err));
    } finally {
      fi.value = '';
      fi.remove();
    }
...
function showExportMenu(e, anchorRect) {
  ...
  options.push({
    ...
    fn: () => _importFromDevice(),
    _divider: true,
  });
  if (isForm)
    options.push({ label: 'Filled PDF (.pdf)', fn: _downloadFilledPdf });
  options.push(
  ...
  options.forEach((opt) => {
    ...
    if (opt._divider) {
      ...
      sep.style.cssText =
        'height:1px;margin:3px 6px;background:color-mix(in srgb,var(--border) 60%,transparent);';
...
async function exportAsPdf() {
  ...
  if (lang === 'markdown' && markdownModule?.mdToHtml) {
    ...
    html =
      '<pre style="white-space:pre-wrap;font-size:11px;font-family:monospace;color:#000;background:#fff;">' +
      text.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;') +
      '</pre>';
  }
  const container = document.createElement('div');
  container.style.cssText =
    'padding:20px;font-family:sans-serif;font-size:12px;color:#000;background:#fff;line-height:1.6;';
...
async function deleteActiveDocument() {
  ...
  try {
    ...
    const tab = document.querySelector(
      `.doc-tab[data-doc-id="${activeDocId}"]`,
    );
    if (tab) tab.remove();
    docs.delete(activeDocId);
...
function _setMarkdownPreviewActive(active, { remember = true } = {}) {
  ...
  if (active) {
    ...
    const richEmailBody = document.getElementById('doc-email-richbody');
    if (
      !(isEmailDoc && richEmailBody && richEmailBody.style.display !== 'none')
    ) {
      wrap.style.display = '';
    }
  }
...
function syncTableToTextarea(preview, textarea) {
  ...
  if (ths.length)
    lines.push([...ths].map((th) => csvEscapeField(th.textContent)).join(','));
  // Body
  table.querySelectorAll('tbody tr').forEach((tr) => {
    const cells = [...tr.querySelectorAll('td')].map((td) =>
      csvEscapeField(td.textContent),
    );
...
function toggleCsvPreview() {
  ...
  if (preview.style.display === 'none') {
    ...
    if (rows.length === 0) {
      ...
      outputPanel.style.display = 'block';
      outputPanel.innerHTML =
        '<pre class="doc-run-error">No data — CSV is empty or unparseable.</pre>';
      return;
    } else {
      const esc = (s) =>
    ...
    mt.dataset.mode = isMd
      ...
      : isPdf
        ...
        : isCodeRenderable
          ? 'code'
          : '';
    if (showToolbar && mt._syncOverflow)
      requestAnimationFrame(mt._syncOverflow);
  }
  _syncHeaderActions();
}
...
function _animateDocEdit(textarea, newContent) {
  ...
  function tick() {
    if (phase === 'delete') {
      ...
      const current =
        oldContent.slice(0, prefixLen) +
        deletedText.slice(0, deletePos) +
        suffix;
...
export function handleDocUpdate(data) {
  ...
  if (
    streamingId &&
    streamingId.startsWith('_streaming_') &&
    docs.has(streamingId)
  ) {
  ...
  const docLang =
    data.language || (docs.has(docId) && docs.get(docId).language) || '';
  ...
  if (isEdit && textarea) {
    ...
    if (changedLines >= DIFF_MODE_THRESHOLD) {
      if (markdownPreviewWasVisible)
        _setMarkdownPreviewActive(false, { remember: false });
      enterDiffMode(oldContent, newContent);
    } else if (
      markdownPreviewWasVisible &&
...
function _buildDiffSummary(oldText, newText) {
  ...
  for (let i = 0; i < maxCheck; i++) {
    ...
    if (ol !== undefined && (nl === undefined || ol !== nl))
      removed.push(ol.trim());
    if (nl !== undefined && (ol === undefined || ol !== nl))
      added.push(nl.trim());
  }
  ...
  for (const line of removed.slice(0, 2)) {
    if (line)
      parts.push(
        `<span class="diff-del">${_escHtml(line.slice(0, 60))}</span>`,
      );
  }
  for (const line of added.slice(0, 2)) {
    if (line)
      parts.push(
        `<span class="diff-add">${_escHtml(line.slice(0, 60))}</span>`,
      );
  }
  ...
  try {
    ...
    for (let i = 0; i < versions.length; i++) {
      if (i < versions.length - 1) {
        diffs.push(
          _buildDiffSummary(versions[i + 1].content, versions[i].content),
        );
      } else {
        diffs.push('');
      }
    ...
    list.querySelectorAll('.doc-version-item').forEach((item) => {
      item.addEventListener('click', (e) => {
        ...
        // Toggle active state
        list
          .querySelectorAll('.doc-version-item.active')
    ...
    list.innerHTML =
      '<div style="padding:8px;opacity:0.5;">Failed to load versions</div>';
  }
}
...
async function previewVersion(num) {
  ...
  try {
    const res = await fetch(
      `${API_BASE}/api/document/${activeDocId}/version/${num}`,
    );
...
async function restoreVersion(num) {
  ...
  try {
    const res = await fetch(
      `${API_BASE}/api/document/${activeDocId}/restore/${num}`,
      {
        method: 'POST',
      },
    );
...
async function updateTitle(overrideDocId, overrideTitle) {
  ...
  if (!docId) return;
  const title =
    overrideTitle || document.getElementById('doc-title-input')?.value;
  if (!title) return;
  try {
...
function autoTitleFromContent(content, docId) {
  ...
  if (!doc || (doc.title && doc.title !== '' && doc.title !== 'Untitled'))
    return;
...
export function findEmailDocId(uid, folder) {
  ...
  for (const [id, d] of docs) {
    ...
    if (
      fields.sourceUid &&
      String(fields.sourceUid) === wantUid &&
