...
export function init() {
  ...
  window.addEventListener('hashchange', () => {
    ...
    if (/^(document|note|image|email|event|task|skill|research)-/.test(hashId))
      return;
    if (hashId && hashId !== currentSessionId) {
      const target = sessions_glob.find((s) => s.id === hashId && !s.archived);
      ...
      if (!targetId) {
        try {
          ...
          if (dc.endpoint_url && dc.model) {
            ...
            const emptyDefault = activeSessions.find(
              (s) => s.model === dc.model && s.message_count === 0,
            );
            if (emptyDefault) {
...
export async function materializePendingSession() {
  ...
  let res;
  try {
    res = await fetch(`${getApiBase()}/api/session`, {
      method: 'POST',
      body: fd,
    });
  } catch (e) {
  ...
  if (!res.ok) {
    uiModule.showError(
      `Session create failed (${res.status}) ${payload.detail || JSON.stringify(payload)}`,
    );
    return false;
  }
  ...
  pushState('#' + payload.id, { lastSessionId: payload.id });
