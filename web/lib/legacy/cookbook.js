...
import {
  pickRecipe,
  RECIPE_BACKENDS,
  RECIPE_DEFAULT_VARIANT,
  recipeCommands,
  recipesForBackend,
} from '$lib/legacy/cookbook-deps-recipes.js';
import {
  _clearDiagnosis,
  _diagnose,
  _runQuickCmd,
  _showDiagnosis,
  ERROR_PATTERNS,
} from '$lib/legacy/cookbook-diagnosis.js';
import {
  _cachedModelIds,
  _expandModelRow,
  _fitColors,
...
} from '$lib/legacy/cookbook-hwfit.js';
import {
  _buildDownloadCmd,
  _runModelDownload,
  _runPanelCmd,
  ...
  if (typeof window !== 'undefined' && !window._tagScrollGuardWired) {
    ...
    ['touchstart', 'touchmove'].forEach((evt) => {
      document.addEventListener(
        ...
        (e) => {
          const t = e.target;
          if (t && t.closest && t.closest('.doclib-lang-chips'))
            e.stopPropagation();
        },
        true,
      );
  ...
  if (typeof window !== 'undefined' && !window._cookbookServeEscBound) {
    ...
    document.addEventListener(
      ...
      (e) => {
        ...
        const expanded = modal.querySelector(
          '.memory-item.doclib-card-expanded',
        );
        ...
        const list =
          expanded.closest('.hwfit-cached-list') ||
          document.getElementById('hwfit-cached-list');
        if (list) {
...
export const _MODELDIR_CHECK_OFF =
  '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/></svg>';
...
export function _lastCacheHost() {
  return _lastCacheHostVal;
}
export function _setLastCacheHost(v) {
  _lastCacheHostVal = v;
}
...
function _setCookbookOpening(on) {
  ...
  const targets = [document.getElementById('rail-cookbook')].filter(Boolean);
  if (!on) {
...
function _isLocalEntry(s) {
  return (
    !s || !s.host || s.host === 'local' || s.host.toLowerCase() === 'localhost'
  );
}
...
export function _selectedServer() {
  ...
  if (_envState.remoteHost)
    return (
      _envState.servers.find((s) => s.host === _envState.remoteHost) || null
    );
  return null;
}
...
const _SERVER_AUTO_COLOR_VALUES = _SERVER_COLOR_CHOICES
  .slice(1)
  .map(([value]) => value);
...
function _autoServerColor(index) {
  ...
  for (let j = 0; j <= index; j++) {
    ...
    const picked =
      _SERVER_AUTO_COLOR_VALUES.find((c) => !used.has(c)) ||
      _SERVER_AUTO_COLOR_VALUES[j % _SERVER_AUTO_COLOR_VALUES.length] ||
      '';
    if (j === index) return picked;
    if (picked) used.add(picked);
  }
  return (
    _SERVER_AUTO_COLOR_VALUES[index % _SERVER_AUTO_COLOR_VALUES.length] || ''
  );
}
...
function _serverColorForValue(value) {
  ...
  const idx = Array.isArray(_envState.servers)
    ? _envState.servers.indexOf(s)
    : -1;
...
export function _syncServerSelectColors(root = document) {
  root
    .querySelectorAll(
      '#hwfit-server-select, #hwfit-dl-server, #hwfit-cache-server, #hwfit-deps-server',
    )
    .forEach(_applyServerSelectColor);
}
...
function _buildServerOpts(excludeLocal = false) {
  ...
  const _localColor = _localSrv
    ? _resolvedServerColor(_localSrv, _localIdx)
    : '';
...
function _getPort(hostOrTask) {
  ...
  if (typeof hostOrTask === 'object')
    return (
      hostOrTask.sshPort ||
      _getPort(hostOrTask.remoteServerKey || hostOrTask.remoteHost)
    );
  const selected =
    hostOrTask === _envState.remoteHost ? _selectedServer() : null;
...
export function _getPlatform(hostOrTask) {
  ...
  if (typeof hostOrTask === 'object')
    return (
      hostOrTask.platform ||
      _getPlatform(hostOrTask.remoteServerKey || hostOrTask.remoteHost)
    );
  const selected =
    hostOrTask === _envState.remoteHost ? _selectedServer() : null;
  const srv = selected || _serverByVal(hostOrTask);
...
export function _isMetal() {
  return ['metal', 'mps', 'apple'].includes(
    String(_hwfitCache?.system?.backend || '').toLowerCase(),
  );
}
...
function _detectModelOptimizations(modelName) {
  ...
  if (_isStepFunStepModel(modelName)) {
    ...
    opts.tips.push(
      'StepFun parser: step3p5 for native tool calls and reasoning tags',
    );
  }
  ...
  // _detectReasoningParser, no longer hardcoded here.
    ...
    n.includes('qwen3.5') ||
    (n.includes('qwen3-') &&
      (n.includes('a10b') || n.includes('a22b') || n.includes('a3b')))
  ) {
    opts.envVars.push(
    ...
    opts.flags.push('--enable-expert-parallel');
    opts.tips.push(
      'MoE optimizations: expert parallel + flashinfer MoE kernels',
    );
  }
  // Qwen3 MoE (non-3.5)
  else if (
  ...
  // checkbox in the serve form to render at all.
    ...
    if (/\bm3\b/.test(n)) {
      opts.kvCacheDtype = 'fp8';
      opts.tips.push(
        'MiniMax M3 defaults: fp8 KV cache, block-size 128, TRITON attention',
      );
    }
  }
  ...
  if (_reasoningParser) {
    ...
    opts.tips.push(
      `Reasoning parser (${_reasoningParser}): splits <think> tokens into a separate channel`,
    );
  }
  ...
  if (
    n.includes('qwen3-next') ||
    (n.includes('qwen3.5') && (n.includes('a10b') || n.includes('a22b')))
  ) {
    ...
    n.includes('glm-4.5') ||
    n.includes('glm4.5') ||
    n.includes('minimax-m1') ||
    n.includes('minimax_m1')
  ) {
  ...
  if (specDefault) {
    ...
    opts.flags.push(
      `--speculative-config '{"method":"${specDefault.method}","num_speculative_tokens":${specDefault.tokens}}'`,
    );
    opts.tips.push(
      `Speculative decoding (${specDefault.method}, ${specDefault.tokens} tokens): ~1.5-2x faster generation`,
    );
  }
...
export function _detectReasoningParser(modelName) {
  ...
  if (n.includes('deepseek') && (n.includes('r1') || n.includes('thinking')))
    return 'deepseek_r1';
  // Qwen3 / Qwen3.5 reasoning models. Qwen3-Coder + Qwen3-Instruct don't
  // emit <think> blocks, so skip the parser there.
  if (n.includes('qwen3') && !n.includes('coder') && !n.includes('instruct'))
    return 'qwen3';
  ...
  if (n.includes('granite') && (n.includes('reason') || n.includes('think')))
    return 'granite';
  // InternLM reasoning.
...
export function _detectBackend(model) {
  const _ollamaName = String(
    model?.repo_id || model?.name || model?.id || '',
  ).trim();
  const _ollamaMeta =
    `${model?.backend || ''} ${model?.endpoint_kind || ''} ${model?.provider || ''} ${model?.source || ''}`.toLowerCase();
  const _looksLikeOllamaTag =
  ...
  const _nm =
    `${model.repo_id || ''} ${model.path || ''} ${model.name || ''}`.toLowerCase();
  ...
  const isAwqLike =
    ...
    /\b(awq|gptq|fp8|fp4|nvfp4|mxfp4|nf4|int4|int8|w4a16|w8a8|w8a16)\b/i.test(
      _nm,
    );
  const hasGgufFile =
    Array.isArray(model.gguf_files) &&
    model.gguf_files.some(
      (f) => f && typeof f.rel_path === 'string' && /\.gguf$/i.test(f.rel_path),
    );
  const isGgufLike =
    model.is_gguf ||
    hasGgufFile ||
...
export function _buildEnvPrefix() {
  ...
  let envVars = [];
  if (_envState.hfToken)
    envVars.push('export HF_TOKEN=' + _shellQuote(_envState.hfToken));
...
function _buildEnvPrefixWindows() {
  ...
  if (_envState.env === 'venv' && _envState.envPath) {
    const p = _envState.envPath;
    const activate = p.endsWith('\\Scripts\\Activate.ps1')
      ? p
  ...
  if (_envState.hfToken)
    parts.push('$env:HF_TOKEN=' + _psQuote(_envState.hfToken));
...
function _venvRootFromPath(path) {
  ...
  if (!p) return '';
  p = p.replace(
    /\/bin\/(?:activate|python(?:3(?:\.\d+)?)?|vllm|pip(?:3)?)$/i,
    '',
  );
  return p;
}
...
function _venvLooksWrongForPlatform(path, platform) {
  ...
  if (
    ...
    /^\/(?:home|usr\/local\/cuda|opt\/conda)\//.test(p)
  )
    return true;
  if (
    (plat === 'linux' || plat === 'termux') &&
    /^\/(?:Users|opt\/homebrew)\//.test(p)
  )
    return true;
  return false;
}
...
export function _buildServeCmd(f, modelName, backend) {
  ...
  const _activeVenvPath = _venvRootFromPath(
    _formVenv || (_envState.env === 'venv' ? _envState.envPath || '' : ''),
  );
  ...
  if (backend === 'vllm') {
    ...
    if (f.moe_env) {
      ...
      if (_opts.envVars.length) {
        ...
        cmd +=
          'VLLM_USE_DEEP_GEMM=0 VLLM_USE_FLASHINFER_MOE_FP16=1 OMP_NUM_THREADS=4 ';
      }
    }
    ...
    if (_swapRaw && !['0', 'off', 'none', 'false'].includes(_swapRaw))
      cmd += ` --swap-space ${_swapRaw}`;
    cmd += ` --dtype ${f.dtype || 'auto'}`;
    const _kv = (f.vllm_kv_cache_dtype ?? '').toString().trim();
    if (_kv === 'fp8') cmd += ' --kv-cache-dtype fp8';
    if (f.max_seqs && f.max_seqs.toString().trim())
      cmd += ` --max-num-seqs ${f.max_seqs.toString().trim()}`;
    if (f.enforce_eager) cmd += ' --enforce-eager';
    if (f.trust_remote) cmd += ' --trust-remote-code';
    if (f.prefix_cache) cmd += ' --enable-prefix-caching';
    if (f.auto_tool)
      cmd += ` --enable-auto-tool-choice --tool-call-parser ${_detectToolParser(modelName)}`;
    ...
    if (f.reasoning_parser) {
      const rp =
        typeof f.reasoning_parser === 'string' && f.reasoning_parser !== 'true'
          ? f.reasoning_parser
          : f._reasoning_parser_value ||
            _detectReasoningParser(modelName) ||
            '';
    ...
    if (f.speculative) {
      ...
      const _specToks =
        Number.isFinite(_specToksRaw) && _specToksRaw > 0 ? _specToksRaw : 3;
    ...
    const _memFraction =
      _isDsv4 && (!f.gpu_mem || f.gpu_mem === '0.90') ? '0.80' : f.gpu_mem;
    if (_memFraction && _memFraction !== '0.90')
      cmd += ` --mem-fraction-static ${_memFraction}`;
    if (f.dtype && f.dtype !== 'auto') cmd += ` --dtype ${f.dtype}`;
    if (f.max_seqs && f.max_seqs.toString().trim())
      cmd += ` --max-running-requests ${f.max_seqs.toString().trim()}`;
    if (f.trust_remote) cmd += ' --trust-remote-code';
    if (f.auto_tool)
      cmd += ` --enable-auto-tool-choice --tool-call-parser ${_detectToolParser(modelName)}`;
    if (f.expert_parallel) cmd += ' --enable-expert-parallel';
    ...
    if (!f.enforce_eager && _decodeGraph === 'disabled') {
      ...
      !f.enforce_eager &&
      _isDsv4 &&
      !/\s--cuda-graph-max-bs-decode\b/.test(cmd) &&
      !/\s--cuda-graph-backend-decode\b/.test(cmd)
    ) {
      cmd += ' --cuda-graph-backend-decode disabled';
    }
    ...
    if (_llamaMode === 'cpu') {
      ...
      ['gpu', 'unified'].includes(_llamaMode) &&
      (!f.ngl || String(f.ngl).trim() === '0')
    ) {
      f.ngl = '99';
    }
    ...
    const lcPrefix = (() => {
      let p = '';
      if (f.unified_mem && !_cpuOnly && !_isWindows() && _isCudaTarget)
        p += `GGML_CUDA_ENABLE_UNIFIED_MEMORY=1 `;
    ...
    if (['none', 'layer', 'row', 'tensor'].includes(_llamaSplitMode))
      _lcExtra += ` --split-mode ${_llamaSplitMode}`;
    ...
    if (f.llama_speculative_mtp) {
      ...
      const specN =
        Number.isFinite(specTokens) && specTokens > 0 ? specTokens : 3;
    ...
    if (modelName.includes('/') && (f.gguf_file || /-GGUF$/i.test(modelName))) {
      ...
      const hostEnv =
        ollamaPort !== '11434' ? `OLLAMA_HOST=${bindHost}:${ollamaPort} ` : '';
    ...
    if (f.diff_dtype && f.diff_dtype !== 'bfloat16')
      cmd += ` --dtype ${f.diff_dtype}`;
    if (f.diff_device_map && f.diff_device_map !== 'balanced')
      cmd += ` --device-map ${f.diff_device_map}`;
...
export function _persistEnvState() {
  try {
    localStorage.setItem(LAST_STATE_KEY, JSON.stringify(_envStateForStorage()));
  } catch (_) {}
  _saveTasks(_loadTasks());
}
...
async function _fetchDependencies() {
  ...
  try {
    ...
    label.style.cssText =
      'text-align:center;opacity:0.5;font-size:11px;margin-top:6px;';
  ...
  try {
    ...
    const _depSrv =
      _dsel && _dsel.value !== 'local' ? _serverByVal(_dsel.value) : null;
    if (_depSrv) {
      _depHost = _depSrv.host || '';
      _depPort = _depSrv.port || '';
      _depVenv = _depSrv.envPath || '';
    } else if (_envState.remoteHost) {
      _depHost = _envState.remoteHost;
      _depPort = _getPort(_envState.remoteHost) || '';
      _depVenv = _envState.envPath || '';
    }
    ...
    const _depBackend = String(
      _hwfitCache?.system?.backend || '',
    ).toLowerCase();
    ...
    const resp = await fetch(
      '/api/cookbook/packages' +
        (_pkgParams.toString() ? '?' + _pkgParams.toString() : ''),
    );
    const data = await resp.json();
    const pkgs = data.packages || [];
    if (!pkgs.length) {
      list.innerHTML = '<div class="hwfit-loading">No packages found</div>';
      return;
    }
    ...
    const _statusTag = (pkg, isLocal, isSystemDep, winBlocked) => {
      ...
      if (
        ...
        const tip = esc(
          pkg.update_note ||
            pkg.status_note ||
            'Found externally; update outside Odysseus.',
        );
      ...
      if (isSystemDep) {
        ...
        const depStyle =
          pkg.name === 'docker'
            ? ' style="width:87.7px;justify-content:center;"'
            : '';
    ...
    const _DEP_GLYPHS = {
      ...
      sglang:
        '<span aria-hidden="true" style="display:block;width:13px;height:13px;background:currentColor;-webkit-mask:url(/static/icons/sglang-mark.png) center/contain no-repeat;mask:url(/static/icons/sglang-mark.png) center/contain no-repeat;"></span>',
      mlx_lm:
        '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 18V6l4 7 4-7v12"/><path d="M16 6v12"/><path d="M20 6v12"/></svg>',
      llama_cpp:
        '<svg width="13" height="13" viewBox="0 0 600 600" fill="none" aria-hidden="true"><path d="M600 392L504.249 558L504.137 557.929C487.252 584.069 458.193 600 426.864 600H120L240 392H600Z" fill="currentColor"/><path d="M240 392H0L199.602 46.0254C216.032 17.5463 246.411 0 279.29 0H466.154L240 392Z" fill="currentColor"/></svg>',
      ollama:
        '<img src="/static/icons/ollama-mark-crop.png" alt="" aria-hidden="true" width="13" height="13" style="display:block;width:13px;height:13px;object-fit:contain;" />',
      diffusers:
        '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="4"/><path d="M12 2v3M12 19v3M2 12h3M19 12h3M5 5l2 2M17 17l2 2M5 19l2-2M17 7l2-2"/></svg>',
    };
    const _depGlyphHtml = (name) => {
      const g = _DEP_GLYPHS[name];
      return g
        ? `<span class="cookbook-dep-glyph" aria-hidden="true" style="display:inline-flex;align-items:center;justify-content:center;width:14px;height:14px;color:var(--accent, var(--red));margin-right:5px;vertical-align:-2px;">${g}</span>`
        : '';
    };
    ...
    const _depRow = (pkg) => {
      ...
      const winBlocked =
        !isLocal && _isWindows() && _winUnsupported.has(pkg.name);
      ...
      const _bdm = Array.isArray(pkg.build_deps_missing)
        ? pkg.build_deps_missing
        : [];
      ...
      const _instCmd =
        _bdm.length && pkg.install_cmd_for_target
          ? String(pkg.install_cmd_for_target)
          : '';
      const _instCmdOs = pkg.install_cmd_os ? String(pkg.install_cmd_os) : '';
      const _instCmdBe = pkg.install_cmd_backend
        ? String(pkg.install_cmd_backend)
        : '';
      const _instLabel =
        _instCmdOs && _instCmdBe
          ? `${_instCmdOs} + ${_instCmdBe}`
          : _instCmdOs || _instCmdBe || 'this target';
      ...
      const _gpuWheelCmd =
        'CMAKE_ARGS="-DGGML_CUDA=on" python3 -m pip install --user --break-system-packages --force-reinstall --no-cache-dir "llama-cpp-python[server]" --extra-index-url https://abetlen.github.io/llama-cpp-python/whl/cu124';
    ...
    function _recipePanelHtml(backend) {
      ...
      const downloadedIds = _cachedModelIds
        ? Array.from(_cachedModelIds).sort()
        : [];
    ...
    list.innerHTML = [
      _viewingRemote
        ...
        : _section(
            'Odysseus app',
            'Run inside the Odysseus app itself.',
            _appDeps,
          ),
      _section(
        'Server',
        'Run on the server chosen above (Local, or a remote box over SSH).',
        _serverDeps,
      ),
    ].join('');
    ...
    async function _installDep(
      ...
      const targetHost = isLocalOnly
        ? 'this server'
        : targetServer?.host || _envState.remoteHost || 'local';
      const targetEnv = isLocalOnly
        ? 'none'
        : targetServer?.env || _envState.env || 'none';
      const targetEnvPath = isLocalOnly
        ? ''
        : targetServer?.envPath || _envState.envPath || '';
      const targetPlatform = isLocalOnly
        ? _envState.hostPlatform || _envState.platform || ''
        : targetServer?.platform || _envState.platform || '';
      const targetRemoteHost = isLocalOnly
        ? ''
        : targetServer?.host || _envState.remoteHost || '';
      ...
      const _isAppleTarget =
        _platform === 'darwin' ||
        _platform === 'macos' ||
        _platform.includes('mac os');
      const _pipFlags =
        !_isWindows() && !_inEnv
          ? _isAppleTarget
            ? ' --user'
      ...
      if (_isWindows()) {
        if (targetEnv === 'venv' && targetEnvPath) {
          envPrefix =
            '& ' +
            _psQuote(
              targetEnvPath.endsWith('\\Scripts\\Activate.ps1')
                ? targetEnvPath
                : targetEnvPath + '\\Scripts\\Activate.ps1',
            );
      ...
      try {
        ...
        if (!res.ok || !data.ok) {
          ...
          uiModule.showToast(
            'Install failed: ' + String(reason).slice(0, 400),
            {
              duration: 20000,
              action: 'OK',
              onAction: () => {},
    ...
    list
      .querySelectorAll(
        '.cookbook-dep-install:not(.cookbook-dep-recipe-run):not(.cookbook-dep-install-sysdeps)',
      )
      .forEach((btn) => {
        btn.addEventListener('click', async (e) => {
          ...
          const pkgName =
            btn
              .closest('.cookbook-dep-row')
              ?.querySelector('.memory-item-title')?.textContent || pipName;
          await _installDep(
            pipName,
            pkgName,
    ...
    list.querySelectorAll('.cookbook-dep-partial').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        try {
          ...
          if (res.ok && data.ok) {
            const payload = {
              repo_id: 'pip llama-cpp-python[CUDA]',
              _cmd: cmd,
              remote_host: _envState.remoteHost || '',
              _dep: true,
            };
            _addTask(
              data.session_id,
              'pip llama-cpp-python[CUDA]',
              'download',
              payload,
            );
            uiModule.showToast(
              `Reinstalling llama-cpp-python with CUDA wheels on ${targetLabel} (~1-3 min)…`,
              4000,
            );
          } else {
            uiModule.showToast(
              'Upgrade failed: ' +
                String(data.detail || data.error || `HTTP ${res.status}`).slice(
                  0,
                  300,
                ),
              {
          ...
          uiModule.showToast('Upgrade request failed: ' + err.message, {
            duration: 20000,
            action: 'OK',
            onAction: () => {},
          });
    ...
    list.querySelectorAll('.cookbook-dep-install-sysdeps').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation();
        const names = (btn.dataset.depSysdeps || '')
          .split(',')
          .map((s) => s.trim())
          .filter(Boolean);
        ...
        try {
          ...
          if (res.ok && data.ok) {
            uiModule.showToast(
              `Installed ${names.join(', ')} on ${targetLabel}. Refreshing…`,
              4000,
            );
            ...
            const _suffix = _resolvedCmd
              ...
              : '';
            uiModule.showToast(
              'System dependency install failed: ' +
                ...
                _suffix,
              {
                ...
                onAction: async () => {
                  if (_resolvedCmd) {
                    try {
                      await navigator.clipboard.writeText(_resolvedCmd);
                    } catch {}
                  }
                },
              },
    ...
    list.querySelectorAll('[data-dep-recipe-toggle]').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        ...
        const panel = list.querySelector(
          `[data-dep-recipe-panel="${CSS.escape(backend)}"]`,
        );
        if (!panel) return;
    ...
    function _refreshRecipePre(backend) {
      ...
      if (!panel) return;
      const variant =
        panel.dataset.depRecipeActiveVariant || RECIPE_DEFAULT_VARIANT;
    ...
    list.querySelectorAll('[data-dep-recipe-pick]').forEach((sel) => {
      sel.addEventListener('change', () =>
        _refreshRecipePre(sel.dataset.depRecipePick),
      );
    });
    ...
    list.querySelectorAll('[data-dep-recipe-variant]').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        ...
        const container = panel.querySelector(
          '.mode-toggle[data-dep-recipe-variants]',
        );
        if (container)
          container.classList.toggle('mode-right', variant === 'docker');
    ...
    list.querySelectorAll('[data-dep-recipe-copy]').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        const pre = list.querySelector(
          `[data-dep-recipe-cmds="${CSS.escape(backend)}"]`,
        );
        if (!pre) return;
        try {
    ...
    async function _rebuildLlamaCpp(updateSource = false, statusEl = null) {
      ...
      const action = updateSource
        ? 'Update llama.cpp source and rebuild'
        : 'Rebuild llama.cpp engine';
      ...
      try {
        ...
        if (!res.ok || !data.ok) {
          const reason = data.detail || data.error || `HTTP ${res.status}`;
          uiModule.showToast(
            `${updateSource ? 'Update' : 'Rebuild'} failed: ` +
              String(reason).slice(0, 300),
            {
              duration: 20000,
              action: 'OK',
          ...
          uiModule.showToast(
            `${updateSource ? 'Updated source and cleared' : 'Cleared'} llama.cpp build on ${where}. Re-launch the serve task to rebuild.`,
          );
        }
      } catch (err) {
        uiModule.showToast(
          `${updateSource ? 'Update' : 'Rebuild'} failed: ` + err.message,
        );
      } finally {
        if (statusEl) {
    ...
    function _showDepMenu(anchor) {
      ...
      const pkgName =
        row.querySelector('.memory-item-title')?.textContent || pipName;
      ...
      if (rowPkgName === 'llama_cpp') {
        ...
        rebuild.title =
          'Clear the cached llama-server build so the next launch rebuilds it.';
        ...
        source.title =
          'Fast-forward ~/llama.cpp when possible, then clear the cached build.';
    ...
    list.querySelectorAll('.cookbook-dep-installed-btn').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        ...
        if (document.querySelector('.cookbook-dep-menu')) {
          document
            .querySelectorAll('.cookbook-dep-menu')
...
async function _refreshScanDownloadTarget() {
  ...
  const selectedVal =
    document.getElementById('hwfit-server-select')?.value ||
    _currentServerValue();
  if (btn) {
    btn.disabled = true;
  ...
  try {
    ...
    _resetGpuToggleState();
    await Promise.allSettled([_hwfitFetch(true), _fetchCachedModels(true)]);
    ...
    if (uiModule?.showError)
      uiModule.showError('Refresh failed: ' + (e?.message || e));
  } finally {
    if (btn) {
      btn.disabled = false;
      btn.style.opacity = '';
...
function _wireTabEvents(body) {
    ...
    body.addEventListener(
      ...
      (e) => {
        ...
        if (
          ...
          e.touches.length !== 1 ||
          e.target.closest('input, textarea, select, .doclib-lang-chips')
        ) {
          _sx = null;
          return;
        }
  ...
  function _syncServers() {
    ...
    entries.forEach((entry) => {
      const name =
        entry.querySelector('.cookbook-srv-name')?.value?.trim() || '';
      const host =
        entry.querySelector('.cookbook-srv-host')?.value?.trim() || '';
      const port =
        entry.querySelector('.cookbook-srv-port')?.value?.trim() || '';
      const env = entry.querySelector('.cookbook-srv-env')?.value || 'none';
      const envPath =
        entry.querySelector('.cookbook-srv-path')?.value?.trim() || '';
      const color = _serverColorValue(
        entry.querySelector('.cookbook-srv-color')?.value || '',
      );
    ...
    localStorage.setItem(
      'cookbook-last-state',
      JSON.stringify(_envStateForStorage()),
    );
    _saveTasks(_loadTasks());
  ...
  document
    .querySelectorAll(
      '.cookbook-srv-name, .cookbook-srv-color, .cookbook-srv-host, .cookbook-srv-port, .cookbook-srv-path',
    )
    .forEach((el) => {
  ...
  if (addServerLink) {
    addServerLink.addEventListener('click', () => {
      const settingsTab = body.querySelector(
        '.cookbook-tab[data-backend="Settings"]',
      );
  ...
  if (cacheServer) {
    cacheServer.addEventListener('change', () => {
      ...
      if (val === 'local') {
        srv =
          _envState.servers.find(_isLocalEntry) || _envState.servers[0] || {};
      } else {
        srv = _serverByVal(val) || {};
      }
      if (cacheDirEl)
        cacheDirEl.value = srv.modelDir || '~/.cache/huggingface/hub';
      const dirsEl = document.querySelector('.cookbook-serve-dirs');
      if (dirsEl) {
        const dirs = (
          Array.isArray(srv.modelDirs)
            ? srv.modelDirs
            : [srv.modelDir || '~/.cache/huggingface/hub']
        )
          .map((d) => _normalizeCookbookModelDir(d))
          .filter(Boolean);
        dirsEl.innerHTML =
          dirs
            .map(
              (d) => `<span class="cookbook-serve-dir-pill">${esc(d)}</span>`,
            )
            .join('') +
  ...
  if (rebuildBtn && !rebuildBtn._wired) {
    ...
    rebuildBtn.addEventListener('click', async () => {
      ...
      const host = _envState.remoteHost || '';
      const where = host || 'this server';
      if (window._cookbookRebuildLlamaCpp)
        await window._cookbookRebuildLlamaCpp(false, rebuildBtn);
    });
  }
  ...
  if (selectBtn && bulkBar) {
    ...
    document
      .getElementById('hwfit-cached-list')
    ...
    document
      .getElementById('serve-bulk-cancel')
      ?.addEventListener('click', () => {
    ...
    document
      .getElementById('serve-bulk-delete')
      ?.addEventListener('click', async () => {
        ...
        if (
          !(await uiModule.styledConfirm(
            `Delete ${repos.length} model(s)? This removes cached files.`,
  ...
  if (dlCardToggle && dlCardBody) {
    dlCardToggle.addEventListener('click', () => {
      ...
      if (dlCardArrow)
        dlCardArrow.style.transform = isOpen ? 'rotate(0deg)' : 'rotate(90deg)';
    });
  }
  if (dlBtn && dlInput) {
    function _stripHfUrl(input) {
      ...
      repo = repo.replace(
        /^(?:huggingface-cli|hf-cli|hf)\s+(?:download|d)\s+/i,
        '',
      );
    ...
    function _ggufQuantFromPath(path) {
      ...
      const dirQuant = dir.match(
        /^(?:I?Q\d(?:_[A-Z0-9]+){0,3}|UD-[A-Z0-9_]+)$/i,
      );
      if (dirQuant) return dirQuant[0].toUpperCase();
      const fileQuant = file.match(
        /(?:^|[-_./])((?:I?Q\d(?:_[A-Z0-9]+){0,3})|(?:UD-[A-Z0-9_]+))(?=(?:[-_.]|\.gguf|$))/i,
      );
    ...
    async function _scanGgufRepo(rawValue) {
      ...
      const split = ollamaName
        ? { repo: ollamaName, include: null }
        : fileSplit || _splitRepoTag(rawRepo);
      ...
      try {
        const res = await fetch(
          `/api/cookbook/hf-gguf-files?repo_id=${encodeURIComponent(repo)}`,
          { credentials: 'same-origin' },
        );
        ...
        const quants = Array.from(byQuant.keys()).sort(
          (a, b) => quantRank(a) - quantRank(b) || a.localeCompare(b),
        );
        dlGgufQuant.innerHTML = quants
          .map((q) => {
    ...
    function _splitRepoFile(raw) {
      ...
      if (!/\.(gguf|safetensors|bin|pt|pth|onnx|mlx)(\?[^?]*)?$/i.test(fname))
        return null;
    ...
    function _ollamaName(raw) {
      ...
      if (
        /^[A-Za-z0-9][A-Za-z0-9._-]{0,200}:[A-Za-z0-9][A-Za-z0-9._-]{0,200}$/.test(
          stripped,
        )
      ) {
        return stripped;
    ...
    const triggerDownload = async () => {
      ...
      if (!ollamaName && !/^[^\s/]+\/[^\s/]+$/.test(repo)) {
        uiModule.showToast(
          'Enter a full HuggingFace repo ID like "org/model-name", or an Ollama name like "qwen2.5:14b".',
        );
        dlInput.focus();
        return;
      }
      const looksGgufRepo =
        !ollamaName && !_fileSplit && !autoInclude && /\bgguf\b/i.test(repo);
      if (looksGgufRepo && !pickerInclude) {
        ...
        try {
          ...
          pickerInclude =
            found && dlGgufQuant?.dataset.repo === repo
              ? dlGgufQuant.value || ''
              : '';
        } finally {
        ...
        if (!pickerInclude) {
          uiModule.showToast(
            'Pick a GGUF quant first. Odysseus will not download the whole GGUF repo without an include pattern.',
          );
          return;
        }
      ...
      if (autoInclude || pickerInclude)
        payload.include = autoInclude || pickerInclude;
      if (_envState.hfToken && !ollamaName)
        payload.hf_token = _envState.hfToken;
      if (host) {
      ...
      if (srvPlatform === 'windows') {
        if (env === 'venv' && envPath) {
          payload.env_prefix =
            '& ' +
            _psQuote(
              envPath.endsWith('\\Scripts\\Activate.ps1')
                ? envPath
      ...
      const displayName = payload.include
        ? `${shortName} · ${
            _ggufQuantFromPath(String(payload.include).replace(/\*/g, '')) ||
  ...
  if (dlFold && dlFoldBody && dlFoldChevron) {
    const _setFolded = (folded, persist = true) => {
      ...
      if (persist) {
        try {
          localStorage.setItem('cookbook_dl_tab_folded_v1', folded ? '1' : '0');
        } catch {}
      }
    };
    ...
    _modal.addEventListener(
      ...
      (e) => {
        ...
        if (
          dlFoldBody.contains &&
          (tgt === dlFoldBody || dlFoldBody.contains(tgt))
        )
          return;
  ...
  if (hfToggle && hfList) {
    ...
    function _hfModelLooksAwqLike(m) {
      const text =
        `${m?.repo_id || ''} ${(m?.tags || []).join(' ')}`.toLowerCase();
    ...
    async function _getSelectedServerHw() {
      ...
      try {
        ...
        if (r.ok) {
          ...
          const hw = {
            vram: sys?.gpu_vram_gb || 0,
            backend: String(sys?.backend || '').toLowerCase(),
          };
    ...
    async function _loadLatest() {
      ...
      try {
        ...
        const _fetchLatest = async (v) => {
          const res = await fetch(
            `/api/cookbook/hf-latest?vram_gb=${v}&limit=10`,
          );
          const data = await res.json();
        ...
        if (
          ['rocm', 'metal', 'mps', 'apple', 'generic', 'cpu'].includes(
            hwInfo.backend,
          )
        ) {
        ...
        for (const m of models) {
          ...
          if (m.downloads)
            meta.push(`${m.downloads.toLocaleString()} downloads`);
    ...
    hfToggle.addEventListener('click', () => {
      ...
      if (hfArrow)
        hfArrow.style.transform = isOpen ? 'rotate(0deg)' : 'rotate(90deg)';
      if (!isOpen && !_loaded) {
        _loaded = true;
        _loadLatest();
      }
    ...
    if (hfRefresh)
      hfRefresh.addEventListener('click', (e) => {
        e.stopPropagation();
        _loaded = true;
        _loadLatest();
    ...
    document
      .getElementById('hwfit-dl-server')
      ?.addEventListener('change', _onServerChange);
    document
      .getElementById('hwfit-server-select')
      ?.addEventListener('change', _onServerChange);
  }
  ...
  if (olToggle && olList) {
    ...
    async function _loadOllama(refresh = false) {
      olList.innerHTML =
        '<div class="hwfit-loading" style="opacity:0.5;font-size:11px;text-align:center;padding:12px;">Loading…</div>';
      try {
        const res = await fetch(
          `/api/cookbook/ollama/library${refresh ? '?refresh=1' : ''}`,
        );
        const data = await res.json();
        ...
        for (const m of models) {
          const sizes =
            Array.isArray(m.sizes) && m.sizes.length ? m.sizes : ['latest'];
          const sizeChips = sizes
            .map(
              (s) =>
                `<button type="button" class="memory-toolbar-btn cookbook-ol-size" data-name="${esc(m.name)}" data-size="${esc(s)}" style="height:20px;padding:0 6px;font-size:10px;border-radius:3px;">${esc(s)}</button>`,
            )
            .join('');
          ...
          if (m.description)
            html += `<div class="memory-item-meta" style="font-size:10px;opacity:0.55;margin-top:2px;">${esc(m.description)}</div>`;
        ...
        olList.querySelectorAll('.cookbook-ollama-card').forEach((card) => {
          card.addEventListener('click', (e) => {
            if (e.target.closest('a') || e.target.closest('.cookbook-ol-size'))
              return;
            const name = card.dataset.name;
            const firstSize =
              card.querySelector('.cookbook-ol-size')?.dataset.size || 'latest';
            if (dlInput) {
    ...
    olToggle.addEventListener('click', () => {
      ...
      if (olArrow)
        olArrow.style.transform = isOpen ? 'rotate(0deg)' : 'rotate(90deg)';
      if (!isOpen && !_olLoaded) {
        _olLoaded = true;
        _loadOllama(false);
      }
    ...
    if (olRefresh)
      olRefresh.addEventListener('click', (e) => {
        e.stopPropagation();
        _olLoaded = true;
        _loadOllama(true);
    ...
    hfInput.addEventListener('change', async () => {
      ...
      if (val) {
        ...
        if (!check) {
          ...
          check.textContent = '✓';
          check.style.cssText =
            'font-weight:800;color:var(--green,#50fa7b);font-size:15px;line-height:1;flex-shrink:0;position:relative;top:2px;';
          hfInput.parentNode.insertBefore(check, hfInput);
        }
        const flash = document.createElement('span');
        flash.textContent = 'Saved';
        flash.style.cssText =
          'margin-left:8px;font-size:11px;color:var(--green,#50fa7b);opacity:0;transition:opacity 0.18s;flex-shrink:0;position:relative;top:1px;';
...
export function _serverDefaultHtml(active) {
  const check = active
    ? '<span class="hwfit-hf-check cookbook-srv-default-check" title="Default server" style="font-weight:800;color:var(--green,#50fa7b);font-size:15px;line-height:1;flex-shrink:0;position:relative;top:2px;">✓</span>'
    : '';
...
export function _serverEntryHtml(s, i, defaultServer, forceRemote, isNew) {
  ...
  const envOpts = [
    ...
    .map(
      ([value, label]) =>
        `<option value="${value}"${s.env === value ? ' selected' : ''}>${label}</option>`,
    )
    .join('');
  const srvColor = _serverColor(s);
  const resolvedSrvColor = _resolvedServerColor(s, i);
  const colorOpts = _SERVER_COLOR_CHOICES
    .map(([value, label]) => {
      const displayLabel = label;
      return `<option value="${esc(value)}"${_serverColorOptionStyle(value)}${srvColor === value ? ' selected' : ''}>${esc(displayLabel)}</option>`;
    })
    .join('');
  const selectedColorLabel = srvColor
    ? _serverColorLabel(srvColor)
    : `Auto · ${_serverColorLabel(resolvedSrvColor)}`;
  const colorMenu = _SERVER_COLOR_CHOICES
    .map(([value, label]) => {
      ...
      const swatch = swatchColor
        ? ` style="--swatch-color:${esc(swatchColor)};"`
        : '';
  ...
  html += `${esc(_srvTitle)}`;
  html += _pIco
    ? `<span class="cookbook-srv-platform" title="${esc(s.platform || '')}" style="display:inline-flex;align-items:center;opacity:0.55;">${_pIco}</span>`
    : '';
  ...
  html += `</div>`;
  const modelDirs =
    Array.isArray(s.modelDirs) && s.modelDirs.length
      ? s.modelDirs
  ...
  for (let j = 0; j < modelDirs.length; j++) {
    ...
    const rmBtn = isDefault
      ? ''
      : ' <span class="cookbook-modeldir-rm" title="Remove">✖</span>';
  ...
  const _btnBaseStyle =
    'position:relative;top:-2px;height:22px;box-sizing:border-box;display:inline-flex;align-items:center;';
...
function _renderRecipes() {
  ...
  html +=
    '<button class="cookbook-tab" data-backend="Serve"><svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" stroke="none" style="vertical-align:-1px;margin-right:3px;"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>Launch</button>';
  html +=
  ...
  // Search group
  html +=
    '<div class="cookbook-group" data-backend-group="Search" style="flex:0 0 auto;">';
  html +=
  ...
  const _dlTabFolded = (() => {
    try {
      return localStorage.getItem('cookbook_dl_tab_folded_v1') === '1';
    } catch {
      return false;
    }
  })();
  ...
  html +=
    '<p class="memory-desc doclib-desc" style="margin-top:6px;">Download from <a href="https://huggingface.co/models" target="_blank" rel="noopener" style="color:var(--accent,var(--red));text-decoration:none;"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:1px;"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>HuggingFace</a> by pasting model link, or download directly in the Scan section below.</p>';
  ...
  html += '</div>';
  html +=
    '<p class="memory-desc doclib-desc" style="margin-top:6px;">Scans your hardware for what models you can run. Hardware is cached; hit the scan button to re-probe after changing GPUs.</p>';
  html += '<div class="hwfit-toolbar" style="margin-top:9px;">';
  html +=
    '<select class="cookbook-field-input hwfit-usecase" id="hwfit-usecase" style="height:28px;">';
  html +=
    '<option value="general" selected>Standard</option><option value="coding">Coding</option>';
  html +=
    '<option value="reasoning">Reasoning</option><option value="chat">Chat</option>';
  ...
  html +=
    '<input type="text" class="cookbook-field-input hwfit-search" id="hwfit-search" placeholder="Search models..." style="flex:1;" />';
  html += '<span class="hwfit-engine-wrap">';
  html +=
    '<select class="cookbook-field-input hwfit-engine" id="hwfit-engine" style="height:28px;" title="Filter by serving engine">';
  ...
  html += '</select>';
  html +=
    '<span class="hwfit-help-chip hwfit-help-chip-inline hwfit-engine-help" title="Rule of thumb: GGUF on single GPU / CPU+RAM → llama.cpp (or Ollama). Safetensors on multi-GPU NVIDIA → vLLM. SGLang is a vLLM-class alternative, sometimes faster on big-MoE / long-context.">?</span>';
  html += '</span>';
  ...
  html +=
    '<select class="cookbook-field-input hwfit-quant" id="hwfit-quant" style="height:28px;">';
  html += '<option value="" selected>Quant</option>';
  html +=
    '<option value="Q4_K_M">Q4 / AWQ</option><option value="Q8_0">Q8</option>';
  ...
  html +=
    '<label class="hwfit-ctx-control" title="Context length for fit estimates. Lower it to find more models that could fit your hardware.">';
  html +=
    '<span>Context</span><span class="hwfit-help-chip hwfit-help-chip-inline" title="Context length. Lower it to find more models that could fit your hardware; raise it when you need longer chats or documents.">?</span><input type="range" id="hwfit-context" min="0" max="5" step="1" value="3" />';
  html += '<output id="hwfit-context-label">50k</output></label>';
  html += '</div>';
  html += '<div class="hwfit-toolbar" style="margin-top:7px;">';
  html +=
    '<select class="cookbook-field-input hwfit-server-select" id="hwfit-server-select" style="height:28px;min-width:88px;position:relative;top:0px;">';
  ...
  html +=
    '<select class="cookbook-field-input hwfit-sort" id="hwfit-sort" style="display:none">';
  html += '<option value="newest" selected>Latest</option>';
  html +=
    '<option value="fit">Fit</option><option value="score">Score</option><option value="vram">VRAM</option>';
  html +=
    '<option value="speed">Speed</option><option value="params">Params</option>';
  html += '<option value="context">Context</option></select>';
  html += '</div>';
  html += '<div class="hwfit-manual-panel hidden" id="hwfit-manual-panel">';
  html +=
    '<span class="hwfit-manual-note" style="font-size:10px;opacity:0.6;width:100%;margin-bottom:2px;">Simulator — these values REPLACE detected hardware.</span>';
  html +=
    '<select class="hwfit-manual-mode"><option value="gpu">GPU</option><option value="ram">RAM</option></select>';
  html +=
    '<label>GPUs<input class="hwfit-manual-gpus" type="text" inputmode="numeric" placeholder="1"></label>';
  html +=
    '<label>VRAM per GPU<input class="hwfit-manual-vram" type="text" inputmode="decimal" placeholder="8 GB"></label>';
  html +=
    '<label>Total RAM<input class="hwfit-manual-ram" type="text" inputmode="decimal" placeholder="32 GB"></label>';
  html +=
    '<select class="hwfit-manual-backend"><option value="cuda">CUDA</option><option value="rocm">ROCm</option></select>';
  html += '<button type="button" class="hwfit-hw-manual-save">✓ Apply</button>';
  html +=
    '<button type="button" class="hwfit-hw-manual-clear">× Clear</button>';
  html += '</div>';
  html +=
    '<div id="hwfit-hw-row" style="display:none;align-items:center;gap:4px;margin-top:3px;padding-top:2px;"><span style="font-size:10px;padding:2px 8px;border-radius:10px;background:color-mix(in srgb, var(--fg) 8%, transparent);color:var(--fg);opacity:0.7;white-space:nowrap;flex-shrink:0;position:relative;top:-1px;">Detected hardware</span><div class="hwfit-hw" id="hwfit-hw" style="flex:1;"></div></div>';
  ...
  html +=
    '<div class="hwfit-list-footer" style="display:none;">' +
    "Don't see a model? " +
  ...
  html +=
    '<div style="display:flex;align-items:baseline;gap:8px;margin-bottom:2px;">';
  html +=
    '<h2 style="margin:0;padding:0;line-height:1;">Serve <span id="serve-stats" class="memory-count" style="font-size:0.6em;opacity:0.6;font-weight:normal"></span></h2>';
  html += '</div>';
  const _selSrv =
    _es.servers.find((s) => s.host === _es.remoteHost) || _es.servers[0] || {};
  const _srvDirs = (
    Array.isArray(_selSrv.modelDirs)
      ? _selSrv.modelDirs
      : [_selSrv.modelDir || '~/.cache/huggingface/hub']
  )
    .map((d) => _normalizeCookbookModelDir(d))
    .filter(Boolean);
  ...
  html +=
    '<select class="memory-sort-select" id="hwfit-cache-server" style="height:24px;">' +
    _buildServerOpts(true) +
    '</select>';
  html +=
    '<select class="memory-sort-select" id="serve-sort" style="height:24px;">';
  html +=
    '<option value="name">Name</option><option value="size-desc">Size \u2193</option><option value="size-asc">Size \u2191</option><option value="recent">Recent</option>';
  html += '</select>';
  html +=
  ...
  html +=
    '<input type="text" class="memory-search-input" id="serve-search" placeholder="Search cached models\u2026" style="flex:1;min-width:120px;" />';
  html +=
    '<button class="memory-toolbar-btn" id="hwfit-cache-select">Select</button>';
  html += '</div>';
  ...
  html +=
    '<label class="memory-bulk-check-all"><input type="checkbox" id="serve-select-all"> All</label>';
  html +=
    '<span id="serve-bulk-count" style="font-size:10px;opacity:0.5;">0 selected</span>';
  html +=
  ...
  html +=
    '<div class="doclib-grid hwfit-cached-list" id="hwfit-cached-list"></div>';
  html += '</div></div>';

  // Dependencies tab
  html +=
    '<div class="cookbook-group hidden" data-backend-group="Dependencies">';
  html +=
    '<div class="admin-card" style="flex:1;display:flex;flex-direction:column;overflow:hidden;">';
  html +=
  ...
  html += '</div>';
  html +=
    '<p class="memory-desc doclib-desc">Optional packages that extend Odysseus capabilities.</p>';
  ...
  html +=
    '<div class="cookbook-group hidden cookbook-settings-stack" data-backend-group="Settings">';

  // ── HuggingFace Token block ─────────────────────────────────────────
  html +=
    '<div class="admin-card" style="flex:0 0 auto;display:flex;flex-direction:column;">';
  html +=
    '<div style="display:flex;align-items:baseline;gap:8px;margin-bottom:2px;">';
  html +=
    '<h2 style="margin:0;padding:0;line-height:1;">HuggingFace Token</h2>';
  html += '</div>';
  html +=
    '<p class="memory-desc doclib-desc">Personal access token for downloading gated and private models.</p>';
  ...
  html +=
    '<div class="admin-card" style="flex:0 0 auto;display:flex;flex-direction:column;">';
  html +=
    '<div style="display:flex;align-items:baseline;gap:8px;margin-bottom:2px;margin-top:-4px;">';
  ...
  html +=
    '<button class="cal-add-btn cal-add-btn-text" id="cookbook-server-add" title="Add server" style="margin-left:auto;"><span class="cal-add-plus">+</span><span class="cal-add-label">Add</span></button>';
  html += '</div>';
  html +=
    '<p class="memory-desc doclib-desc">Configure SSH servers, install Odysseus keys, choose model directories, and set the default server. Local is this machine.</p>';
  html +=
    '<div class="memory-toolbar cookbook-servers-toolbar" style="margin-top:4px;">';
...
export async function open(opts) {
  ...
  const _applyIntent = () => {
    if (!opts) return;
    if (opts.tab) {
      const t = modal.querySelector(
        `.cookbook-tab[data-backend="${opts.tab}"]`,
      );
      if (t && !t.classList.contains('active')) t.click();
    }
    if (opts.usecase) {
      const u = document.getElementById('hwfit-usecase');
      if (u && u.value !== opts.usecase) {
        u.value = opts.usecase;
  ...
  try {
    ...
    if (_envState.defaultServer) {
      ...
      if (_dk === 'local') {
        ...
        if (_ds) {
          _envState.remoteHost = _ds.host;
          _envState.env = _ds.env || 'none';
          _envState.envPath = _ds.envPath || '';
          _envState.platform = _ds.platform || '';
        }
      }
    }
    ...
    try {
      ...
      console.error('[cookbook] renderRecipes failed', e);
    }
    _rendered = true;
    _clearCookbookNotif();
    try {
      ...
      console.error('[cookbook] renderRunningTab failed', e);
    }
    ...
    try {
      _selfHealStaleTasks({ oneShot: true });
    } catch {}
    if (_content) {
  ...
  try {
    ...
    if (activeTab === 'Running') {
      ...
      const editingSettings =
        active && active.closest && active.closest('.cookbook-settings-stack');
      if (!editingSettings) {
        _renderRecipes();
...
const cookbookModule = {
  open,
  close,
  isVisible,
  startBackgroundMonitor: _startBackgroundMonitor,
};
