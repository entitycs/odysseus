...
export function _buildServeCmd(f, modelName, backend) {
  ...
  if (backend === 'vllm') {
    ...
    if (f.gpu_mem && f.gpu_mem !== '0.90')
      cmd += ` --mem-fraction-static ${f.gpu_mem}`;
...
async function _fetchDependencies() {
  ...
  try {
    ...
    async function _installDep(
      ...
      const _pipFlags =
        !_isWindows() && !_inEnv ? ' --user --break-system-packages' : '';
...
function _wireTabEvents(body) {
  ...
  document
    .querySelectorAll(
      '.cookbook-srv-name, .cookbook-srv-host, .cookbook-srv-port, .cookbook-srv-path',
    )
    .forEach((el) => {
...
export function _serverEntryHtml(s, i, defaultServer, forceRemote, isNew) {
  ...
  const envOpts = ['none', 'venv']
    .map(
      (e) =>
        `<option value="${e}"${s.env === e ? ' selected' : ''}>${e === 'none' ? 'None' : e}</option>`,
    )
    .join('');
  let html = '';
  ...
  const _btnStyle =
    'margin-left:auto;position:relative;top:-2px;height:22px;box-sizing:border-box;display:inline-flex;align-items:center;';
  if (isNew) {
