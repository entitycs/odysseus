...
import { getAllPresets, PROMPT_TEMPLATES } from '$lib/legacy/presets.js';
  ...
  try {
    ...
    if (allPresets && allPresets.custom && allPresets.custom.character_name) {
      chars.push({
        ...
        prompt:
          allPresets.custom.system_prompt || allPresets.custom.prompt || '',
      });
    }
  } catch (e) {}
...
function _createGroupBubble(model, box) {
  ...
  const roleLabel =
    model._groupName ||
    (model.character ? model.character.characterName : shortModel(model.mid));
