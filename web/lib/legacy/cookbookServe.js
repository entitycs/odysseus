...
function _readCachedModelScan(sig) {
  try {
    const all = JSON.parse(
      localStorage.getItem(_CACHED_MODELS_SCAN_KEY) || '{}',
    );
    const entry = all[sig];
    if (entry && Date.now() - (entry.ts || 0) < _CACHED_MODELS_SCAN_TTL) {
      const data = entry.data || null;
      const models = Array.isArray(data?.models) ? data.models : [];
      const staleDownloading = models.some(
        (m) =>
          (m?.status === 'downloading' || m?.has_incomplete) &&
          !_isActivelyDownloading(m?.repo_id),
      );
...
function _saveServeFavorites(favorites) {
  try {
    ...
    document.dispatchEvent(
      new CustomEvent('cookbook:state-dirty', {
        detail: { key: SERVE_FAVORITES_KEY },
      }),
    );
  } catch {}
}
...
function _redactServeStateForStorage(value) {
  ...
  for (const key of Object.keys(safe)) {
    if (/token|password|passwd|secret|api[_-]?key/i.test(key)) {
      ...
      /cmd|command|args|env/i.test(key)
    ) {
...
function _formatServeCmdPreview(cmd) {
  let raw = String(cmd || '');
  const mlxDeepSeekV4Compat =
    /\bmlx_lm\.server\b/i.test(raw) &&
  ...
  if (raw.startsWith('MODEL_FILE=$({')) {
    const marker =
      /&&\s+([A-Za-z_][A-Za-z0-9_]*=\S+\s+)*(?:[A-Za-z_][A-Za-z0-9_]*=\S+\s+)?(?:llama-server|python3?\s+-m\s+llama_cpp\.server)\b/;
  ...
  const formatted = lines
    .map((line, idx) => {
      ...
      const hasNextCommandPart = lines
        .slice(idx + 1)
        .some((next) => !next.startsWith('export '));
...
function _normalizeServeCmdForLaunch(cmd) {
  ...
  if (
    lines.some((line) => /^(?:export\s+)?[A-Za-z_][A-Za-z0-9_]*=/.test(line))
  ) {
    const env = [];
    const body = [];
...
function _estimateVllmContextFit(
  model,
  fields,
  modelCtxMax,
  modelWeightsGb = 0,
  ...
  const gpuIds = String(fields.gpus || '')
    .split(',')
  ...
  const perGpuGb =
    Number(activeGroup?.vram_each) ||
    Number(sys.gpu_vram_gb) /
      Math.max(1, Number(sys.gpu_count) || selectedCount) ||
    0;
  if (!perGpuGb) {
    return {
      needsHardwareScan: true,
      reason: 'scan hardware first to estimate context from VRAM',
    };
  }

  const gpuUtil = Math.min(
    0.99,
    Math.max(0.1, parseFloat(fields.gpu_mem) || 0.9),
  );
  ...
  if (!modelGb)
    return {
      needsModelSize: true,
      reason:
        'model weight size unknown; scan model files or enter context manually',
    };
  ...
  if (isMiniMaxM3) {
    ...
    const kvGbPerToken =
      (29.25 / 1048576) *
      (String(fields.vllm_kv_cache_dtype || '').toLowerCase() === 'fp8'
        ? 1
        : 1.8);
    if (freeForKv <= 0) {
      return {
  ...
  const isMoE = /\bmoe\b|a\d+b|minimax|deepseek|mixtral|kimi-k2|glm-4\.5/.test(
    lower,
  );
  ...
  const activeParams =
    activeFromName || (isMoE ? Math.min(totalParams, 32) : totalParams);
  const effectiveActiveParams =
    /minimax/.test(lower) && /\bm3\b/.test(lower) ? 23 : activeParams;
  const kvDtype = String(fields.vllm_kv_cache_dtype || '').toLowerCase();
  const kvFactor = kvDtype === 'fp8' ? 0.55 : 1;
  const kvGbPerTokenTotal = Math.max(
    0.00002,
    0.000008 * effectiveActiveParams * kvFactor,
  );
...
function _estimateLlamaContextFit(
  model,
  fields,
  modelCtxMax,
  modelWeightsGb = 0,
  ...
  const profiles = Array.isArray(profileData?.profiles)
    ...
    : [];
  const preferred =
    profiles.find((p) => String(p?.key || '').toLowerCase() === 'balanced') ||
  ...
  const isUnifiedMode =
    backend === 'llamacpp' && (llamaMode === 'unified' || fields.unified_mem);
  if (!modelGb) {
    return {
  ...
  const selectedCount = Math.max(
    1,
    gpuIds.length || parseInt(fields.tp, 10) || 1,
  );
  const groups = Array.isArray(sys.gpu_groups) ? sys.gpu_groups : [];
  const activeGroup = sys.active_group || groups[0] || null;
  ...
  const kvFactor =
    kv === 'q4_0' ? 0.55 : kv === 'q8_0' ? 1 : kv === 'f16' ? 1.9 : 1;
...
function _selectedServeTarget(panel) {
  const select =
    panel?.querySelector?.('#hwfit-server-select') ||
...
function _backendChoicesForTarget(target) {
  if (target?.platform === 'windows') {
    if (_remoteWindowsDiffusersUnsupported(target))
...
function _runtimeNoteText(backend, pkg, target) {
  ...
  if (pkg.installed === null || pkg.probe_error) {
    return note
      ? `${label} readiness unavailable for ${target.label}: ${note}`
      : `${label} readiness unavailable for ${target.label}.`;
  }
  if (pkg.installed) {
    return note
...
function _rerenderCachedModels() {
  ...
  const _parseSize = (s) => {
    ...
    if (!m) return 0;
    const n = parseFloat(m[1]);
    if (m[2] === 'GB') return n * 1024;
    if (m[2] === 'MB') return n;
    return n / 1024;
  };
  if (sortVal === 'name')
    allModels.sort((a, b) => (a.repo_id || '').localeCompare(b.repo_id || ''));
  else if (sortVal === 'size-desc')
    allModels.sort((a, b) => _parseSize(b.size) - _parseSize(a.size));
  else if (sortVal === 'size-asc')
    allModels.sort((a, b) => _parseSize(a.size) - _parseSize(b.size));
  else if (sortVal === 'recent')
    allModels.sort((a, b) => (b.mtime || 0) - (a.mtime || 0));
  const favorites = _loadServeFavorites();
  allModels.sort((a, b) => {
  ...
  for (const m of allModels) {
    ...
    const _isDlActive = _isDownloading
      ? _isActivelyDownloading(m.repo_id)
      : false;
    const _isFavorite = favorites.has(String(m.repo_id || ''));
    const isSelectMode = document
    ...
    const _downloadingPill = _isDownloading
      ...
      : '';
    const _favoritePill = _isFavorite
      ? ' <span class="memory-cat-badge memory-cat-pinned cookbook-serve-fav-badge">pinned</span>'
      : '';
  ...
  list.querySelectorAll('.hwfit-cached-menu-btn').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      ...
      const items = [];
      items.push({
        label: _favNow ? 'Unfavorite' : 'Favorite',
        icon: _favIco,
        action: 'favorite',
      });
      if (m && m.status === 'ready')
        items.push({ label: 'Serve', icon: _serveIco, action: 'serve' });
      if (m && m.status === 'downloading')
        items.push({ label: 'Retry', icon: _retryIco, action: 'retry' });
      if (m && m.status === 'ready')
        items.push({ label: 'Schedule…', icon: _schedIco, action: 'schedule' });
      ...
      for (const opt of items) {
        ...
        div.addEventListener('click', () => {
          ...
          if (opt.action === 'serve') item.click();
            ...
            uiModule.showToast(
              favored ? 'Favorited — pinned to top' : 'Unfavorited',
            );
            _rerenderCachedModels();
          } else if (opt.action === 'delete')
            _deleteCachedModel(repo, item, false, m);
  ...
  list.querySelectorAll('.memory-item[data-repo]').forEach((item) => {
    item.addEventListener('click', (e) => {
      ...
      let _allSs = {};
      try {
        _allSs = JSON.parse(localStorage.getItem(SERVE_STATE_KEY)) || {};
      } catch {}
      const _byRepo =
        (_allSs && typeof _allSs === 'object' && _allSs._byRepo) || {};
      const _lastUsed =
        (_allSs && typeof _allSs === 'object' && _allSs._lastUsed) || null;
      const _isLegacyFlat =
        _allSs &&
        typeof _allSs === 'object' &&
        !_allSs._byRepo &&
        !_allSs._lastUsed;
      const ss =
        _byRepo[repo] && typeof _byRepo[repo] === 'object'
          ? _byRepo[repo]
      ...
      const svm = (k, def) =>
        _modelSs && _hasOwn(_modelSs, k) ? _modelSs[k] : def;
      ...
      let defaultBackend =
        _repoForcedBackend && ss.backend && _allowedBackends.has(ss.backend)
          ? ss.backend
          : detectedBackend;
      if (!_allowedBackends.has(defaultBackend))
        defaultBackend = _backendChoices[0]?.[0] || detectedBackend;
      const savedMatchesBackend =
        _repoForcedBackend || (ss.backend || 'vllm') === detectedBackend;
      const sv = (k, def) =>
        ss[k] !== undefined && savedMatchesBackend ? ss[k] : def;
      const defaultTp =
        defaultBackend === 'llamacpp'
          ? '1'
          : sv('tp', _isMiniMaxMSeries ? '8' : '1');
      ...
      const tpOpts = [1, 2, 4, 8]
        .map(
          (n) =>
            `<option${defaultTp == String(n) ? ' selected' : ''}>${n}</option>`,
        )
        .join('');
      const dtypeOpts = ['auto', 'float16', 'bfloat16']
        .map(
          (d) =>
            `<option value="${d}"${sv('dtype', 'auto') === d ? ' selected' : ''}>${d}</option>`,
        )
        .join('');
      ...
      const _kvAutoDefault =
        (_kvOptsCheck && _kvOptsCheck.kvCacheDtype) ||
        (_isMiniMaxMSeries ? 'fp8' : 'auto');
      const _kvSelected = sv('vllm_kv_cache_dtype', _kvAutoDefault);
      const vllmKvCacheOpts = ['auto', 'fp8']
        .map(
          (d) =>
            `<option value="${d}"${_kvSelected === d ? ' selected' : ''}>${d}</option>`,
        )
        .join('');
      const _l = (name, tip) =>
        `<span>${name}<span class="hwfit-hint" title="${tip}">?</span></span>`;
      const _ggufChoices = _runnableGgufFiles(m);
      const _savedGguf = String(sv('gguf_file', '') || '');
      const _preferredGgufInclude = String(
        sv('_preferredGgufInclude', '') || '',
      )
        .replace(/\*/g, '')
        .toLowerCase();
      const _preferredGguf = _preferredGgufInclude
        ? _ggufChoices.find((f) =>
            String(f.rel_path || '')
              .toLowerCase()
              .includes(_preferredGgufInclude),
          ) ||
          _ggufChoices.find((f) =>
            String(f.name || '')
              .toLowerCase()
      ...
      const _ggufOptions = _ggufChoices
        .map(
          (f) =>
            `<option value="${esc(f.rel_path)}"${f.rel_path === _defaultGguf ? ' selected' : ''}>${esc(_ggufFileLabel(f))}</option>`,
        )
        .join('');
      const _minimaxM3Snapshot =
        '/home/pewds/.cache/huggingface/hub/models--cyankiwi--MiniMax-M3-AWQ-INT4/snapshots/4082acbbec1236d21828d55b6bb0fe02ade4ab5b';
      const _defaultServeModel = _isMiniMaxM3
        ? _minimaxM3Snapshot
        : m.is_local_dir && m.path
          ? `${m.path}/${repo}`
          : repo;
      const _savedModelPath = String(
        svm('model_path', _defaultServeModel) || '',
      ).trim();
      const _modelPathValue =
        _isMiniMaxM3 && (!_savedModelPath || _savedModelPath === repo)
          ? _minimaxM3Snapshot
          : _savedModelPath;
      ...
      const _arrowLabel =
        _modelPresets.length > 0 ? `${_modelPresets.length} ▾` : '▾';
      const _arrowTitle =
        _modelPresets.length > 0
      ...
      let _slotsHtml =
        `<div class="cookbook-serve-slots cookbook-saved-split" title="Saved launch configurations for this model — click ▾ to load or delete">` +
      ...
      const backendOpts = _backendChoices
        .map(
          ([v, l]) =>
            `<option value="${v}"${defaultBackend === v ? ' selected' : ''}>${l}</option>`,
        )
        .join('');
      ...
      // hides it when the user switches to vLLM/SGLang/Ollama.
      {
        ...
        const _hwBackend = String(
          _hwfitCache?.system?.backend || '',
        ).toLowerCase();
        const _hwScanMatch =
          String(_hwfitCache?._scannedHost || '') ===
          String(_envState.remoteHost || '');
        const _llamaModeDefault =
          _hwScanMatch &&
          ['cuda', 'rocm', 'vulkan', 'metal', 'mps', 'apple'].includes(
            _hwBackend,
          )
            ? 'gpu'
            : 'cpu';
        ...
        const _llamaMode =
          _savedUnified && _llamaModeRaw !== 'cpu' ? 'unified' : _llamaModeRaw;
      ...
      const _ctxDefault = _knownCtxDefault
        ? String(_knownCtxDefault)
        : m.context_length || m.context || '20000';
      const _ctxSavedValue = sv('ctx', _ctxDefault);
      const _ctxValue =
        _isMiniMaxMSeries && ['20000', '32768'].includes(String(_ctxSavedValue))
          ? _ctxDefault
          : _ctxSavedValue;
      ...
      const vllmAttnBackendOpts = [
        ...
        .map(
          (b) =>
            `<option value="${b === 'auto' ? '' : b}"${_attnSelected === (b === 'auto' ? '' : b) ? ' selected' : ''}>${b}</option>`,
        )
        .join('');
      ...
      panelHtml += `<label class="hwfit-backend-vllm">${_l('Swap', 'vLLM CPU swap space in GB. Blank/off omits the flag; enter a positive number only for older vLLM runtimes that support --swap-space.')}<input type="text" class="hwfit-sf" data-field="swap" value="${esc(sv('swap', ''))}" placeholder="off" /></label>`;
      {
        ...
        const _envPresetOpts = [
          ...
          .map(
            ([v, label]) =>
              `<option value="${v}"${_envPresetVal === v ? ' selected' : ''}>${label}</option>`,
          )
          .join('');
      ...
      const _rp_flag = _opts2_row3.flags.find((f) =>
        f.includes('--reasoning-parser'),
      );
      ...
      panelHtml += `<label class="hwfit-sf-cb hwfit-backend-vllm"><input type="checkbox" class="hwfit-sf" data-field="disable_custom_all_reduce"${sv('disable_custom_all_reduce', _isMiniMaxM3) ? ' checked' : ''} /> Disable Custom All Reduce${_h('vLLM --disable-custom-all-reduce. Useful for some 8-GPU/nightly configurations.')}</label>`;
      {
        ...
        if (!_specMethods.includes(_specMethod))
          _specMethods.unshift(_specMethod);
        const _specOpts = _specMethods
          .map(
            (m) =>
              `<option value="${m}"${m === _specMethod ? ' selected' : ''}>${m}</option>`,
          )
          .join('');
      ...
      const _kvOpts = ['', 'q4_0', 'q8_0', 'f16']
        .map(
          (k) =>
            `<option value="${k}"${sv('cache_type', '') === k ? ' selected' : ''}>${k || 'default'}</option>`,
        )
        .join('');
      const llamaFitOpts = ['', 'off', 'on']
        .map(
          (d) =>
            `<option value="${d}"${sv('llama_fit', '') === d ? ' selected' : ''}>${d || 'default'}</option>`,
        )
        .join('');
      const llamaSplitModeOpts = ['', 'layer', 'tensor', 'row', 'none']
        .map(
          (d) =>
            `<option value="${d}"${sv('llama_split_mode', '') === d ? ' selected' : ''}>${d || 'default'}</option>`,
        )
        .join('');
      ...
      function updateCmd() {
        ...
        const serveModel =
          (f.model_path || '').trim() ||
          (m.is_local_dir && m.path ? `${m.path}/${repo}` : repo);
        if (backend === 'llamacpp') {
        ...
        if (f.vllm_env_preset === 'minimax_m3_cuda') {
          ...
          if (!hasEnv('VLLM_TARGET_DEVICE'))
            envParts.unshift('VLLM_TARGET_DEVICE=cuda');
          if (!hasEnv('VLLM_USE_FLASHINFER_SAMPLER'))
            envParts.push('VLLM_USE_FLASHINFER_SAMPLER=0');
        ...
        _ce2.value = _formatServeCmdPreview(cmd);
        _ce2.style.height = 'auto';
        _ce2.style.height = _ce2.scrollHeight + 'px';
        panel._cmd = cmd;
        panel._host = f.host || '';
        return cmd;
      }
      ...
      function _updateRecommendedCtx(apply = true) {
        ...
        if (backend === 'vllm' || backend === 'sglang') {
          fit = _estimateVllmContextFit(
            m,
            f,
            panel._modelCtxMax,
            panel._modelWeightsGb,
            panel._fitSystem,
          );
        } else if (backend === 'llamacpp' || backend === 'ollama') {
          const ggufGb = _selectedGgufSizeGb(m, f.gguf_file);
          fit = _estimateLlamaContextFit(
            m,
            f,
            panel._modelCtxMax,
            ggufGb || panel._modelWeightsGb,
            panel._fitSystem,
        ...
        if (_ctxAutoNote) {
          ...
          const _llamaMemoryLabel =
            String(f.llama_mode || '').toLowerCase() === 'unified' ||
            f.unified_mem
      ...
      async function _loadContextProfile() {
        ...
        const profileModelPath = panel
          .querySelector('[data-field="model_path"]')
          ?.value?.trim();
        if (profileModelPath && profileModelPath !== repo)
          params.set('model_path', profileModelPath);
        if (host) {
          params.set('host', host);
          const _sp = (
            _serverByVal?.(target.serverKey || host) ||
            (_es.servers || []).find((s) => s.host === host) ||
            {}
          ).port;
      ...
      if (_ctxEl0) {
        _ctxEl0.addEventListener('input', () => {
          _ctxEl0.dataset.autoCtx = '0';
        });
        _ctxEl0.addEventListener('change', () => {
          _ctxEl0.dataset.autoCtx = '0';
          _clampCtx(false);
      ...
      (async () => {
        try {
          ...
          /* clamp falls back to the static default */
        }
      })();
      ...
      const _BACKEND_GLYPHS = {
        ...
        sglang:
          '<span aria-hidden="true" style="display:block;width:14px;height:14px;background:currentColor;-webkit-mask:url(/static/icons/sglang-mark.png) center/contain no-repeat;mask:url(/static/icons/sglang-mark.png) center/contain no-repeat;"></span>',
        ...
        llamacpp:
          '<svg width="14" height="14" viewBox="0 0 600 600" fill="none" aria-hidden="true"><path d="M600 392L504.249 558L504.137 557.929C487.252 584.069 458.193 600 426.864 600H120L240 392H600Z" fill="currentColor"/><path d="M240 392H0L199.602 46.0254C216.032 17.5463 246.411 0 279.29 0H466.154L240 392Z" fill="currentColor"/></svg>',
        ollama:
          '<img src="/static/icons/ollama-mark-crop.png" alt="" aria-hidden="true" width="14" height="14" style="display:block;width:14px;height:14px;object-fit:contain;" />',
        diffusers:
          '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="4"/><path d="M12 2v3M12 19v3M2 12h3M19 12h3M5 5l2 2M17 17l2 2M5 19l2-2M17 7l2-2"/></svg>',
      };
      ...
      const _backendBtnIconSlot = _backendBtn?.querySelector(
        '[data-backend-icon-slot]',
      );

      function _setBackendBtnState(v) {
        if (!_backendBtn) return;
        const opt = _backendSource?.querySelector(
          `option[value="${CSS.escape(v)}"]`,
        );
        const label = opt ? opt.textContent : v;
        if (_backendBtnLabel) _backendBtnLabel.textContent = label;
        if (_backendBtnIconSlot)
          _backendBtnIconSlot.innerHTML =
            _BACKEND_GLYPHS[v] || _BACKEND_GLYPHS.vllm;
      }

      function _renderBackendMenu() {
        if (!_backendMenu || !_backendSource) return;
        const items = Array.from(_backendSource.options).map((o) => ({
          value: o.value,
          label: o.textContent,
        }));
        _backendMenu.innerHTML = items
          .map(
            (it) => `
      ...
      if (_backendBtn) {
        ...
        document.addEventListener('click', (ev) => {
          if (!_backendMenu.hidden && !_backendPicker?.contains(ev.target))
            _closeBackendMenu();
        });
      ...
      function updateBackendVisibility() {
        const b =
          panel.querySelector('[data-field="backend"]')?.value || 'vllm';
      ...
      async function updateRuntimeReadinessNote() {
        ...
        const titleChip = titleEl
          ? titleEl.querySelector('.hwfit-serve-runtime-chip')
          : null;
        ...
        const _writeNote = (s) => {
          if (noteText) noteText.textContent = s;
          else note.textContent = s;
        };
        if (
          !['vllm', 'sglang', 'llamacpp', 'mlx', 'diffusers'].includes(backend)
        ) {
        ...
        try {
          ...
          if (pkg?.installed === null || pkg?.probe_error) {
            ...
            if (noteText) {
              ...
              const repo =
                panel.closest('.doclib-card, .memory-item')?.dataset?.repo ||
                '';
              const link = document.createElement('a');
              link.href = '#';
              link.textContent = ' Install in Dependencies →';
              link.style.cssText =
                'color:var(--accent, var(--red));text-decoration:underline;font-weight:600;margin-left:4px;';
              link.addEventListener('click', (ev) => {
                ev.preventDefault();
                if (pkgName)
                  openCookbookDependencies(pkgName, {
                    expandRecipe: pkgName,
                    model: repo,
                  });
              });
            ...
            note.style.borderColor =
              'color-mix(in srgb, var(--green, #4caf50) 40%, transparent)';
            note.style.background =
              'color-mix(in srgb, var(--green, #4caf50) 8%, transparent)';
          }
        } catch (err) {
          if (panel._runtimeReadinessSeq !== seq) return;
      ...
      function _loadSlotIntoPanel(slotIdx) {
        ...
        if (_gf) _gf.value = activeGpus.join(',');
        {
          ...
          const unifiedHidden = panel.querySelector(
            '[data-field="unified_mem"]',
          );
          const loadedUnified = ['1', 'true', 'yes', 'on'].includes(
            String(unifiedHidden?.value || '').toLowerCase(),
          );
          const loadedMode =
            loadedUnified && modeHidden?.value !== 'cpu'
              ? 'unified'
              : modeHidden?.value || 'gpu';
          if (modeHidden) modeHidden.value = loadedMode;
          if (unifiedHidden)
            unifiedHidden.value = loadedMode === 'unified' ? '1' : '';
      ...
      async function _saveCurrentConfig() {
        ...
        presets.push(
          _redactServeStateForStorage({
            name: shortName,
            model: repo,
            cmd,
            remoteHost: host,
            port: fields.port || '8000',
            label,
            fields,
          }),
        );
      ...
      function _showSavedConfigMenu(anchor) {
        document.querySelectorAll('.cookbook-saved-menu').forEach((d) => {
          if (typeof d._dismiss === 'function') d._dismiss();
          else d.remove();
        });
        const modelSlots = _presetsForModel(_loadPresets(), repo)
          ...
          .sort((a, b) => {
            const favDelta =
              (b.preset.favorite ? 1 : 0) - (a.preset.favorite ? 1 : 0);
        ...
        modelSlots.forEach(({ preset: p, slotIdx }, idx) => {
          ...
          it.className =
            ...
            (p.favorite ? ' cookbook-saved-favorite' : '');
          it.style.cssText =
          ...
          fav.type = 'button';
          fav.className =
            'cookbook-saved-fav-btn' + (p.favorite ? ' active' : '');
          ...
          if (p.favorite) {
            ...
            badge.className =
              'memory-cat-badge memory-cat-pinned cookbook-saved-fav-badge';
          ...
          it.addEventListener('click', (e) => {
            if (e.target === del || e.target === fav || fav.contains(e.target))
              return;
            e.stopPropagation();
            ...
            if (_cmdBox) {
              ...
              setTimeout(
                () => _cmdBox.classList.remove('cookbook-cmd-flash'),
                600,
              );
            }
          });
          fav.addEventListener('click', (e) => {
            ...
            if (target) {
              ...
              uiModule.showToast(
                target.favorite ? 'Favorited — pinned to top' : 'Unfavorited',
              );
      ...
      if (_launchMoreBtn) {
        _launchMoreBtn.addEventListener('click', (ev) => {
          ...
          ev.stopPropagation();
          document
            .querySelectorAll('.cookbook-launch-actions-menu')
            .forEach((m) => {
          ...
          menu.className =
            'cookbook-task-dropdown cookbook-launch-actions-menu';
          ...
          menu.appendChild(
            mk('Copy launch command', '', () => {
              updateCmd();
              const cmdBox = panel.querySelector('.hwfit-serve-cmd');
              const cmd =
                _cmdManuallyEdited && cmdBox
                  ? cmdBox.value
                  : _formatServeCmdPreview(panel._cmd || cmdBox?.value || '');
              _copyText(cmd).then(() =>
                uiModule.showToast('Launch command copied'),
              );
            }),
          );
          menu.appendChild(
            mk('Schedule', '', () => {
              const direct = new MouseEvent('click', {
                bubbles: true,
                cancelable: true,
              });
          ...
          menu.appendChild(
            mk('Clear Server', 'cookbook-dropdown-danger', () =>
              _clearBtn?.click(),
            ),
          );
          ...
          closeMenu = bindMenuDismiss(
            ...
            (e) => !menu.contains(e.target) && e.target !== _launchMoreBtn,
          );
        ...
        group.querySelectorAll('.mode-toggle-btn').forEach((btn) => {
          btn.addEventListener('click', (e) => {
            ...
            const hidden = group.parentElement.querySelector(
              '[data-field="llama_mode"]',
            );
            if (hidden) {
              hidden.value = want;
              hidden.dispatchEvent(new Event('change', { bubbles: true }));
            }
            const unified = group.parentElement.querySelector(
              '[data-field="unified_mem"]',
            );
            if (unified) {
      ...
      {
        const _saved =
          panel.querySelector('[data-field="llama_mode"]')?.value || 'gpu';
      ...
      panel
        .querySelector('.hwfit-serve-cancel')
      ...
      // Launch button
      panel
        .querySelector('.hwfit-serve-launch')
        .addEventListener('click', async (ev) => {
          ...
          const _rawLaunchCmd =
            _cmdManuallyEdited && _cmdTextarea
              ? _cmdTextarea.value
              : panel._cmd;
          ...
          panel.querySelectorAll('.hwfit-sf').forEach((el) => {
            if (el.type === 'checkbox')
              serveState[el.dataset.field] = el.checked;
            else serveState[el.dataset.field] = el.value;
          });
          serveState.backend =
            serveState.backend || _detectBackend(m).backend || 'vllm';
          const launchTarget = _selectedServeTarget(panel);
          if (
            serveState.backend === 'diffusers' &&
            _remoteWindowsDiffusersUnsupported(launchTarget)
          ) {
            _restoreLaunchBtn();
            uiModule.showToast(
              'Diffusers serving is not supported on remote Windows servers yet. Use local Windows or a Linux server.',
              9000,
            );
            return;
          }
          ...
          try {
            ...
            const _active = (
              _runningMod._loadTasks ? _runningMod._loadTasks() : []
            ).filter(
              (t) =>
                t &&
                t.type === 'serve' &&
                ((t.remoteHost || '') === _hostStr ||
                  (t.remoteServerKey || '') === _serverKeyStr) &&
                (t.status === 'running' ||
                  t.status === 'ready' ||
                  t._serveReady),
            );
            if (_active.length) {
              const _names = _active
                .map((t) => t.payload?.repo_id || t.repo || t.name || '?')
                .filter(Boolean);
                ...
                try {
                  ...
                  if (_btn) {
                    ...
                    await fetch('/api/shell/exec', {
                      ...
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({
                        command: _runningMod._tmuxGracefulKill(t),
                      }),
                    });
                  }
                } catch (_killErr) {
          ...
          const backendWarning = _serveBackendWarning(
            m,
            repo,
            serveState.backend,
            serveState,
          ...
          if (
            ...
            try {
              ...
              const _modelBytes =
                Number(m?.size_bytes || 0) ||
                Math.round(Number(m?.size_gb || 0) * 1024 * 1024 * 1024);
              ...
              try {
                ...
                if (_gh) {
                  _gp.set('host', _gh);
                  const _sp = (
                    _serverByVal?.(launchTarget.serverKey || _gh) || {}
                  ).port;
                  if (_sp) _gp.set('ssh_port', _sp);
                }
                const _gr = await fetch(
                  '/api/cookbook/gpus' + (_gp.toString() ? '?' + _gp : ''),
                  { credentials: 'same-origin' },
                );
                if (_gr.ok) {
              ...
              if (_selFreeGb > 0 && _needGb > _selFreeGb && _modelGb > 0) {
                ...
                const _recCtxRaw = Math.floor(
                  (_selFreeGb - _modelGb) / _kvGbPerToken,
                );
                const _recCtx = Math.max(
                  1024,
                  Math.floor(_recCtxRaw / 1024) * 1024,
                );
                ...
                const _action = await new Promise((resolve) => {
                  ...
                  ov.className = 'modal';
                  ov.style.cssText =
                    'display:flex;align-items:center;justify-content:center;z-index:10050;position:fixed;inset:0;background:rgba(0,0,0,0.4);';
                  const _btnRow = [];
                  if (_recCtx > 1024 && _recCtx < _ctx) {
                    _btnRow.push(
                      `<button data-vram-action="reduce" class="confirm-btn confirm-btn-primary" style="width:100%;">Reduce ctx to ${_recCtx.toLocaleString()}</button>`,
                    );
                  }
                  if (_canAddGpu) {
                    _btnRow.push(
                      `<button data-vram-action="add_gpus" class="confirm-btn confirm-btn-primary" style="width:100%;">Add GPU${_addGpus.length > 1 ? 's' : ''} ${_addGpus.join(', ')}</button>`,
                    );
                  }
                  _btnRow.push(
                    `<button data-vram-action="allow_cpu" class="confirm-btn confirm-btn-secondary" style="width:100%;">Allow CPU overflow (slow)</button>`,
                  );
                  _btnRow.push(
                    `<button data-vram-action="cancel" class="confirm-btn confirm-btn-secondary" style="width:100%;">Cancel</button>`,
                  );
                  ov.innerHTML =
                    '<div class="modal-content" style="max-width:480px;">' +
                    '<div class="modal-header"><h4>Will not fit on selected GPU' +
                    (_selIdx.length > 1 ? 's' : '') +
                    '</h4></div>' +
                    '<div class="modal-body" style="font-size:12px;line-height:1.5;">' +
                    '<p>Model + KV cache would overflow VRAM on the selected GPU' +
                    (_selIdx.length > 1 ? 's' : '') +
                    '. llama-cpp-python will silently spill to CPU → very slow inference.</p>' +
                ...
                if (_action === 'reduce') {
                  ...
                  if (_ctxEl) {
                    ...
                    _ctxEl.dispatchEvent(
                      new Event('change', { bubbles: true }),
                    );
                  }
                } else if (_action === 'add_gpus') {
                  for (const i of _addGpus) {
                    const _b = panel.querySelector(
                      `.cookbook-gpu-btn[data-gpu="${i}"]`,
                    );
                  ...
                  const _ov = panel.querySelector(
                    '[data-field="llama_cpu_overflow"]',
                  );
                  if (_ov) {
                    _ov.checked = true;
          ...
          const _needsGpu =
            ['vllm', 'sglang'].includes(serveState.backend) ||
            serveState.backend === 'diffusers';
          if (_needsGpu) {
            try {
              ...
              if (_probeHost) {
                ...
                if (launchTarget.port)
                  _probeParams.set('ssh_port', launchTarget.port);
              }
              const _probeRes = await fetch(
                '/api/cookbook/gpus' +
                  (_probeParams.toString() ? '?' + _probeParams : ''),
                { credentials: 'same-origin' },
              );
              const _probeData = await _probeRes.json();
              const _probeGpus = Array.isArray(_probeData)
                ? _probeData
                : _probeData.gpus || [];
          ...
          try {
            const _portMatch =
              launchCmd.match(
                /(?:^|\s)(?:--port|-p|--host\s+\S+\s+--port)\s+(\d{2,5})\b/,
              ) ||
            ...
            if (_port) {
              ...
              if (_stdout) {
                ...
                const _procMatch = _stdout.match(
                  /users:\(\("([^"]+)",pid=(\d+)/,
                );
          ...
          const venvVal = panel
            .querySelector('[data-field="venv"]')
            ?.value?.trim();
          const gpusVal = panel
            .querySelector('[data-field="gpus"]')
            ?.value?.trim();
          ...
          if (venvVal) {
            _envState.env = 'venv';
            _envState.envPath = venvVal;
          } else if (_srvEnvPath) {
            _envState.env = _srvEnv === 'conda' ? 'conda' : 'venv';
            _envState.envPath = _srvEnvPath;
          }
          ...
          try {
            ...
            const _wantsGpu = [
              ...
              'sglang',
              'diffusers',
            ].includes(serveState.backend);
            const _detectedBackend = String(
              _hwfitCache?.system?.backend || '',
            ).toLowerCase();
            const _gpuBackends = [
              'cuda',
            ...
            if (
              ...
              _wantsGpu &&
              _detectedBackend &&
              !_gpuBackends.includes(_detectedBackend)
            ) {
              const _proceed = await window.styledConfirm(
              ...
              if (!_proceed) {
                if (typeof _restoreLaunchBtn === 'function')
                  _restoreLaunchBtn();
          ...
          try {
            await _withSpinner(_launchBtn, async () => {
              ...
              const taskDisplayName = _serveTaskDisplayName(
                shortName,
                m,
                serveState,
              );
              await _launchServeTask(
                ...
                serveState,
                serveHost,
                { serverKey: serveServerKey, serverName: serveServerName },
              );
            });
          } finally {
            _envState.env = origEnv;
      ...
      panel
        .querySelector('.hwfit-serve-copy')
        .addEventListener('click', (e) => {
          ...
          e.preventDefault();
          e.stopPropagation();
          const cmd = _cmdManuallyEdited
            ? panel.querySelector('.hwfit-serve-cmd').value
            : _formatServeCmdPreview(panel._cmd);
          _copyText(cmd).then(() => {
...
async function _deleteCachedModel(
  repo,
  itemEl,
  skipConfirm = false,
  model = null,
  ...
  if (!skipConfirm) {
    if (ggufFiles.length > 1) {
      ...
      !(await uiModule.styledConfirm(`Delete ${repo} from cache?`, {
        confirmText: 'Delete',
        danger: true,
      }))
    ) {
  ...
  if (_isWindows()) {
    ...
    if (deleteChoice.mode === 'files') {
      ...
      cmd = targets
        .map(
          (p) =>
            `Remove-Item -Force ${_psSingleQuote(p)} -ErrorAction SilentlyContinue`,
        )
        .join('; ');
    } else {
    ...
    const unixTarget = target.startsWith('~')
      ? target.replace(/^~/, '$HOME')
      : target;
    if (deleteChoice.mode === 'files') {
      const targets = deleteChoice.files
  ...
  try {
    ...
    if (!res.ok) {
      uiModule.showError(`Delete failed (${res.status})`);
      return;
    }
    if (deleteChoice.mode === 'files') {
      if (m && Array.isArray(m.gguf_files)) {
        const removed = new Set(
          deleteChoice.files.map((f) => _safeGgufRelPath(f.rel_path)),
        );
        m.gguf_files = m.gguf_files.filter(
          (f) => !removed.has(_safeGgufRelPath(f.rel_path)),
        );
      }
  ...
  if (fields && typeof fields === 'object') {
    try {
      ...
      localStorage.setItem(
        SERVE_STATE_KEY,
        JSON.stringify(
          _redactServeStateForStorage({ _byRepo: byRepo, _lastUsed: _seeded }),
        ),
      );
    } catch {}
  }
  ...
  try {
    ...
    if (selectedServer && Array.isArray(selectedServer.modelDirs)) {
      for (const d of selectedServer.modelDirs) {
        ...
        if (normalized && normalized !== '~/.cache/huggingface/hub')
          modelDirs.push(normalized);
      }
    }
    ...
    if (!allowNetwork) {
      ...
      list.innerHTML =
        '<div class="hwfit-loading serve-empty-auto-scan" style="flex-direction:column;gap:8px;text-align:center;"><div class="serve-empty-auto-wp"></div><div>No cached model scan yet</div><div style="font-size:11px;opacity:0.55;max-width:420px;line-height:1.4;">Scanning this server\'s model cache…</div></div>';
      list.querySelector('.serve-empty-auto-wp')?.appendChild(wp.element);
      setTimeout(() => {
        if (list.querySelector('.serve-empty-auto-scan'))
          _fetchCachedModels(true);
      }, 60);
      const tagContainer = document.getElementById('serve-tags');
