...
export async function init() {
  ...
  try {
    ...
    try {
      ...
      hideOn(
        '#tool-research-btn, #research-toggle-btn',
        privs.can_use_research,
      );
      ...
      if (privs.can_use_agent === false) {
        ...
        if (_chat) {
          _chat.classList.add('active');
          _chat.click?.();
        }
      }
    } catch (_) {
      /* DOM not ready or unexpected shape — UI gates are non-fatal */
    }
  } catch (_) {
    /* anonymous / loopback mode — nothing to do */
  }
  ...
  if (rail && typeof MutationObserver !== 'undefined') {
    new MutationObserver(_sync).observe(rail, {
      attributes: true,
      attributeFilter: ['class', 'style'],
    });
  }
...
function clearFreshComposerRestore() {
  ...
  const hasSessionTarget = !!(
    window.location.hash || Storage.get('lastSessionId')
  );
