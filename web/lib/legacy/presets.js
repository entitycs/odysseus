...
function initCharTabs() {
  document.querySelectorAll('.preset-tab[data-chartab]').forEach((tab) => {
    tab.addEventListener('click', () => {
      ...
      document
        .querySelectorAll('.preset-tab[data-chartab]')
        .forEach((t) => t.classList.toggle('active', t === tab));
      document
        .querySelectorAll('.preset-chartab[data-chartab-panel]')
        .forEach((p) => {
...
function initExpandButton() {
  ...
  btn.addEventListener('click', async () => {
    ...
    btn.innerHTML =
      '<svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor" style="vertical-align:-1px;margin-right:2px;"><path d="M12 0L14.59 8.41L23 12L14.59 15.59L12 24L9.41 15.59L1 12L9.41 8.41Z"/></svg> Expanding...';
...
function initNameDropdown() {
  ...
  select.addEventListener('change', () => {
    ...
    if (!val || val === '__default__') {
      ...
      if (tokensInput) {
        tokensInput.value = 8448;
        if (tokensValue) tokensValue.textContent = 'No limit';
    ...
    const hasName =
      isSaved || (builtin && builtin.isCharacter && !builtin.noName);
    if (nameInput) nameInput.value = hasName ? val : '';
    const nameRow = document.getElementById('char-name-row');
    if (nameRow)
      nameRow.style.display = builtin && builtin.noName ? 'none' : '';
    _tryLoadTemplate(val);
    const isPreset = builtin && builtin.isPreset;
    if (delBtn)
      delBtn.style.display = isSaved || (builtin && !isPreset) ? '' : 'none';
  });
  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (match) {
          await fetch(`${API_BASE}/api/presets/templates/${match.id}`, {
            method: 'DELETE',
          });
        }
        ...
        if (presets.custom && presets.custom.character_name === charName) {
          ...
          const charIndicator = document.getElementById(
            'character-indicator-btn',
          );
          if (charIndicator) {
            charIndicator.style.display = 'none';
            charIndicator.classList.remove('active');
          }
  ...
  if (!tmpl) {
    ...
    if (builtin) {
      ...
      if (tempInput && builtin.temperature != null) {
        ...
        if (tempValue)
          tempValue.textContent = parseFloat(builtin.temperature).toFixed(1);
        tempInput.dispatchEvent(new Event('input'));
      }
      return;
    }
  ...
  if (tokensInput) {
    ...
    if (tokensValue)
      tokensValue.textContent =
        v === 0 || v > 8192 ? 'No limit' : v.toLocaleString();
    tokensInput.dispatchEvent(new Event('input'));
  }
  const delBtn = document.getElementById('char-delete-template-btn');
...
function _populateCharSelect() {
  ...
  select.innerHTML =
    '<option value="__default__">Default (no persona)</option>';
  ...
  const builtins = PROMPT_TEMPLATES.filter(
    (t) => !savedNames.has(t.name) && !hiddenPresets.includes(t.name),
  );
  if (builtins.length) {
    const group = document.createElement('optgroup');
    group.label = 'Presets';
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const sessionModule = await import('$lib/legacy/sessions.js');
      const sessions = sessionModule.getSessions();
      const current = sessions.find(
        (s) => s.id === sessionModule.getCurrentSessionId(),
      );
      ...
      await fetch(`${API_BASE}/api/session/${sessionId}/important`, {
        method: 'POST',
        body: favFd,
      });
      ...
      localStorage.setItem(
        'odysseus-char-sessions',
        JSON.stringify(charSessions),
      );
      ...
      setTimeout(() => {
        btn.textContent = 'Create Persistent Chat';
      }, 1500);
    } catch (e) {
...
export function openCustomPresetModal() {
  ...
  function _updateStartBtn() {
    ...
    if (!btn) return;
    const changed =
      (nameInput && nameInput.value !== _snapshot.name) ||
    ...
    const activeTab =
      document.querySelector('.preset-tab.active')?.dataset.chartab || 'inject';
    let label;
    if (activeTab === 'group') {
      label = 'Start Group';
    ...
    if (cancelBtn) {
      ...
      const featOn =
        activeTab === 'group'
          ? groupOn
          : !!(presets.custom && presets.custom.enabled);
    ...
    if (resetBtn)
      resetBtn.style.display =
        changed && activeTab === 'character' ? '' : 'none';
  }
  ...
  document.querySelectorAll('.preset-tab[data-chartab]').forEach((tab) => {
    if (tab._startLabelSync)
      tab.removeEventListener('click', tab._startLabelSync);
  ...
  if (_cancelBtn && !_cancelBtn._wired) {
    ...
    _cancelBtn.addEventListener('click', () => {
      const t =
        document.querySelector('.preset-tab.active')?.dataset.chartab ||
        'inject';
      if (t === 'group') {
        try {
          if (window.groupModule && window.groupModule.stopGroup)
        ...
        try {
          fetch(`${API_BASE}/api/presets/custom`, {
            ...
            body: JSON.stringify({
              ...(presets.custom || {}),
              name: (presets.custom && presets.custom.character_name) || '',
              enabled: false,
            }),
          }).catch(() => {});
        } catch {}
  ...
  function _syncCharRows() {
    ...
    const delBtn = document.getElementById('char-delete-template-btn');
    if (delBtn)
      delBtn.style.display = userTemplates.find(
        (t) => t.name === (nameInput ? nameInput.value.trim() : ''),
      )
        ? ''
        : 'none';
  ...
  if (isPersistent) {
    ...
    if (!lockNotice) {
      ...
      notice.style.cssText =
        'font-size:11px;color:var(--color-muted);text-align:center;padding:6px;margin-bottom:8px;border:1px dashed var(--border);border-radius:6px;';
      notice.textContent =
...
export async function saveCustomPreset(showToast, showError) {
  ...
  try {
    ...
    if (result.success) {
      ...
      const _selVal =
        document.getElementById('char-template-select')?.value || '';
      const isBuiltinPreset = PROMPT_TEMPLATES.some(
        (t) => t.isPreset && (t.name === name || t.name === _selVal),
      );
...
export function deactivateCharacter() {
  ...
  if (charInd) {
    charInd.style.display = 'none';
    charInd.classList.remove('active');
  }
  const miniBtn = document.getElementById('overflow-preset-btn');
  if (miniBtn) miniBtn.classList.remove('active');
}
...
function _syncCharIndicator() {
  ...
  const _hasTuning =
    (!isNaN(_t) && _t !== 1.0) ||
    (!!custom?.max_tokens && custom.max_tokens !== 0);
  const _hasInject = !!(custom?.inject_prefix || custom?.inject_suffix);
  const injectActive =
    enabled && !custom?.character_name && (_hasTuning || _hasInject);
  ...
  if (hasChar || injectActive) {
    ...
    if (xIcon)
      xIcon.style.display = window._persistentChatSession ? 'none' : '';
    if (!btn._wired) {
      btn._wired = true;
      btn.addEventListener('click', (e) => {
        ...
        if (e.target.closest('.tool-indicator-x')) {
          ...
          fetch(`${API_BASE}/api/presets/custom`, {
            ...
            body: JSON.stringify({
              ...presets.custom,
              name: presets.custom.character_name || '',
              enabled: false,
            }),
          }).catch(() => {});
          return;
        }
        if (typeof openCustomPresetModal === 'function')
...
export function onSessionSwitch(sessionId) {
  ...
  if (charName) {
    ...
    const tmpl =
      userTemplates.find((t) => t.name === charName) ||
