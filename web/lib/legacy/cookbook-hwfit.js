...
import { _loadTasks, _tmuxGracefulKill } from '$lib/legacy/cookbookRunning.js';
...
function _wireServerColorPicker(entry) {
  ...
  menu.querySelectorAll('.cookbook-srv-color-item').forEach((item) => {
    item.addEventListener('click', (e) => {
      ...
      select.value = color;
      const label =
        item.querySelector('span:last-child')?.textContent || 'Auto';
      ...
      menu
        .querySelectorAll('.cookbook-srv-color-item')
        .forEach((b) => b.classList.toggle('active', b === item));
      close();
...
async function _ensureBackendInstalled(
  runBackend,
  host,
  port,
  envPath,
  ...
  try {
    ...
    const r = await fetch(
      '/api/cookbook/packages' + (params.toString() ? '?' + params : ''),
    );
...
function _looksLikeGgufRepo(model) {
  const haystack =
    `${model?.quant_repo || ''} ${model?.repo_id || ''} ${model?.path || ''} ${model?.name || ''}`.toLowerCase();
  return (
    !!model?.is_gguf || haystack.includes('gguf') || haystack.includes('.gguf')
  );
}
...
export function _renderGpuToggles(system) {
  ...
  const grp = groups[container._activeGroup] ||
    groups[0] || { count: total, vram_each: 0, name: system.gpu_name || 'GPU' };
  ...
  html +=
    '<button class="hwfit-gpu-btn" data-count="0" title="CPU / RAM only">RAM</button>';
  ...
  if (!container._gpuBound) {
    ...
    container.addEventListener('click', (e) => {
      ...
      const wasActive =
        btn.classList.contains('active') && container._activeCount === count;
      container
        .querySelectorAll('.hwfit-gpu-btn')
...
function _ctxValue() {
  ...
  const idx = Math.max(
    0,
    Math.min(_CTX_PRESETS.length - 1, Number(slider?.value ?? 3) || 0),
  );
...
function _manualDisplaySystem(sys, manual) {
  ...
  if (manual.mode !== 'ram') {
    ...
    base.gpu_name =
      `Simulated ${backend} GPU` + (count > 1 ? ` × ${count}` : '');
...
function _writeScanCache(sig, data) {
  try {
    ...
    all[sig] = {
      ts: Date.now(),
      data: { system: data.system, models: data.models },
    };
    const keys = Object.keys(all);
    if (keys.length > _SCAN_CACHE_MAX) {
      keys.sort((a, b) => (all[a].ts || 0) - (all[b].ts || 0));
      for (const k of keys.slice(0, keys.length - _SCAN_CACHE_MAX))
        delete all[k];
    }
...
function _hwfitShowError(list, host, detail) {
  ...
  div.innerHTML =
    ...
    (detail
      ? `<div style="opacity:0.6;font-size:11px;max-width:340px;line-height:1.4;">${esc(detail)}</div>`
      : '') +
...
function _applyEngineFilter(models) {
  ...
  return models.filter((m) => {
    try {
      return _detectBackend(m).backend === want;
    } catch {
      return true;
    }
  });
  ...
  for (const m of libModels) {
    ...
    for (const sz of sizes) {
      ...
      if (vramGb && vramAvail) {
        ...
        else if (ramAvail && vramGb <= ramAvail)
      ...
      const paramsLabel = params
        ? params >= 1
          ? params.toFixed(params >= 10 ? 0 : 1) + 'B'
          : (params * 1000).toFixed(0) + 'M'
        : '?';
...
export async function _hwfitFetch(fresh = false) {
  ...
  if (_cached) {
    ...
    const canKeepPrevious =
      keepPrevious && _hwfitCache && Array.isArray(_hwfitCache.models);
    if (canKeepPrevious) {
      try {
        wp.destroy();
      } catch {}
      ...
      loadingLbl.style.cssText =
        'font-size:11px;opacity:0.55;max-width:420px;line-height:1.4;';
    ...
    if (!canKeepPrevious) {
      ...
      loadingLbl.style.cssText =
        'text-align:center;opacity:0.5;font-size:11px;';
      loadingDiv.appendChild(loadingLbl);
      setTimeout(() => {
        if (loadingLbl.isConnected)
          loadingLbl.textContent = fresh
            ? 'Scanning hardware…'
            : 'Loading model list…';
      }, 2000);
      list.innerHTML = '';
    ...
    fetch(`/api/model/cached?${_cacheParams}`, { credentials: 'same-origin' })
      ...
      .then((d) => {
        ...
        list.querySelectorAll('.hwfit-row[data-model]').forEach((row) => {
          ...
          if (
            _cachedModelIds.has(name) ||
            [..._cachedModelIds].some((id) =>
              id.endsWith('/' + name?.split('/').pop()),
            )
          ) {
            ...
            if (nameEl && !nameEl.querySelector('.hwfit-dl-dot')) {
              nameEl.insertAdjacentHTML(
                'beforeend',
                '<span class="hwfit-dl-dot" title="Downloaded">\u25CF</span>',
              );
            }
          }
        });
  ...
  try {
    ...
    if (
      !hasManualOrDismissed &&
      toggleContainer &&
      typeof toggleContainer._activeCount === 'number'
    ) {
    ...
    if (_dismissedHwChips.has('gpu') || _dismissedHwChips.has('vram'))
      params.set('ignore_detected_gpu', 'true');
    ...
    if (!isImageMode) {
      ...
      const _fitOnly = (() => {
        try {
          return localStorage.getItem('hwfit_fit_only_v1') === '1';
        } catch {
          return false;
        }
      })();
    ...
    const endpoint = isImageMode
      ? `/api/hwfit/image-models?${params}`
      : `/api/hwfit/models?${params}`;
    ...
    if (data.error) {
      ...
      if (!_cached) {
        _hwfitShowError(list, remoteHost, data.error);
        if (hw) hw.innerHTML = '';
      }
      return;
    }
    ...
    if (!isImageMode) {
      ...
      const _olFiltered = _s
        ? _olRows.filter(
            (r) =>
              r.name.toLowerCase().includes(_s) ||
              (r._description || '').toLowerCase().includes(_s),
          )
        : _olRows;
...
function _renderHwVisibilityWarning(sys) {
  ...
  const warning = sys?.manual_hardware
    ? null
    : sys?.hardware_visibility_warning;
  ...
  box
    .querySelector('[data-hw-action="manual"]')
  ...
  box
    .querySelector('[data-hw-action="rescan"]')
    ?.addEventListener('click', () => {
      _resetGpuToggleState();
      _hwfitCache = null;
      _hwfitFetch(true);
  ...
  box
    .querySelector('[data-hw-action="copy"]')
    ?.addEventListener('click', () => {
...
export function _hwfitRenderHw(el, sys) {
  ...
  if (sys.gpu_name) {
    ...
    if (groups.length > 1) {
      ...
      label = groups
        .map((g) => `${g.count}× ${esc(_shortGpuName(g.name))}`)
        .join(' + ');
    } else if (gpuCount > 1) {
    ...
    const tip = gpus.length
      ? gpus
          .map(
            (g) => `GPU ${g.index}: ${g.name} · ${(+g.vram_gb).toFixed(1)} GB`,
          )
          .join('\n')
...
function _wireManualHardwareControls(el) {
  ...
  if (manual) {
    ...
    panel.querySelector('.hwfit-manual-backend').value =
      manual.backend || 'cuda';
  }
  const syncMode = () => {
    const isRam = panel.querySelector('.hwfit-manual-mode')?.value === 'ram';
    panel
      .querySelector('.hwfit-manual-gpus')
      ?.closest('label')
      ?.style.setProperty('display', isRam ? 'none' : '');
    panel
      .querySelector('.hwfit-manual-vram')
      ?.closest('label')
      ?.style.setProperty('display', isRam ? 'none' : '');
  ...
  el.querySelector(
    '.hwfit-hw-chip-toggle[data-hw-chip="manual"]',
  )?.addEventListener('click', () => {
  ...
  if (!panel._hwfitManualBound) {
    ...
    panel
      .querySelector('.hwfit-manual-mode')
      ?.addEventListener('change', syncMode);
    panel
      .querySelector('.hwfit-hw-manual-save')
      ?.addEventListener('click', () => {
        const mode = panel.querySelector('.hwfit-manual-mode')?.value || 'gpu';
        const gpuCount = _manualNumber(
          panel.querySelector('.hwfit-manual-gpus')?.value,
          1,
        );
        const vramGb = _manualNumber(
          panel.querySelector('.hwfit-manual-vram')?.value,
          8,
        );
        const ramGb = _manualOptionalNumber(
          panel.querySelector('.hwfit-manual-ram')?.value,
        );
        const backend =
          panel.querySelector('.hwfit-manual-backend')?.value || 'cuda';
        ...
        _hwfitRenderHw(
          el,
          _manualDisplaySystem(window._hwfitSystemCache, manual),
        );
        _hwfitFetch(true);
      });
    panel
      .querySelector('.hwfit-hw-manual-clear')
      ?.addEventListener('click', clearManual);
  }
  syncMode();
}

export const _fitColors = {
  perfect: 'var(--green, #50fa7b)',
  good: 'var(--yellow, #f1fa8c)',
  marginal: 'var(--orange, #ffb86c)',
  too_tight: 'var(--red, #ff5555)',
};

function _requiresAcceleratorBackend(model) {
  const q = String(model?.quant || model?.quantization || '').toUpperCase();
  const text =
    `${model?.name || ''} ${model?.repo_id || ''} ${model?.path || ''}`.toLowerCase();
  return (
    /^AWQ|^GPTQ|^NVFP4/.test(q) ||
    q === 'FP8' ||
    /\b(awq|gptq|fp8|nvfp4)\b/i.test(text)
  );
}
...
export function _hwfitRenderList(el, models) {
  ...
  if (!models.length) {
    ...
    const hasHw =
      sys && ((sys.gpu_vram_gb || 0) > 0 || (sys.total_ram_gb || 0) > 8);
    const hasFilters = !!(
      document.getElementById('hwfit-search')?.value?.trim() ||
    ...
    if (hasFilters)
      msg =
        'No models match these filters — try clearing the search, use-case, quant, or engine.';
    else if (hasHw)
      msg =
        'No models fit — the hardware probe may have under-reported. Try Rescan.';
  ...
  const tc = document.getElementById('hwfit-gpu-toggles');
  const _budget =
    tc && typeof tc._activeCount === 'number'
      ? tc._activeCount === 0
        ? 'RAM'
        : tc._activeCount === 1
          ? 'GPU'
  ...
  for (const m of models) {
    ...
    const moeBadge = m.is_moe
      ? '<span class="hwfit-badge hwfit-moe">MoE</span>'
      : '';
    const imgBadge = m.is_image_gen
      ? '<span class="hwfit-badge" style="background:color-mix(in srgb, var(--red) 20%, transparent);color:var(--red);font-size:8px;padding:1px 4px;border-radius:3px;margin-left:4px;">IMG</span>'
      : '';
    const dlDot =
      _cachedModelIds &&
      (_cachedModelIds.has(m.name) ||
        [..._cachedModelIds].some((id) => id === m.name?.split('/').pop()))
        ? '<span class="hwfit-dl-dot" title="Downloaded">\u25CF</span>'
        : '';
    ...
    if (_quantTag) {
      ...
      const _remaining = _parts.filter(
        (p) => !_lowerShort.includes(p.toLowerCase()),
      );
      if (_remaining.length && _remaining.length < _parts.length + 1) {
        // at least one part is new
  ...
  el.querySelectorAll('.hwfit-row:not(.hwfit-header)').forEach((row) => {
    row.addEventListener('click', () => {
      const name = row.dataset.model;
      if (!name) return;
      const modelData = (_hwfitCache?.models || []).find(
        (m) => m.name === name,
      );
      if (!modelData) return;
  ...
  el.querySelectorAll('.hwfit-header .hwfit-sortable').forEach((col) => {
    col.addEventListener('click', (e) => {
      ...
      if (e.target.closest('[data-fit-dot]')) {
        ...
        try {
          localStorage.setItem('hwfit_fit_only_v1', on ? '1' : '0');
        } catch {}
...
function _minBackendVersion(modelName, backend) {
  ...
  if (backend === 'vllm') {
    ...
    if (n.includes('deepseek') && (n.includes('v3') || n.includes('r1')))
      return '0.7.0';
    ...
    if (n.includes('glm-4.5') || n.includes('glm-4.6') || n.includes('glm-5'))
      return '0.8.0';
...
function _sglangHashFor(modelData) {
  const sys =
    (typeof _hwfitCache !== 'undefined' ? _hwfitCache?.system : null) || {};
...
export function _expandModelRow(row, modelData) {
  ...
  list
    .querySelectorAll('.hwfit-row-active')
    .forEach((r) => r.classList.remove('hwfit-row-active'));
  ...
  if (modelData.is_image_gen) {
    ...
    const _hasGpuAccel =
      !!_sys.has_gpu && (_backend === 'cuda' || _backend === 'rocm');
    if (!_hasGpuAccel) {
  ...
  if (dlBtn) {
    dlBtn.addEventListener('click', () => {
      ...
      if (backend === 'ollama') {
        _runPanelCmd(panel, _buildDownloadCmd(modelData, backend), {
          timeout: 0,
        });
      } else {
    ...
    quickRunBtn.addEventListener('click', async () => {
      ...
      const _downloaded =
        ...
        (_cachedModelIds.has(modelData.name) ||
          [..._cachedModelIds].some(
            (id) => id === modelData.name || id.endsWith('/' + _short),
          ));
      if (_cachedModelIds && !_downloaded) {
        uiModule.showToast(
          'Model not downloaded yet — starting download. Run again to serve once it finishes.',
        );
        if (backend === 'ollama') {
          _runPanelCmd(panel, _buildDownloadCmd(modelData, backend), {
            timeout: 0,
      ...
      try {
        ...
        const _activeServes = _loadTasks().filter(
          (t) =>
            t &&
            t.type === 'serve' &&
        ...
        if (_activeServes.length) {
          ...
          for (const t of _activeServes) {
            try {
              ...
              const _taskEl = document.querySelector(
                `.cookbook-task[data-task-id="${t.sessionId}"]`,
              );
              const _stopBtn = _taskEl?.querySelector(
                '.cookbook-task-action-stop',
              );
              if (_stopBtn) {
                _stopBtn.click();
              } else {
      ...
      if (_qrRunBackend === 'vllm' || _qrRunBackend === 'sglang') {
        ...
        if (_sys.gpu_error) {
          uiModule.showError(
            `Can't launch: GPU driver error — ${_sys.gpu_error}. Reinstall or repair the NVIDIA driver, then re-scan.`,
          );
          return;
        }
        if (!_sys.has_gpu || !(_sys.gpu_count > 0)) {
          uiModule.showError(
            `Can't launch: no GPU detected by nvidia-smi. ${_qrRunBackend === 'vllm' ? 'vLLM' : 'SGLang'} needs a working CUDA or ROCm device.`,
          );
          return;
        }
      }
        ...
        try {
          ...
          if (_chkRes.ok) {
            ...
            if (_out.includes('MISSING')) {
              ...
              uiModule.showError(
                `Can't launch: ${_pkg} isn't installed${_qrHostStr ? ' on ' + _qrHostStr : ''}. Install it first:\n${_hint}`,
              );
              return;
            }
            ...
            if (_minVer && _curVer && _cmpSemver(_curVer, _minVer) < 0) {
              ...
              const _hint =
                _qrRunBackend === 'vllm'
                  ...
                  : "pip install -U 'sglang[all]'";
              uiModule.showError(
                `Can't launch: ${modelData.name} needs ${_pkg} ≥ ${_minVer}, but ${_curVer} is installed${_qrHostStr ? ' on ' + _qrHostStr : ''}. Upgrade:\n${_hint}`,
              );
              return;
            }
          }
      ...
      const gpuMem =
        (grp && grp.vram_each) ||
        system.gpu_vram_gb / (system.gpu_count || 1) ||
        20;
      ...
      if (poolCount > 0 && !_tpOpts.includes(poolCount))
        _tpOpts.push(poolCount);
      let tp = _tpOpts[_tpOpts.length - 1] || 1;
      for (const n of _tpOpts) {
      ...
      let cudaDevices = '';
      if (grp && Array.isArray(grp.indices))
        cudaDevices = grp.indices.slice(0, tp).join(',');
      ...
      try {
        ...
        if (data.ok) {
          ...
          _addTask(data.session_id, shortName, 'serve', {
            _cmd: cmd,
          ...
          const runTab = document.querySelector(
            '.cookbook-tab[data-backend="Running"]',
          );
    ...
    configBtn.addEventListener('click', async () => {
      ...
      const downloaded =
        ...
        (_cachedModelIds.has(repo) ||
          [..._cachedModelIds].some(
            (id) => id === repo || id.endsWith('/' + short),
          ));
      if (_cachedModelIds && !downloaded) {
        uiModule.showToast(
          'Download the model first, then configure from Serve tab',
        );
        return;
      }
      ...
      try {
        ...
        uiModule.showToast(
          'Could not open Serve: ' + (e && e.message ? e.message : e),
        );
      }
    });
  }
...
const _HWFIT_ENGINE_GLYPHS = {
  ...
  llamacpp:
    '<svg width="14" height="14" viewBox="0 0 600 600" fill="none" aria-hidden="true"><path d="M600 392L504.249 558L504.137 557.929C487.252 584.069 458.193 600 426.864 600H120L240 392H600Z" fill="currentColor"></path><path d="M240 392H0L199.602 46.0254C216.032 17.5463 246.411 0 279.29 0H466.154L240 392Z" fill="currentColor"></path></svg>',
  ollama:
    '<span aria-hidden="true" style="display:block;width:14px;height:14px;background:currentColor;-webkit-mask:url(/static/icons/ollama-mark-crop.png) center/contain no-repeat;mask:url(/static/icons/ollama-mark-crop.png) center/contain no-repeat;"></span>',
  diffusers:
    '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="4"></circle><path d="M12 2v3M12 19v3M2 12h3M19 12h3M5 5l2 2M17 17l2 2M5 19l2-2M17 7l2-2"></path></svg>',
};
...
function _bindHwfitEnginePicker(engine) {
  ...
  if (!engine || !wrap || !btn || !menu || wrap.dataset.enginePickerBound)
    return;
  ...
  const currentLabel = () => {
    const opt = Array.from(engine.options).find(
      (o) => o.value === engine.value,
    );
  ...
  const renderMenu = () => {
    menu.innerHTML = Array.from(engine.options)
      .map(
        (opt) =>
...
export function _hwfitInit() {
  ...
  if (ctx && !ctx.dataset.bound) {
    ...
    ctx.addEventListener('change', () => {
      const targetCtx = _ctxValue();
      try {
        localStorage.setItem(_CTX_KEY, String(targetCtx));
      } catch {}
  ...
  function _rebuildServerSelect() {
    ...
    for (const sel of selectors) {
      ...
      const localColor = /^#[0-9a-fA-F]{6}$/.test(
        String(localSrv.color || '').trim(),
      )
        ? String(localSrv.color).trim()
        : '';
      ...
      _envState.servers.forEach((s, i) => {
        ...
        const color = /^#[0-9a-fA-F]{6}$/.test(String(s.color || '').trim())
          ? String(s.color).trim()
          : '';
  ...
  function _syncServers() {
    ...
    entries.forEach((entry) => {
      ...
      const host =
        hostEl?.disabled || hostEl?.readOnly ? '' : hostEl?.value.trim() || '';
      const port = row.querySelector('.cookbook-srv-port')?.value.trim() || '';
      const env = row.querySelector('.cookbook-srv-env')?.value || 'none';
      const envPath =
        row.querySelector('.cookbook-srv-path')?.value.trim() || '';
      const colorRaw =
        row.querySelector('.cookbook-srv-color')?.value?.trim() || '';
      ...
      _envState.servers.push({
        ...
        modelDirs,
        modelDir:
          modelDirs.filter((d) => d !== '~/.cache/huggingface/hub')[0] ||
          modelDirs[0],
        downloadDir,
        platform,
      });
  ...
  async function _testServerConnection(entry) {
    ...
    try {
      ...
      if (data.exit_code === 0 && out.startsWith('ok')) {
        ...
        const err = (data.stderr || data.stdout || `exit ${data.exit_code}`)
          .toString()
          .trim()
          .slice(0, 240);
  ...
  async function _populateServerKeyPanel(entry, generate = false) {
    ...
    if (
      ...
      (port && !/^\d{1,5}$/.test(port))
    ) {
      cmdBox.value =
        'Use a plain SSH target like user@host and an optional numeric port.';
      if (copyBtn) copyBtn.disabled = true;
      return;
    }
    if (genBtn) {
  ...
  function _wireServerEntry(entry) {
    ...
    if (_defBtn && !_defBtn.dataset.bound) {
      ...
      _defBtn.addEventListener('click', (e) => {
        ...
        document.querySelectorAll('.cookbook-srv-default').forEach((b) => {
          const on =
            !!_envState.defaultServer &&
            b.dataset.srvKey === _envState.defaultServer;
          b.classList.toggle('active', on);
          b.innerHTML = _serverDefaultHtml(on);
          b.title = on
            ? 'Default server — Cookbook opens here'
        ...
        uiModule.showToast(
          _envState.defaultServer
            ? 'Default server: ' +
                (_envState.defaultServer === 'local'
                  ? 'Local'
                  : defaultSrv?.name || defaultSrv?.host || 'selected server')
    ...
    if (keyGenBtn && !keyGenBtn.dataset.bound) {
      ...
      keyGenBtn.addEventListener('click', () =>
        _populateServerKeyPanel(entry, true),
      );
    }
    ...
    if (keyCopyBtn && !keyCopyBtn.dataset.bound) {
      ...
      keyCopyBtn.addEventListener('click', async () => {
        const cmd =
          entry.querySelector('.cookbook-server-key-command')?.value?.trim() ||
          '';
    ...
    entry.querySelectorAll('input, select').forEach((el) => {
      el.addEventListener('change', () => {
        ...
        const color =
          entry.querySelector('.cookbook-srv-color')?.value?.trim() || '';
        ...
        if (hasColor) {
          ...
          const autoColor = (
            colorWrap?.style.getPropertyValue('--cookbook-server-color') ||
            entry.style.getPropertyValue('--cookbook-server-color') ||
            ''
          ).trim();
        ...
        if (
          !entry
            .querySelector('.cookbook-server-key-panel')
    ...
    entry
      .querySelectorAll('.cookbook-srv-host, .cookbook-srv-port')
      .forEach((el) => {
      ...
      saveBtn.addEventListener('click', () => {
        ...
        try {
          document.dispatchEvent(
            new CustomEvent('cookbook:servers-changed', {
        ...
        saveBtn.innerHTML =
          '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#50fa7b" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;flex-shrink:0;"><polyline points="20 6 9 17 4 12"/></svg>Saved';
    ...
    if (rmBtn)
      rmBtn.addEventListener('click', async () => {
        ...
        if (uiModule && uiModule.styledConfirm) {
          ok = await uiModule.styledConfirm(`Remove "${name}"?`, {
            confirmText: 'Remove',
            danger: true,
          });
        } else {
        ...
        try {
          ...
          if (data.ok) {
            ...
            if (data.platform) {
              ...
              _syncServers();
              // Show platform badge
              const existingBadge = entry.querySelector(
                '.cookbook-platform-badge',
              );
            ...
            if (data.platform === 'termux') {
              ...
              if (container) {
                const existing = [
                  ...container.querySelectorAll('.cookbook-modeldir-tag'),
                ].map((t) =>
                  t.textContent
                    .replace('\u2716', '')
                    .replace('\u2715', '')
                ...
                if (!existing.includes(termuxDir)) {
                  ...
                  tag
                    .querySelector('.cookbook-modeldir-rm')
                    .addEventListener('click', () => {
                      tag.remove();
                      _syncServers();
                    });
    ...
    if (addDirBtn)
      addDirBtn.addEventListener('click', () => {
        ...
        const existing = [
          ...entry.querySelectorAll('.cookbook-modeldir-tag'),
        ].some((t) => (t.dataset.dir || t.textContent.trim()) === dir);
        if (existing) return;
        const container = entry.querySelector('.cookbook-modeldirs');
        const tag = document.createElement('span');
        tag.className = 'cookbook-modeldir-tag';
        tag.dataset.dirIdx = container.querySelectorAll(
    ...
    entry.querySelectorAll('.cookbook-modeldir-rm').forEach((rm) => {
      rm.addEventListener('click', () => {
        rm.closest('.cookbook-modeldir-tag').remove();
        _syncServers();
      });
    });
    ...
    entry
      .querySelectorAll('.cookbook-modeldir-dl')
      .forEach((dl) => _wireModelDirTarget(entry, dl));
  }
  ...
  function _wireModelDirTarget(entry, dlEl) {
    ...
    tag.addEventListener('click', (e) => {
      ...
      entry.querySelectorAll('.cookbook-modeldir-dl').forEach((d) => {
        ...
        d.closest('.cookbook-modeldir-tag')?.classList.remove(
      ...
      uiModule.showToast(
        dlEl.dataset.dlDir
          ? 'Downloads \u2192 ' + dlEl.dataset.dlDir
          : 'Downloads \u2192 default HF cache',
      );
    });
  }
  ...
  if (addBtn && !addBtn.dataset.bound) {
    ...
    addBtn.addEventListener('click', () => {
      ...
      wrap.innerHTML = _serverEntryHtml(
        blank,
        idx,
        _envState.defaultServer || '',
        true,
  ...
  if (serverSelect && !serverSelect.dataset.bound) {
    ...
    serverSelect.addEventListener('change', () => {
      ...
      document
        .querySelectorAll(
          '#hwfit-dl-server, #hwfit-cache-server, #hwfit-deps-server',
        )
        .forEach((sel) => {
