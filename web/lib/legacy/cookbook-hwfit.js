...
export async function _hwfitFetch(fresh = false) {
  ...
  if (_cached) {
    ...
    setTimeout(() => {
      if (loadingLbl.isConnected) loadingLbl.textContent = 'Scanning hardware…';
    }, 2000);
    list.innerHTML = '';
    ...
    if (_defBtn && !_defBtn.dataset.bound) {
      ...
      _defBtn.addEventListener('click', (e) => {
        ...
        document.querySelectorAll('.cookbook-srv-default').forEach((b) => {
          ...
          b.innerHTML =
            (on ? _MODELDIR_CHECK_ON : _MODELDIR_CHECK_OFF) +
            '<span class="cookbook-srv-default-label">default</span>';
          b.title = on
